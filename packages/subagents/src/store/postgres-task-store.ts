...
export function createPostgresSubagentSchemaSql(
  ...
  const taskTable = sanitizeIdentifier(options.taskTableName ?? "subagent_tasks");
...
export class PostgresTaskStore implements TaskStore {
  ...
  async put(task: BackgroundTask): Promise<void> {
    await this.client.query(
      ...
      [task.id, structuredClone(task), task.status, task.parentRunId, task.endedAt ?? null],
    );
  }
...
export class PostgresTaskQueue implements TaskQueue {
  ...
  constructor(options: PostgresTaskQueueOptions) {
    this.client = options.client;
    this.tableName = sanitizeIdentifier(options.tableName ?? "subagent_task_queue");
    this.workerId =
      options.workerId ?? `pg-worker-${process.pid}-${Math.random().toString(36).slice(2)}`;
...
export async function recoverStaleRunningTasks(
  ...
  for (const task of running) {
    ...
    const patch: Partial<BackgroundTask> =
      action === "requeue"
        ? { status: "queued", startedAt: undefined, error: "stale_running_task_recovered" }
        : {
            status: "failed",
...
function rowToVersionedTask(row: Record<string, unknown> | undefined): VersionedTask | null {
  if (!row) return null;
