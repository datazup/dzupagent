...
export class HostTaskQueue implements TaskQueue {
  ...
  constructor(options: HostTaskQueueOptions) {
    ...
    this.workerId =
      options.workerId ?? `worker-${process.pid}-${Math.random().toString(36).slice(2)}`;
  ...
  private async claimNext(): Promise<(QueueRecord & { leasedBy: string }) | null> {
  ...
  private async readRecords(): Promise<QueueRecord[]> {
    try {
      const parsed = JSON.parse(await readFile(this.queuePath(), "utf8")) as unknown;
