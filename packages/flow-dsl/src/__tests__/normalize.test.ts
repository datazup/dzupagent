...
describe('normalizeDslDocument', () => {
  ...
  describe('valid minimal flow', () => {
    ...
    it('rejects malformed top-level policy', () => {
      const raw = makeMinimalRaw({ policy: { budgetCents: 0, timeoutMs: 'fast' } })
      const { document, partialDocument, diagnostics } = normalizeDslDocument(raw)
      expect(document).toBeNull()
      expect(partialDocument?.policy).toEqual({})
      expect(diagnostics.some((d) => d.path === 'root.policy.budgetCents')).toBe(true)
      expect(diagnostics.some((d) => d.path === 'root.policy.timeoutMs')).toBe(true)
    })
...
// ---------------------------------------------------------------------------
// normalizeSteps — agent node
// ---------------------------------------------------------------------------

describe('normalizeSteps — agent', () => {
  it('accepts a template-backed agent without literal instructions', () => {
    const raw = [{
      agent: {
        id: 'agent-step',
        agentId: 'reviewer',
  ...
  it('rejects an agent without instructions or valid template.ref', () => {
    const raw = [{
      agent: {
        agentId: 'reviewer',
        template: { inputDefaults: { severity: 'high' } },
        output: {
          key: 'review',
          schema: { type: 'object' },
        },
    ...
    expect(diagnostics.some((d) => d.path === 'root.steps[0].template.ref')).toBe(true)
    expect(diagnostics.some((d) => d.path === 'root.steps[0].instructions')).toBe(true)
  })

  it('preserves inline agent validate blocks', () => {
    const raw = [{
      agent: {
        agentId: 'reviewer',
        ...
        validate: {
          ...
          errorMessage: 'Review status is required.',
          failBehavior: 'retry',
          maxRetries: 2,
        },
      },
  ...
  it('rejects malformed inline agent validate blocks', () => {
    const raw = [{
      agent: {
        agentId: 'reviewer',
        instructions: 'Review the change.',
    ...
    const diagnostics: Parameters<typeof normalizeSteps>[2] = []
    normalizeSteps(raw, 'root.steps', diagnostics)
    expect(diagnostics.some((d) => d.path === 'root.steps[0].validate.schema')).toBe(true)
    expect(diagnostics.some((d) => d.path === 'root.steps[0].validate.failBehavior')).toBe(true)
    expect(diagnostics.some((d) => d.path === 'root.steps[0].validate.maxRetries')).toBe(true)
  })

  it('keeps unknown agent key rejection strict', () => {
    const raw = [{
      agent: {
        agentId: 'reviewer',
        instructions: 'Review the change.',
        ...
        unknownAgentKey: true,
      },
    }]
    const diagnostics: Parameters<typeof normalizeSteps>[2] = []
    normalizeSteps(raw, 'root.steps', diagnostics)
    expect(diagnostics.some((d) => d.code === 'UNSUPPORTED_FIELD'
      && d.path === 'root.steps[0].unknownAgentKey')).toBe(true)
  })
})
