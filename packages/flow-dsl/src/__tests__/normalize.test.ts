...
describe("normalizeDslDocument", () => {
  ...
  describe("steps array", () => {
    it("errors when steps is missing", () => {
      ...
      expect(
        diagnostics.some(
          (d) => d.code === "MISSING_REQUIRED_FIELD" && d.path === "root.steps"
        )
      ).toBe(true);
    });
  ...
  describe("inputs", () => {
    it("normalizes shorthand string type input", () => {
      ...
      const { document, diagnostics } = normalizeDslDocument(raw);
      expect(
        diagnostics.filter((d) => d.path?.startsWith("root.inputs")).length
      ).toBe(0);
      expect(document?.inputs?.["name"]).toEqual({
        type: "string",
        required: true,
      });
    });
    ...
    it("preserves JSON-like (nested array/object) input defaults", () => {
      ...
      const raw = makeMinimalRaw({
        inputs: {
          payload: {
            type: "object",
            default: { nested: ["a", 1, true, null] },
          },
        },
      });
      const { document, diagnostics } = normalizeDslDocument(raw);
      expect(diagnostics).toEqual([]);
      expect(document?.inputs?.["payload"]).toEqual({
        type: "object",
        default: { nested: ["a", 1, true, null] },
      });
    });
    ...
    it("errors on unsupported input type", () => {
      ...
      const { diagnostics } = normalizeDslDocument(raw);
      expect(diagnostics.some((d) => d.code === "INVALID_INPUT_SPEC")).toBe(
        true
      );
    });
    ...
    it("errors on non-object inputs value", () => {
      ...
      expect(
        diagnostics.some(
          (d) => d.code === "INVALID_INPUT_SPEC" && d.path === "root.inputs"
        )
      ).toBe(true);
    });
    ...
    it("accepts all valid input types", () => {
      ...
      for (const t of types) {
        ...
        const { diagnostics } = normalizeDslDocument(raw);
        const inputErrors = diagnostics.filter(
          (d) => d.path === `root.inputs.x`
        );
  ...
  describe("defaults", () => {
    ...
    it("normalizes retry config", () => {
      const raw = makeMinimalRaw({
        defaults: { retry: { attempts: 3, delayMs: 100 } },
      });
      const { document } = normalizeDslDocument(raw);
    ...
    it("errors when defaults is not an object", () => {
      ...
      expect(
        diagnostics.some(
          (d) =>
            d.code === "INVALID_TOP_LEVEL_SHAPE" && d.path === "root.defaults"
        )
      ).toBe(true);
    });
  });
...
describe("normalizeSteps — action", () => {
  ...
  it("errors when action.ref is missing", () => {
    ...
    expect(diagnostics.some((d) => d.code === "MISSING_REQUIRED_FIELD")).toBe(
      true
    );
  });
...
describe("normalizeSteps — parallel", () => {
  it("normalizes a valid parallel with two branches", () => {
    ...
    expect(
      diagnostics.filter((d) => d.code !== "INVALID_NODE_SHAPE")
    ).toHaveLength(0);
  });
...
describe("normalizeSteps — for_each", () => {
  ...
  it("errors when for_each.source is missing", () => {
    ...
    expect(
      diagnostics.some(
        (d) => d.code === "MISSING_REQUIRED_FIELD" && d.path.includes("source")
      )
    ).toBe(true);
  });
  ...
  it("errors when for_each.as is missing", () => {
    ...
    expect(
      diagnostics.some(
        (d) => d.code === "MISSING_REQUIRED_FIELD" && d.path.includes(".as")
      )
    ).toBe(true);
  });
...
describe("normalizeSteps — classify", () => {
  it("normalizes a classify node with an explicit default choice", () => {
    ...
    expect((nodes[0] as { defaultChoice?: string }).defaultChoice).toBe(
      "infra"
    );
    expect(diagnostics).toHaveLength(0);
  });
  ...
  it("errors when default choice is not one of the classify choices", () => {
    ...
    const nodes = normalizeSteps(raw, "root.steps", diagnostics);
    expect(
      (nodes[0] as { defaultChoice?: string }).defaultChoice
    ).toBeUndefined();
    expect(
      diagnostics.some(
        (d) =>
          d.code === "INVALID_ENUM_VALUE" && d.path === "root.steps[0].default"
      )
    ).toBe(true);
  });
});
...
describe("checkpoint node", () => {
    ...
    expect(
      diagnostics.some(
        (d) =>
          ...
          d.path === "root.steps[0].captureOutputOf"
      )
    ).toBe(true);
  });
  ...
  it("errors when label is non-string", () => {
    ...
    expect(
      diagnostics.some(
        (d) =>
          d.code === "INVALID_NODE_SHAPE" && d.path === "root.steps[0].label"
      )
    ).toBe(true);
  });
  ...
  it("reports unsupported fields on checkpoint", () => {
    ...
    expect(
      diagnostics.some(
        (d) =>
          d.code === "UNSUPPORTED_FIELD" && d.path === "root.steps[0].unknown"
      )
    ).toBe(true);
  });
});
  ...
  it("errors when checkpointLabel is missing", () => {
    ...
    expect(
      diagnostics.some(
        (d) =>
          d.code === "MISSING_REQUIRED_FIELD" &&
          d.path === "root.steps[0].checkpointLabel"
      )
    ).toBe(true);
  });
    ...
    expect(
      diagnostics.some(
        (d) =>
          ...
          d.path === "root.steps[0].onNotFound"
      )
    ).toBe(true);
  });
});
