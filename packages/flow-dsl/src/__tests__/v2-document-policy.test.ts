...
describe("DSL-MVP-POLICY-01 document policy", () => {
  it.each<[string, FlowDocumentPolicy | undefined]>([
    ["absent", undefined],
    ["budget", { budgetCents: 125 }],
    ["timeout", { timeoutMs: 5000 }],
    ["directory", { workingDirectory: "/workspace/packet" }],
    ["full", FULL_POLICY],
    ["empty", {}],
    ["empty directory", { workingDirectory: "" }],
  ])(
    "preserves %s policy through parsing, formatting and exact migration",
    (_name, policy) => {
      ...
      if (report.candidateSource === undefined)
        throw new Error("missing exact candidate");
      expect(parseDslToDocument(report.candidateSource).document).toEqual(
        v1.document,
      );
  ...
  it.each([
    ...
    ["array", []],
    ["string", "125"],
    ["string budget", { budgetCents: "125" }],
    ["zero budget", { budgetCents: 0 }],
    ["negative budget", { budgetCents: -1 }],
    ["string timeout", { timeoutMs: "5000" }],
    ["zero timeout", { timeoutMs: 0 }],
    ["negative timeout", { timeoutMs: -1 }],
    ["numeric directory", { workingDirectory: 1 }],
  ])(
    "rejects %s with policy diagnostics and no migration candidate",
    (_name, policy) => {
      const parsed = parseDslToDocument(source("v2", policy));
      expect(parsed.ok).toBe(false);
      expect(parsed.document).toBeNull();
      const diagnostic = parsed.diagnostics.find((item) =>
        item.path?.startsWith("root.policy"),
      );
      const expectedPath =
        policy !== null && typeof policy === "object" && !Array.isArray(policy)
          ? `root.policy.${Object.keys(policy)[0]}`
          : "root.policy";
      expect(diagnostic?.path).toBe(expectedPath);
  ...
  it.each([Number.NaN, Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY])(
    "rejects non-JSON numeric policy %s at authoring",
    (value) => {
  ...
  it("preserves semantic identity across key order while retaining source-specific hashes", () => {
    const reordered = {
      workingDirectory: FULL_POLICY.workingDirectory,
      timeoutMs: 5000,
      budgetCents: 125,
    };
    ...
    expect(
      formatted(FULL_POLICY).canonicalSource.indexOf("\npolicy:"),
    ).toBeLessThan(formatted(FULL_POLICY).canonicalSource.indexOf("\nsteps:"));
  });

  it.each<FlowDocumentPolicy>([
    { ...FULL_POLICY, budgetCents: 126 },
    { ...FULL_POLICY, timeoutMs: 5001 },
    { ...FULL_POLICY, workingDirectory: "/workspace/another-packet" },
  ])("binds each changed policy value into semantic identity: %j", (policy) => {
    expect(formatted(policy).semanticSha256).not.toBe(
      formatted(FULL_POLICY).semanticSha256,
    );
  });
  ...
  it("retains unknown-field rejection and lossy migration guards", () => {
    const unknown = formatDslV2Document({
    ...
    expect(unknown.diagnostics).toContainEqual(
      expect.objectContaining({
        ...
        path: "root.inventedPolicy",
      }),
    );
    const lossy = previewDslV1ToV2Migration(
      source("v1", FULL_POLICY).replace(
        "      result:",
