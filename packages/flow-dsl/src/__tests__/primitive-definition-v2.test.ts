...
describe("PrimitiveDefinitionV2", () => {
  it("is complete and deterministically hashed for every built-in", () => {
    expect(BUILT_IN_PRIMITIVE_DEFINITIONS_V2).toHaveLength(10);
  ...
  it("generates the legacy registry view without identity drift", () => {
    expect(BUILT_IN_PRIMITIVE_DEFINITIONS_V2.map(primitiveKind)).toEqual(
      BUILT_IN_PRIMITIVES.map((definition) => definition.kind)
    );
    expect(
  ...
  it("changes the semantic hash when a semantic field changes", () => {
    ...
    const { semanticHash: _semanticHash, ...compatibility } =
