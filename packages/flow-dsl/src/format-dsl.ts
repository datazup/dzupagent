...
function formatNode(lines: string[], node: FlowNode, indentLevel: number): void {
  ...
  switch (node.type) {
    ...
    case 'fleet.dispatch':
      ...
      lines.push(`${'  '.repeat(indentLevel + 1)}repos: ${formatScalar(node.repos)}`)
      lines.push(`${'  '.repeat(indentLevel + 1)}task: ${formatScalar(node.task)}`)
      if (node.on_contract_change) lines.push(`${'  '.repeat(indentLevel + 1)}on_contract_change: ${node.on_contract_change}`)
      if (node.output) lines.push(`${'  '.repeat(indentLevel + 1)}output: ${node.output}`)
      return
    case 'fleet.gather':
      lines.push(`${indent}- type: fleet.gather`)
      pushCommon(lines, node, indentLevel + 1)
      lines.push(`${'  '.repeat(indentLevel + 1)}source: ${node.source}`)
      if (node.strategy) lines.push(`${'  '.repeat(indentLevel + 1)}strategy: ${node.strategy}`)
      if (node.output) lines.push(`${'  '.repeat(indentLevel + 1)}output: ${node.output}`)
      return
    ...
    case 'knowledge.write':
      ...
      lines.push(`${'  '.repeat(indentLevel + 1)}entry: ${formatScalar(node.entry)}`)
      return
    case 'knowledge.query':
      lines.push(`${indent}- type: knowledge.query`)
      pushCommon(lines, node, indentLevel + 1)
      lines.push(`${'  '.repeat(indentLevel + 1)}filter: ${formatScalar(node.filter)}`)
