...
function projectNode(node: FlowNode, state: ProjectionState): ProjectionResult {
  ...
  switch (node.type) {
    ...
    case "try_catch": {
      ...
      for (const entry of body.entryIds)
        pushEdge(state, `${id}__body__${entry}`, id, entry, "body");
      const catchBody = projectSequence(node.catch, state);
      for (const entry of catchBody.entryIds)
        pushEdge(state, `${id}__catch__${entry}`, id, entry, "catch");
    ...
    case "sequence": {
      ...
      if (inner.entryIds.length > 0) {
        for (const entry of inner.entryIds)
          pushEdge(state, `${id}__${entry}`, id, entry);
      }
      return {
        entryIds: [id],
        exitIds: inner.exitIds.length > 0 ? inner.exitIds : [id],
      };
    }
    case "for_each":
    case "persona":
    case "route": {
      ...
      return {
        entryIds: [id],
        exitIds: body.exitIds.length > 0 ? body.exitIds : [id],
      };
    }
    case "branch": {
      const thenBody = projectSequence(node.then, state);
      for (const entry of thenBody.entryIds)
        pushEdge(state, `${id}__then__${entry}`, id, entry, "then");
      ...
      if (node.else && node.else.length > 0) {
        ...
        for (const entry of elseBody.entryIds)
          pushEdge(state, `${id}__else__${entry}`, id, entry, "else");
      }
      return {
        entryIds: [id],
        exitIds:
          [...thenBody.exitIds, ...elseExits].length > 0
            ? [...thenBody.exitIds, ...elseExits]
            : [id],
      };
    }
    case "approval": {
      const approve = projectSequence(node.onApprove, state);
      for (const entry of approve.entryIds)
        pushEdge(state, `${id}__approve__${entry}`, id, entry, "approve");
      const exits = [...approve.exitIds];
      if (node.onReject && node.onReject.length > 0) {
        const reject = projectSequence(node.onReject, state);
        for (const entry of reject.entryIds)
          pushEdge(state, `${id}__reject__${entry}`, id, entry, "reject");
    ...
    case "parallel": {
      ...
      node.branches.forEach((branch, index) => {
        ...
        for (const entry of projected.entryIds)
          pushEdge(state, `${id}__${label}__${entry}`, id, entry, label);
...
function labelForNode(node: FlowNode): string {
  ...
  switch (node.type) {
    ...
    case "worker.dispatch":
      return `worker.dispatch:${node.dispatchId}`;
    case "fleet.dispatch":
