...
async function createTask(app: Hono): Promise<string> {
  ...
  const body = await res.json() as JsonRpcResponse & { result?: { id: string } }
...
describe('A2A push notification security', () => {
  it('rejects internal callback URLs before storage', async () => {
    ...
    const body = await res.json() as JsonRpcResponse & { error?: { code: number; message: string } }
    expect(body.error?.code).toBe(-32602)
    expect(body.error?.message).toContain('A2A push callback URL rejected')
  ...
  it('allows public HTTPS callbacks and does not echo stored tokens', async () => {
    ...
    expect((await store.get(taskId))?.pushNotificationConfig?.token).toBe('secret-token')
  ...
  it('revalidates redirects and blocks redirects to internal addresses', async () => {
    ...
    const task: A2ATask = {
      ...
      createdAt: '2026-04-01T00:00:00.000Z',
      updatedAt: '2026-04-01T00:00:01.000Z',
      messages: [{ role: 'user', parts: [{ type: 'text', text: 'sensitive message' }] }],
    ...
    await expect(deliverA2APushNotification(task, {
  ...
  it('supports explicit allowlists for intentional internal callbacks', async () => {
    ...
    await expect(store.setPushConfig(task.id, {
