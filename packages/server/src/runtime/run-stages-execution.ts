...
export async function dispatchExecutionStage(options: {
  ...
  const executeRun = () =>
    options.workerOptions.runExecutor({
  ...
  const tokenUsage = isStructuredResult(execution)
    ? execution.tokenUsage
    : undefined;
  const costCents = isStructuredResult(execution)
    ? execution.costCents
    : undefined;
  const metadata = isStructuredResult(execution)
    ? execution.metadata
    : undefined;
  ...
  const compressionLog =
    ...
    execution.compressionLog.length > 0
      ? execution.compressionLog
      : undefined;
  const mergedMetadata =
    metadata || compressionLog
      ? {
          ...(metadata ?? {}),
  ...
  const tokenLifecycleForOutput =
    isRecord(metadata) && isRecord(metadata["tokenLifecycleReport"])
      ? metadata["tokenLifecycleReport"]
      : undefined;
  const finalOutput =
    tokenLifecycleForOutput && isRecord(output)
  ...
  try {
    ...
    if (currentIntent && currentIntent !== "unknown") {
      ...
      if (priorContext) {
        ...
        await options.runStore.addLog(options.job.runId, {
          ...
          data: {
            fromIntent: priorContext.fromIntent,
            tokenEstimate: priorContext.tokenEstimate,
          },
        });
      }
    }
    ...
    await options.runStore
      .addLog(options.job.runId, {
        ...
        // ERR-L-02: route nested addLog failure to stderr so it is observable
      })
