...
describe("Learning routes", () => {
  ...
  describe("GET /api/learning/feedback/stats", () => {
    it("returns feedback statistics", async () => {
      ...
      expect(body.data.byType["correctness"]).toEqual({
        approved: 1,
        rejected: 0,
      });
    });
  ...
  describe("Error handling", () => {
    ...
    beforeEach(() => {
      // logRouteError emits a structured error log; spy + silence it so we can
      // assert observability without flooding test output.
    ...
    it("returns 500 when store fails on dashboard — sanitized body, logged", async () => {
      ...
      const body = (await res.json()) as { success: boolean; error: string };
      expect(body.success).toBe(false);
      // DZUPAGENT-ERR-H-01/02/03: raw internal error text must NOT leak to client.
      expect(body.error).toBe("Internal server error");
      expect(body.error).not.toContain("Store connection failed");
      // ...but the failure is still observable server-side.
    ...
    it("returns 500 when store fails on trends/cost", async () => {
      ...
      const body = (await res.json()) as { success: boolean; error: string };
      expect(body.success).toBe(false);
      // DZUPAGENT-ERR-H-01/02/03: raw internal error text must NOT leak.
      expect(body.error).toBe("Internal server error");
      expect(body.error).not.toContain("Store connection failed");
      expect(errorSpy).toHaveBeenCalled();
    });
    ...
    it("returns 500 with non-Error thrown values", async () => {
      const throwStringService: MemoryServiceLike = {
        get: (() => {
          throw "string error";
        }) as MemoryServiceLike["get"],
        search: (() => {
          throw "string error";
        }) as MemoryServiceLike["search"],
        put: (() => {
          throw "string error";
      ...
      const body = (await res.json()) as { success: boolean; error: string };
      expect(body.success).toBe(false);
      // Non-Error thrown values are also sanitized — raw value never leaks.
      expect(body.error).toBe("Internal server error");
      expect(body.error).not.toContain("string error");
      expect(errorSpy).toHaveBeenCalled();
    });
  });
  ...
  describe("GET /api/learning/dashboard — deep", () => {
    ...
    it("dashboard truncates lessons, rules, skills to 20 items", async () => {
      for (let i = 0; i < 25; i++) {
        ...
        await memoryService.put("skills", scope, `skill-${i}`, {
          name: `skill-${i}`,
        });
      }
  ...
  describe("GET /api/learning/feedback/stats — deep", () => {
    it("computes approvalRate correctly", async () => {
      ...
      await memoryService.put("feedback", scope, "fb-a", {
        approved: true,
        type: "q",
      });
      await memoryService.put("feedback", scope, "fb-b", {
        approved: true,
        type: "q",
      });
      await memoryService.put("feedback", scope, "fb-c", {
        approved: true,
        type: "q",
      });
      await memoryService.put("feedback", scope, "fb-d", {
        approved: false,
        type: "q",
      });
    ...
    it("groups feedback by type correctly with multiple types", async () => {
      ...
      await memoryService.put("feedback", scope, "fb-e", {
        approved: false,
        type: "style",
      });
      ...
      expect(body.data.byType["quality"]).toEqual({ approved: 1, rejected: 1 });
      expect(body.data.byType["accuracy"]).toEqual({
        approved: 2,
        rejected: 0,
      });
    ...
    it('defaults type to "general" for feedback without type field', async () => {
      await memoryService.put("feedback", scope, "fb-no-type", {
        approved: true,
      });
  ...
  describe("POST /api/learning/skill-packs/load — deep", () => {
    ...
    it("skips non-string items in packIds array", async () => {
      const res = await app.request("/api/learning/skill-packs/load", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          packIds: ["pack-valid", 123, null, "pack-also-valid"],
        }),
      });
      expect(res.status).toBe(200);
  ...
  describe("GET /api/learning/lessons — deep", () => {
    it("filters by both nodeId and taskType simultaneously", async () => {
      ...
      const res = await app.request(
        "/api/learning/lessons?nodeId=generate&taskType=backend"
      );
      const body = (await res.json()) as {
    ...
    it("returns empty when taskType filter matches nothing", async () => {
      ...
      const res = await app.request(
        "/api/learning/lessons?taskType=nonexistent"
      );
      const body = (await res.json()) as { data: unknown[] };
      expect(body.data).toHaveLength(0);
    });
    ...
    it("returns empty when combined filters match nothing", async () => {
      ...
      const res = await app.request(
        "/api/learning/lessons?nodeId=generate&taskType=security"
      );
      const body = (await res.json()) as { data: unknown[] };
      expect(body.data).toHaveLength(0);
    });
    ...
    it("applies limit after filtering", async () => {
      ...
      const res = await app.request(
        "/api/learning/lessons?nodeId=generate&limit=1"
      );
      const body = (await res.json()) as { data: unknown[] };
      expect(body.data).toHaveLength(1);
    });
  ...
  describe("Tenant ID isolation", () => {
    it("uses defaultTenantId when no context tenantId", async () => {
      ...
      await memoryService.put("lessons", scope, "lesson-t", {
        text: "tenant lesson",
      });
  ...
  describe("Feedback stats with no feedback", () => {
    ...
    it("GET /feedback/stats with empty byType map", async () => {
      const res = await app.request("/api/learning/feedback/stats");
      const body = (await res.json()) as {
        data: { byType: Record<string, unknown> };
      };
  ...
  describe("Skill-packs list after loading", () => {
    it("GET /skill-packs after loading 3 packs returns correct IDs", async () => {
      await app.request("/api/learning/skill-packs/load", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          packIds: ["pack-ts", "pack-react", "pack-node"],
        }),
      });
  ...
  describe("Lessons with nodeId filter", () => {
    it("GET /lessons?nodeId=agent-1 returns only that node lessons", async () => {
      await memoryService.put("lessons", scope, "l-a1", {
        text: "Agent-1 lesson A",
        importance: 5,
        nodeId: "agent-1",
        taskType: "gen",
      });
      await memoryService.put("lessons", scope, "l-a2", {
        text: "Agent-1 lesson B",
        importance: 3,
        nodeId: "agent-1",
        taskType: "gen",
      });
  ...
  describe("Dashboard with partial data", () => {
    it("some namespaces empty, others populated — no 500", async () => {
      ...
      await memoryService.put("lessons", scope, "l-partial", {
        text: "partial",
        importance: 1,
      });
    ...
    it("only errors namespace populated — dashboard still succeeds", async () => {
      await memoryService.put("errors", scope, "err-1", {
        message: "boom",
      ...
      const body = (await res.json()) as {
        success: boolean;
        data: { errorCount: number; lessonCount: number };
      };
      expect(body.success).toBe(true);
  ...
  describe("TenantId from context", () => {
    it("authenticated apiKey tenantId wins over legacy tenantId context and defaultTenantId", async () => {
      await memoryService.put(
        ...
        "lesson-auth",
        {
          text: "Authenticated tenant lesson",
          importance: 5,
        }
      );
      await memoryService.put(
        ...
        "lesson-legacy",
        {
          text: "Legacy tenant lesson",
          importance: 4,
        }
      );
      await memoryService.put("lessons", scope, "lesson-default", {
        text: "Default tenant lesson",
        importance: 3,
      });
    ...
    it("writes learning feedback and ingested lessons under authenticated apiKey tenantId", async () => {
      ...
      const ingest = await authApp.request("/api/learning/ingest", {
        ...
        body: JSON.stringify({
          ...
          patterns: [
            {
              pattern: "tenant-specific pattern",
              context: "auth",
              confidence: 0.9,
            },
          ],
      ...
      expect(
        ((await lessonsA.json()) as { data: unknown[] }).data
      ).toHaveLength(1);
      const feedbackStatsA = await authApp.request(
        "/api/learning/feedback/stats"
      );
      expect(
        ((await feedbackStatsA.json()) as { data: { total: number } }).data
          .total
      ).toBe(1);

      const lessonsB = await otherApp.request("/api/learning/lessons");
      expect(
        ((await lessonsB.json()) as { data: unknown[] }).data
      ).toHaveLength(0);
      const feedbackStatsB = await otherApp.request(
        "/api/learning/feedback/stats"
      );
      expect(
        ((await feedbackStatsB.json()) as { data: { total: number } }).data
          .total
      ).toBe(0);
    });
