...
describe("Benchmark routes — tenant isolation (DZUPAGENT-SEC-H-01)", () => {
  ...
  it("GET /runs returns only the requesting tenant's runs", async () => {
    ...
    // Pin `data` itself, not just `count`: a response reporting count=2 with an
    // empty `data` array would satisfy both the count check and `every()`.
    expect(bodyA.data).toHaveLength(2);
    expect(bodyA.data.filter((r) => r.id !== undefined)).toHaveLength(2);
  ...
  it("POST /runs forces metadata.tenantId to the requesting tenant even when spoofed", async () => {
    ...
    const otherTenant = await app.request(
      `/api/benchmarks/runs/${body.data.id}`,
      {
        headers: tenantHeaders("tenant-b"),
      }
    );
  ...
  it("PUT /baselines/:suiteId rejects cross-tenant run references with 404", async () => {
    ...
    // for the qa suite.
    const baselinesB = await app.request(
      "/api/benchmarks/baselines?suiteId=qa",
      {
        headers: tenantHeaders("tenant-b"),
      }
    );
  ...
  it("GET /runs?suiteId paginates within tenant scope and excludes cross-tenant rows", async () => {
    ...
    // Cross-tenant leakage is what this test exists to catch, so the row count
    // and the per-row tenant must both be pinned. `every()` alone would pass on
    // an empty `data`, i.e. on a response that returned nothing at all.
    expect(tenantABody.data).toHaveLength(3);
    expect(
      tenantABody.data.filter((r) => r.metadata?.tenantId === "tenant-a")
    ).toHaveLength(3);
  });
});
