...
describe('EventBridge', () => {
  it('broadcasts events to unfiltered clients', () => {
  ...
  it('does not send runless events to run-filtered clients', () => {
    const bus = createEventBus()
    const bridge = new EventBridge(bus)
    const ws = new MockWsClient()
  ...
  it('sends only matching runId events to run-filtered clients', () => {
    const bus = createEventBus()
    const bridge = new EventBridge(bus)
    const ws = new MockWsClient()
    bridge.addClient(ws, { runId: 'r1' })
