...
} from "../routes/openai-compat/request-mapper.js";
import type {
  AgentExecutionSpec,
  ...
  return {
    ...
    async list(filter?) {
      ...
      if (filter?.active !== undefined)
        return all.filter((a) => (a.active ?? true) === filter.active);
      return all;
    },
    async save(spec: AgentExecutionSpec) {
...
describe("createModelsRoute", () => {
  ...
  it("falls back to now() when agent.createdAt is missing", async () => {
    const app = buildApp([
      makeExecutionSpec({ id: "a1", createdAt: undefined }),
    ]);
    const res = await app.request("/");
    const body = (await res.json()) as { data: Array<{ created: number }> };
...
describe("OpenAICompletionMapper — extras", () => {
  ...
  it("mapResponseWithTools() builds a well-formed ChatCompletionResponse", () => {
    const res = mapResponseWithTools(
      "hello world",
      "model-x",
      "chatcmpl-abc",
      { totalInputTokens: 3, totalOutputTokens: 2 },
      [],
      false
    );
  ...
  it("mapToolCallInitChunk() includes id, name, and empty arguments", () => {
    const chunk = mapper.mapToolCallInitChunk(
      "call-1",
      "search",
      0,
      "model",
  ...
  it("mapRequest() preserves stop arrays and strings", () => {
    ...
    expect(mapRequest(req).options.stop).toEqual(["\n", "END"]);
  });
  ...
  it("mapResponseWithTools() created timestamp is a unix second", () => {
    const res = mapResponseWithTools(
      "out",
      "m",
      "id",
