...
describe("Integration: full request lifecycle", () => {
  ...
  it("2. POST /api/runs with a freshly-issued key returns 201", async () => {
    ...
    // SEC-H-01: route-issued keys default to the least-privileged `viewer`
    // role, which can only read runs. Creating a run requires an operator (or
    // admin/agent) role, so the run-issuing key must request it explicitly.
  ...
  it("3. GET /api/runs/:id returns the run with correct status", async () => {
    ...
    const createRes = await authedReq(
      harness.app,
      "POST",
      "/api/runs",
      rawKey,
  ...
  it("4. GET /api/runs/:id/logs returns the logs array", async () => {
    ...
    const logsRes = await authedReq(
      harness.app,
      "GET",
      `/api/runs/${created.id}/logs`,
      rawKey
    );
    ...
    const seeded = logsBody.data.find(
      (entry) => entry.message === "hello from integration test"
    );
