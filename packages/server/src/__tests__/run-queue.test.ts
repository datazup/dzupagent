...
describe("InMemoryRunQueue", () => {
  ...
  describe("binary insert priority ordering", () => {
    it("maintains priority order on enqueue without sort", async () => {
      ...
      await waitForCondition(() => processed.length === 5, {
  ...
  describe("attempts tracking", () => {
    it("sets attempts to 0 on initial enqueue", async () => {
      ...
      await waitForCondition(() => capturedJob !== null, {
  ...
  describe("dead-letter tracking", () => {
    it("clearDeadLetter removes all entries", async () => {
      ...
      await waitForCondition(() => queue.getDeadLetter().length === 1, {
  ...
  describe("cancel", () => {
    it("cancels active jobs on stop", async () => {
      ...
      await waitForCondition(() => jobStarted && queue.stats().active === 1, {
      ...
      // CODE-M-03: stop() aborts active jobs and counts them as draining, so
      // the abort-aware `active` count drops to 0 even though the job's own
      // promise has not yet settled (it is tracked in the `aborting` map).
      await queue.stop(false);
    ...
    it("cancel() returns true for an actively-running job", async () => {
      queue = new InMemoryRunQueue({ concurrency: 1 });
    ...
    it("CODE-M-03: a second cancel() still locates a job that is mid-abort", async () => {
      ...
      queue.start(async () => {
        jobStarted = true;
        // Block until the test explicitly releases it — abort alone does not
        // settle this promise, so the job remains in the `aborting` map.
        await new Promise<void>((resolve) => {
          release = resolve;
        });
      });
      ...
      // First cancel aborts and moves the job into the `aborting` map.
      expect(queue.cancel("r1")).toBe(true);
      // The abort-aware active count drops immediately.
      expect(queue.stats().active).toBe(0);

      // BEFORE the fix this returned false because the job had been dropped from
      // the only tracking map. Now the second cancel still finds it in
      // `aborting` and reports success.
      expect(queue.cancel("r1")).toBe(true);

      // Once the job promise settles, both maps drop it and a later cancel
      // returns false (nothing left to cancel).
      release?.();
      await waitForCondition(() => queue.cancel("r1") === false, {
        description: "timed out waiting for job to settle and drop tracking",
      });
    });
