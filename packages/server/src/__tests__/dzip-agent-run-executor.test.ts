...
describe("dzip-agent-run-executor", () => {
  ...
  it("wraps plain fallback output into structured result", async () => {
    ...
    // createDzupAgentRunExecutor's implementation always resolves to a
    // structured RunExecutorResult; the RunExecutor type erases this to unknown.
    const result = (await executor(baseContext())) as RunExecutorResult;
