...
import { RUN_ERROR_DETAIL_METADATA_KEY } from "../routes/route-error.js";

// DZUPAGENT-ERR-H-02: run failures redact the client-facing message to a
// generic string on run.error / agent:failed.message; the raw detail (stack /
// driver text) is retained admin-only under run.metadata[errorDetail]. These
// helpers assert the redaction WITHOUT losing the internal detail.
const REDACTED_RUN_ERROR = "Internal server error";

function runErrorDetail(run: Run): string {
  const metadata = (run.metadata ?? {}) as Record<string, unknown>;
  const detail = metadata[RUN_ERROR_DETAIL_METADATA_KEY];
...
describe("Server run-executor deep coverage (W21-B2)", () => {
  ...
  describe("Run lifecycle state transitions", () => {
    ...
    it("created → running → failed on executor throw", async () => {
      ...
      // ERR-H-02: public run.error is redacted; raw detail retained admin-only.
      expect(failed.error).toBe(REDACTED_RUN_ERROR);
      expect(runErrorDetail(failed)).toContain("unreachable provider");
    });
  ...
  describe("Event emission", () => {
    ...
    it("emits agent:completed with durationMs", async () => {
      ...
      ) as Extract<DzupEvent, { type: "agent:completed" }> | undefined;
    ...
    it("emits agent:failed with errorCode INTERNAL_ERROR when executor throws", async () => {
      ...
      // ERR-H-02: agent:failed carries the redacted safe message, never raw text.
      expect(failed?.message).toBe(REDACTED_RUN_ERROR);
    });
    ...
    it("event runIds match createdAt run id (correlation)", async () => {
      ...
      const ids = new Set(
        startedEvents.map((e) => (e as { runId: string }).runId)
      );
  ...
  describe("Cancellation", () => {
    it("pending run cancelled before execution: executor not invoked", async () => {
      ...
      // Fill slot 1
      const blocker = await runStore.create({
        agentId: "agent-main",
        input: {},
      });
      await q.enqueue({
      ...
      const cancelable = await runStore.create({
        agentId: "agent-main",
        input: {},
      });
      await q.enqueue({
  ...
  describe("Error propagation", () => {
    it("tool throws: run status → failed, error captured", async () => {
      ...
      expect(out.status).toBe("failed");
      // ERR-H-02: public run.error is redacted; raw detail retained admin-only.
      expect(out.error).toBe(REDACTED_RUN_ERROR);
      expect(runErrorDetail(out)).toContain("EACCES");
    });
    ...
    it("LLM provider error: error message persisted in run.error", async () => {
      ...
      // ERR-H-02: public run.error is redacted; raw detail retained admin-only.
      expect(out.error).toBe(REDACTED_RUN_ERROR);
      expect(runErrorDetail(out)).toContain("LLM_PROVIDER_UNAVAILABLE");
    });
    ...
    it("non-Error throw values are stringified into run.error", async () => {
      ...
      // ERR-H-02: public run.error is redacted; raw detail retained admin-only.
      expect(out.error).toBe(REDACTED_RUN_ERROR);
      expect(runErrorDetail(out)).toContain("literal string error");
    });
  });
  ...
  describe("Stream / delta path", () => {
    it("stream_delta and stream_done events can be emitted via executor", async () => {
      ...
      const deltas = streamEvents.filter(
        (e) => e.type === "agent:stream_delta"
      );
      const dones = streamEvents.filter((e) => e.type === "agent:stream_done");
      expect(deltas).toHaveLength(2);
      expect(dones).toHaveLength(1);
      expect((dones[0] as { finalContent: string }).finalContent).toBe(
        "hello world"
      );
    });
  ...
  describe("Log aggregation", () => {
    ...
    it("additional logs preserve their level attributes", async () => {
      ...
      const logs = await runStore.getLogs(run.id);
      expect(logs.some((l) => l.level === "warn" && l.phase === "policy")).toBe(
        true
      );
      expect(logs.some((l) => l.level === "error" && l.phase === "tool")).toBe(
        true
      );
    });
  });
  ...
  describe("Event ordering", () => {
    it("agent:started precedes agent:completed", async () => {
      ...
      expect(order.indexOf("agent:started")).toBeLessThan(
        order.indexOf("agent:completed")
      );
    });
  ...
  describe("RunExecutor contract", () => {
    it("accepts a vi.fn mock and receives all context fields", async () => {
      const executorMock = vi.fn<RunExecutor>(async () => ({
        output: { ok: true },
      }));
