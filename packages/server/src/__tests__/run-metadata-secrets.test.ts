...
describe("sanitizeRunMetadataForPersistence", () => {
  ...
  it("strips env and headers from mcpServers entries", () => {
    ...
    const server = (result?.mcpServers as unknown[])?.[0] as Record<
      string,
      unknown
    >;
  ...
  it("redacts named secret keys nested inside objects (SEC-L-01)", () => {
    const result = sanitizeRunMetadataForPersistence({
      provider: {
        githubToken: "gh_nested",
        httpAuthorization: "Bearer nested",
        config: { apiKey: "ak_nested", endpoint: "https://api.example.com" },
      },
      label: "keep-me",
    }) as Record<string, Record<string, unknown>>;

    const provider = result?.provider as Record<string, unknown>;
    expect(provider?.["githubToken"]).toBe("[REDACTED]");
    expect(provider?.["httpAuthorization"]).toBe("[REDACTED]");
  ...
  it("redacts secret keys inside objects nested in arrays", () => {
    const result = sanitizeRunMetadataForPersistence({
      providers: [
        { name: "gh", token: "gh_tok" },
        { name: "slack", token: "sl_tok" },
      ],
    }) as Record<string, Array<Record<string, unknown>>>;
    expect(result?.providers?.[0]?.["token"]).toBe("[REDACTED]");
    expect(result?.providers?.[0]?.["name"]).toBe("gh");
    expect(result?.providers?.[1]?.["token"]).toBe("[REDACTED]");
  });
});
...
describe("sanitizeRunForResponse", () => {
  it("sanitizes the metadata on a Run and keeps other fields intact", () => {
    ...
    const result = sanitizeRunForResponse(run) as typeof run & {
