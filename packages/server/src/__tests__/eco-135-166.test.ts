...
function createMockMemoryService(): MemoryServiceLike {
  ...
  return {
    ...
    async getKeyed(
      ...
      // Required on the contract: a record's store key is not recoverable from
      // its value, so callers needing identity read through here.
      const sk = storeKey(namespace, scope);
...
describe("Config commands", () => {
  ...
  it("configValidate returns valid for correct config", () => {
    ...
    writeFileSync(
      path,
      JSON.stringify({ port: 4000, auth: { mode: "api-key" } })
    );
  ...
  it("configShow returns parsed config", () => {
    ...
    expect((result["database"] as Record<string, unknown>)["url"]).toBe(
      "postgres://localhost"
    );
  });
...
describe("Memory browse routes", () => {
  ...
  it("GET /api/memory-browse/:namespace respects limit", async () => {
    ...
    const res = await app.request(
      `/api/memory-browse/lessons?scope=${scope}&limit=1`
    );
    expect(res.status).toBe(200);
  ...
  it("GET /api/memory-browse/:namespace with search", async () => {
    const scope = encodeURIComponent(JSON.stringify({ tenant: "t1" }));
    const res = await app.request(
      `/api/memory-browse/lessons?scope=${scope}&search=validate`
    );
    expect(res.status).toBe(200);
