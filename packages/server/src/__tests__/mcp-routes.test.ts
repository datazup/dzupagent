...
describe("MCP routes", () => {
  ...
  // --- RF-009: HTTP/SSE endpoint URL policy ---

  describe("HTTP/SSE URL policy (RF-009)", () => {
    it("allows public HTTP/SSE endpoints at registration", async () => {
      const res = await req(app, "POST", "/api/mcp/servers", {
        id: "public-sse",
        transport: "sse",
    ...
    it("rejects private HTTP endpoints at registration", async () => {
      const res = await req(app, "POST", "/api/mcp/servers", {
        id: "private-http",
        transport: "http",
        endpoint: "http://192.168.1.10:8080/mcp",
        enabled: true,
      });
      ...
      expect(json.error.message).toContain("URL policy");
    });

    it("rejects private HTTP endpoints on patch", async () => {
    ...
    it("allows private HTTP endpoints when the host is explicitly allowlisted", async () => {
      const guardedApp = createForgeApp({
        ...createTestConfig(mcpManager),
        mcpAllowedHttpHosts: ["localhost"],
      });
      const res = await req(guardedApp, "POST", "/api/mcp/servers", {
        id: "allowed-local",
        transport: "http",
    ...
    it("revalidates persisted HTTP definitions before test connections", async () => {
      await mcpManager.addServer({
      ...
      const res = await req(
        app,
        "POST",
        "/api/mcp/servers/persisted-private/test"
      );
      expect(res.status).toBe(403);
    });
  });
  ...
  // --- SEC-H-02: stdio command argument sandboxing ---

  describe("stdio command argument sandboxing (SEC-H-02)", () => {
    it("rejects POST that launches node with an inline-eval arg", async () => {
      const guardedApp = createForgeApp({
        ...createTestConfig(mcpManager),
        mcpAllowedExecutables: ["node"],
      });
      const res = await req(guardedApp, "POST", "/api/mcp/servers", {
        id: "rce-node",
        transport: "stdio",
    ...
    it("rejects POST that launches bash -c", async () => {
      const guardedApp = createForgeApp({
        ...createTestConfig(mcpManager),
        mcpAllowedExecutables: ["bash"],
      });
      const res = await req(guardedApp, "POST", "/api/mcp/servers", {
        id: "rce-bash",
        transport: "stdio",
    ...
    it("allows POST that launches node with a script path", async () => {
      const guardedApp = createForgeApp({
        ...createTestConfig(mcpManager),
        mcpAllowedExecutables: ["node"],
      });
    ...
    it("allows node -e under the legacy policy opt-in", async () => {
      const guardedApp = createForgeApp({
        ...createTestConfig(mcpManager),
        mcpAllowedExecutables: ["node"],
        mcpStdioArgPolicy: "legacy",
      });
      const res = await req(guardedApp, "POST", "/api/mcp/servers", {
        id: "legacy-node",
        transport: "stdio",
    ...
    it("rejects PATCH that adds an inline-eval arg to an allowlisted stdio command", async () => {
      const server = await mcpManager.addServer({
        id: "stdio-patch-args",
        transport: "stdio",
        endpoint: "node",
