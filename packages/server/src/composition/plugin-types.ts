 ...
 *
 * `ServerRoutePlugin` is re-exported from this module so callers can reference
 * the plugin type without importing from the route-plugin module directly. As of
 * RF-8 the plugin carries a NARROW context ({ eventBus, auth, metrics,
 * declaredServices }) rather than the full server config.
 */
