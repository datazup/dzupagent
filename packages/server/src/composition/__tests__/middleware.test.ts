...
describe("composition/middleware", () => {
  it("does not emit CORS headers by default", async () => {
    ...
    expect(warn).not.toHaveBeenCalledWith(
      expect.stringContaining("CORS is open to all origins")
    );
  });
  ...
  it("warns when framework /api routes are created without explicit auth outside production", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "/api/* routes are running without authentication"
      )
    );
  });
  ...
  it("throws a startup error when production framework /api routes omit auth", () => {
    ...
    expect(() =>
      createForgeApp(baseConfig({ corsOrigins: "https://app.example.com" }))
    ).toThrow(
      /Refusing to start production framework \/api\/\* routes without explicit auth/
    );
    expect(warn).not.toHaveBeenCalled();
  });

  it("allows explicit auth none as a development opt-out with a warning outside production", () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    ...
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining('auth.mode="none" only for local development')
    );
  });

  // SEC-M-02: an explicit `auth.mode: 'none'` is no longer a silent prod opt-out.
  it("refuses auth none in production without the unsafe opt-in (SEC-M-02)", () => {
    vi.stubEnv("NODE_ENV", "production");
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
  ...
  it("allows auth none in production with allowUnsafeNoAuthInProduction (SEC-M-02 opt-in)", () => {
    vi.stubEnv("NODE_ENV", "production");
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    ...
    expect(() =>
      createForgeApp(
        baseConfig({
          auth: { mode: "none" },
          allowUnsafeNoAuthInProduction: true,
          corsOrigins: "https://app.example.com",
        })
      )
    ).not.toThrow();
  ...
  it("honors route-specific JSON body size overrides", async () => {
    ...
    await expect(res.json()).resolves.toEqual({
      data: { message: "allowed by override" },
    });
  });
  ...
  it("applies a default rate limiter when auth is enabled without explicit rateLimit (SEC-M-03)", async () => {
    ...
    applyMiddleware(
      app,
      baseConfig({
        auth: { mode: "api-key", validateKey: async () => ({ id: "k1" }) },
      })
    );
    ...
    expect(res.status).toBe(200);
    // The default limiter advertises its window via X-RateLimit-Limit.
  ...
  it("enforces the default limit and returns 429 past the threshold (SEC-M-03)", async () => {
    ...
    // 100/min default — exhaust the bucket for a single key.
    for (let i = 0; i < 130; i++) {
      const res = await app.request("/api/runs", {
  ...
  it("rejects oversized JSON without buffering the whole body (SEC-M-04)", async () => {
    ...
    // No Content-Length header → forces the streaming size check path.
  ...
  it("allows within-limit JSON through the streaming size check (SEC-M-04)", async () => {
    ...
    expect(res.status).toBe(200);
    await expect(res.json()).resolves.toEqual({ data: { message: "small" } });
  });
  ...
  it("threads apiKeyStore.validate into the resolved auth config when no validateKey was supplied", async () => {
    ...
    const apiKeyStore = {
      validate,
    } as unknown as ForgeServerConfig["apiKeyStore"];
