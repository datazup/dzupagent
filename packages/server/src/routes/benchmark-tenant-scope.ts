...
export function getRecordTenantId(
...
export function filterRecordsByTenant<
  T extends { metadata?: Record<string, unknown> | undefined },
>(records: readonly T[], tenantId: string): T[] {
...
export class TenantScopedBenchmarkOrchestrator {
  ...
  async runSuite(input: {
    ...
    const stampedMetadata = stampTenantMetadata(
      this.opts.tenantId,
      input.metadata,
    );
  ...
  async listRuns(
    ...
    const hasMore =
      lastPage?.hasMore === true && owned.length >= requestedLimit;
  ...
  async compareRuns(
  ...
  ): Promise<Awaited<ReturnType<BenchmarkOrchestratorLike["compareRuns"]>>> {
    ...
    if (!currentRun || !recordBelongsToTenant(currentRun, this.opts.tenantId)) {
      // NotFound-prefixed so the route-error sanitizer forwards this safe,
      // deliberate cross-tenant 404 message (maps to 404 via mapErrorToStatus).
      throw new Error(`NotFound: Current run "${currentRunId}" not found`);
    }
    if (
      !previousRun ||
      !recordBelongsToTenant(previousRun, this.opts.tenantId)
    ) {
      throw new Error(`NotFound: Previous run "${previousRunId}" not found`);
    }
  ...
  async setBaseline(input: {
    ...
    if (!run || !recordBelongsToTenant(run, this.opts.tenantId)) {
      ...
      // NotFound-prefixed so the sanitizer forwards this safe message at 404.
      throw new Error(`NotFound: Run "${input.runId}" not found`);
    }
  ...
  async getBaseline(
    ...
    const baseline = await this.opts.orchestrator.getBaseline(
      suiteId,
      targetId,
    );
