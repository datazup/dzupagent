...
export async function handleCancelRun(
  ...
  return c.json({
    data: sanitizeRunForResponse({ ...run, status: "cancelled" }),
  });
}
    ...
    return c.json(
      {
        error: {
          code: "INVALID_STATE",
          message: `Cannot pause run in '${run.status}' state`,
        },
      },
      400,
    );
...
export async function handleResumeRun(
    ...
    return c.json(
      {
        error: {
          code: "INVALID_STATE",
          message: `Cannot resume run in '${run.status}' state`,
        },
      },
      400,
    );
  ...
  if (config.journal) {
    try {
      ...
      // ERR-M-02: log journal errors before degrading so the silent fall-through
      // is observable in structured logs. Status 200 because the resume path
      // continues — this is a degradation, not a hard failure.
      logRouteError(c, "runs.resume.journal", err, 200);
  ...
  if (config.runQueue && config.runExecutor) {
    const priorityRaw =
      typeof (run.metadata ?? {})["priority"] === "number"
      ...
      : checkpoint !== undefined
        ? {
            input: {
              ...
              ...(lastStateSeq !== undefined
                ? { _lastStateSeq: lastStateSeq }
                : {}),
            },
          }
        : {}),
  ...
  return c.json({
    data: {
      ...
      ...(checkpoint !== undefined ? { checkpoint } : {}),
      ...(lastStateSeq !== undefined ? { lastStateSeq } : {}),
      ...(queueAccepted !== undefined
        ? { queue: { accepted: true, ...queueAccepted } }
        : {}),
    },
  });
}
    ...
    return c.json(
      {
        error: {
          code: "NOT_CONFIGURED",
          message: "Journal is not configured; fork is unavailable",
        },
      },
      501,
    );
