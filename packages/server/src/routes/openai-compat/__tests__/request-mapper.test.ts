...
describe("validateCompletionRequest", () => {
  ...
  // RF-4 (CODE-H-01): content must be string | null.
  ...
  it("rejects non-string, non-null content", () => {
    const result = validateCompletionRequest({
      model: "a",
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error.error.param).toBe("messages[0].content");
    }
  });
  ...
  // RF-4 (CODE-H-02): tool_calls structure check.
  it("accepts well-formed tool_calls", () => {
    const result = validateCompletionRequest({
      model: "a",
      messages: [
        {
  ...
  it("rejects malformed tool_calls (missing function.name)", () => {
    const result = validateCompletionRequest({
      model: "a",
      messages: [
        {
  ...
  it("rejects malformed tool_calls (wrong type literal)", () => {
    const result = validateCompletionRequest({
      model: "a",
      messages: [
        {
  ...
  // RF-4 (SEC-L-01): sampling option bounds.
  ...
  it("rejects temperature above 2", () => {
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error.error.param).toBe("temperature");
    }
  });
  ...
  it("rejects non-positive max_tokens", () => {
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error.error.param).toBe("max_tokens");
    }
  });
  ...
  it("accepts stop as an array of up to 4 strings", () => {
    const result = validateCompletionRequest({
      model: "a",
      messages: [{ role: "user", content: "hi" }],
      stop: ["a", "b", "c", "d"],
  ...
  it("rejects stop array with more than 4 entries", () => {
    const result = validateCompletionRequest({
      model: "a",
      messages: [{ role: "user", content: "hi" }],
      stop: ["a", "b", "c", "d", "e"],
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error.error.param).toBe("stop");
    }
  });

  it("rejects stop given as a number", () => {
    const result = validateCompletionRequest({
      model: "a",
      messages: [{ role: "user", content: "hi" }],
      stop: 5,
