...
describe("OpenAI-compat completions route", () => {
  ...
  describe("POST / (non-streaming) — basic", () => {
    ...
    it("ERR-C-01: does not leak the raw error message to the client", async () => {
      // A Prisma/DB-style internal error must never reach the API client body.
      mockGenerate.mockRejectedValue(
        new Error('PRISMA: column "tenant_id" does not exist in table runs'),
      );
      ...
      // Generic, sanitized message — no table/column/Prisma internals.
  ...
  describe("GAP-2: Streaming finish_reason=length for iteration limits", () => {
    ...
    it("emits finish_reason=stop for a normal done event", async () => {
      ...
      const choice = (
        finalChunk["choices"] as Array<Record<string, unknown>>
      )[0]!;
    ...
    it("emits finish_reason=length when done event has hitIterationLimit=true", async () => {
      mockStream.mockImplementation(async function* () {
        ...
        yield {
          type: "done",
          data: { hitIterationLimit: true, stopReason: "iteration_limit" },
        };
      });
    ...
    it("emits finish_reason=length when stopReason=budget_exceeded", async () => {
      mockStream.mockImplementation(async function* () {
        yield {
          type: "done",
          data: { hitIterationLimit: true, stopReason: "budget_exceeded" },
        };
      });
    ...
    it("always terminates the SSE stream with [DONE]", async () => {
      ...
      const lastLine = dataLines[dataLines.length - 1]!.replace(
        /^data:\s*/,
        "",
      ).trim();
    ...
    it("handles error events from stream without crashing", async () => {
      ...
      expect(res.status).toBe(200);
      const text = await res.text();
      // ERR-C-01: the stream emits a sanitized error frame; the internal event
      // message ("model unavailable") must NOT be forwarded to the client.
      expect(text).toContain("server_error");
      expect(text).toContain("Internal error during streaming");
  ...
  describe("GAP-3: Non-streaming tool_calls in response", () => {
    ...
    it("sets finish_reason=tool_calls when tools were invoked", async () => {
      const messages = [
        makeAIMessageWithTools([{ id: "c1", name: "fn", args: {} }]),
      ];
    ...
    it("includes correct tool argument JSON in tool_calls", async () => {
      ...
      const messages = [
        makeAIMessageWithTools([{ id: "c1", name: "search", args }]),
      ];
      mockGenerate.mockResolvedValue(defaultGenerateResult({ messages }));
