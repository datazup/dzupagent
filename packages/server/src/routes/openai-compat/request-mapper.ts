...
/**
 * Map a single Zod issue to the OpenAI `param` path string. Top-level fields map
 * to their own name (`model`, `temperature`, …); message-array issues map to
 * `messages` (empty/missing) or `messages[i].<field>` to preserve the legacy
 * error contract that callers and tests rely on.
 */
function zodIssueToParam(issue: z.core.$ZodIssue): string | null {
  ...
  if (head === "messages") {
    // `messages` itself (too small / wrong type) → 'messages'.
    if (rest.length === 0) {
      return "messages";
    }
...
function zodIssueToMessage(issue: z.core.$ZodIssue): string {
  ...
  // Fall back to Zod's own message for anything else (e.g. malformed
  // tool_calls), prefixed with the offending param for clarity.
  return param
