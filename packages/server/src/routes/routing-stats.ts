...
/**
 * Scope a key must explicitly carry (in addition to an operator/admin role)
 * before legacy ownerless runs are included in routing telemetry. SEC-M-03.
 */
export const LEGACY_OWNERLESS_SCOPE = "routing:legacy-ownerless";
...
export function createRoutingStatsRoutes(
  config: RoutingStatsConfig
): Hono<AppEnv> {
  ...
  app.get("/routing-stats", async (c) => {
      ...
      return c.json(
        {
          error: {
            code: "FORBIDDEN",
            message: "API key must have a valid id to access routing stats",
          },
        },
        403
      );
    ...
    // SEC-M-03: Legacy ownerless runs (no ownerId) are only visible to a key
    // that BOTH holds an operator/admin role AND carries an explicit
    // legacy-access scope. Default OFF — a plain owner-scoped key never sees
    // other owners' legacy ownerless telemetry. (The route is also
    // operator/admin-gated by the centralised RBAC middleware; the role check
    // here is defense-in-depth for hosts that mount this route directly.)
    const role = typeof key?.["role"] === "string" ? key["role"] : undefined;
    const isPrivilegedRole = role === "operator" || role === "admin";
    const scopes = Array.isArray(key?.["scopes"])
      ? (key["scopes"] as unknown[]).filter(
          (s): s is string => typeof s === "string"
        )
      : [];
    const includeLegacyOwnerless =
      isPrivilegedRole && scopes.includes(LEGACY_OWNERLESS_SCOPE);
    ...
    const runs = await config.runStore.list({
      ...
      ...(requestingOwnerId
        ? {
            ...
            ...(includeLegacyOwnerless ? { includeLegacyOwnerless: true } : {}),
          }
        : {}),
    });
    ...
    for (const run of visibleRuns) {
      ...
      const tier =
        typeof meta?.["modelTier"] === "string" ? meta["modelTier"] : "unknown";
      const reason =
        typeof meta?.["routingReason"] === "string"
          ? meta["routingReason"]
          : "unknown";
      const complexity =
        typeof meta?.["complexity"] === "string"
          ? meta["complexity"]
          : "unknown";
    ...
    for (const run of visibleRuns) {
      const meta = run.metadata as Record<string, unknown> | undefined;
      const reflectionScore = meta?.["reflectionScore"] as
        | { overall?: number }
        | undefined;
