...
async function handleJsonRpcMethod(
  ...
  try {
    ...
    // ERR-H-11: route through the sanitize+log chokepoint so raw err.message
    // (DB driver text, tool internals, stack-bearing strings) never reaches the
    // external A2A client, and every failure is logged server-side.
    const { safe } = logRouteError(c, `a2a.jsonrpc.${req.method}`, err, 500);
    return createJsonRpcError(req.id, JSON_RPC_ERRORS.INTERNAL_ERROR, safe);
  }
}

export function registerJsonRpcRoute(
  app: Hono<AppEnv>,
  config: A2ARoutesConfig
): void {
  app.post("/a2a", async (c) => {
    ...
    if (Array.isArray(body)) {
      ...
      for (const req of batchResult.requests) {
        if (
          getSerializedJsonSizeBytes(req.params) > A2A_JSON_RPC_PARAMS_MAX_BYTES
        ) {
          responses.push(
            createJsonRpcError(
              req.id,
        ...
        const resp = await handleJsonRpcMethod(c, req, config, scope);
        responses.push(resp);
      }
    ...
    if (
      getSerializedJsonSizeBytes(validation.request.params) >
      A2A_JSON_RPC_PARAMS_MAX_BYTES
    ) {
      return c.json(
        createJsonRpcError(
    ...
    const response = await handleJsonRpcMethod(
      c,
      validation.request,
      config,
      scope
