...
import { notFound } from "./crud-helpers.js";
import { secureLogger } from "@dzupagent/core/utils";
import type {
  AdapterSkillBundle,
  CompiledAdapterSkill,
  AdapterProviderId,
} from "@dzupagent/agent-adapters/skills";

export function createSkillRoutes(
  config: Pick<ForgeServerConfig, "skillRegistry">
): Hono<AppEnv> {
      ...
      return c.json(
        {
          error: {
            ...
            message: "bundle with bundleId is required",
          },
        },
        400
      );
  ...
  app.get("/:provider", async (c) => {
    try {
      ...
      if (!compiler) {
        return notFound(c, `No skill compiler for provider "${provider}"`);
      }
      return c.json({
        data: {
  ...
  app.get("/:provider/:skillId", async (c) => {
    ...
    return notFound(
      c,
      `Compiled skill "${c.req.param(
        "skillId"
      )}" not found. Use POST /compile to generate compiled skills.`
    );
  });
