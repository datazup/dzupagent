...
describe("API key routes", () => {
  ...
  it("POST /api/keys returns 201 with the raw key and metadata", async () => {
    ...
    expect(store.create).toHaveBeenCalledWith(
      ...
      "my-key",
      "premium",
      { expiresIn: undefined, role: "viewer", tenantId: "default" }
    );
  });
  ...
  it.each([
    ...
    "POST /api/keys rejects names that are %s",
    async (_caseName, payload, message) => {
      const res = await app.request("/api/keys", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  it("DELETE /api/keys/:id returns 404 when the key belongs to another owner", async () => {
    (store.get as ReturnType<typeof vi.fn>).mockResolvedValue(
      buildRecord({ id: "k1", ownerId: "owner-2" })
    );
  ...
  it("uses the authenticated identity id as ownerId when present", async () => {
    ...
    expect(res.status).toBe(201);
    expect(store.create).toHaveBeenCalledWith("user-42", "scoped", "standard", {
      expiresIn: undefined,
      role: "viewer",
      tenantId: "default",
    });
...
describe("API key routes — allowedTiers validation", () => {
  it("accepts a tier that is in the allowedTiers list", async () => {
    ...
    app.route(
      "/api/keys",
      createApiKeyRoutes({ store, allowedTiers: ["standard", "premium"] })
    );
  ...
  it("rejects a tier not in allowedTiers with 400", async () => {
    ...
    app.route(
      "/api/keys",
      createApiKeyRoutes({ store, allowedTiers: ["standard"] })
    );
  ...
  it('defaults to "standard" tier and validates it against allowedTiers', async () => {
    ...
    expect(res.status).toBe(201);
    expect(store.create).toHaveBeenCalledWith("anonymous", "k", "standard", {
      expiresIn: undefined,
      role: "viewer",
      tenantId: "default",
    });
...
describe("API key routes — expiresIn and rotate", () => {
  it("POST /api/keys with expiresIn passes it through to store.create", async () => {
    ...
    expect(store.create).toHaveBeenCalledWith(
      ...
      "expiring",
      "standard",
      { expiresIn: 3600, role: "viewer", tenantId: "default" }
    );
  ...
  it.each([
    ...
    "POST /api/keys rejects %s expiresIn values",
    async (_caseName, expiresIn) => {
      const store = buildStore();
      const app = buildApp(store);
      const res = await app.request("/api/keys", {
        method: "POST",
      ...
      expect(body.error.message).toBe(
        "expiresIn must be a positive integer no greater than 31536000 seconds"
      );
      expect(store.create).not.toHaveBeenCalled();
    }
  );
  ...
  it("POST /api/keys/:id/rotate returns 404 for unknown key", async () => {
    ...
    const app = buildApp(store);
    const res = await app.request("/api/keys/bad-id/rotate", {
      method: "POST",
    });
  ...
  it("POST /api/keys/:id/rotate returns 400 for an already-revoked key", async () => {
    const store = buildStore();
    (store.get as Mock).mockResolvedValue(
  ...
  it("POST /api/keys/:id/rotate atomically revokes old key and creates a new one", async () => {
    ...
    (store.create as Mock).mockResolvedValue({
      key: newRawKey,
      record: newRecord,
    });

    const app = buildScopedApp(store, "owner-1");
    const res = await app.request("/api/keys/old-id/rotate", {
      method: "POST",
    });
...
describe("PostgresApiKeyStore — event bus audit trail", () => {
  ...
  it("emits api-key:created on create()", async () => {
    ...
    const store = new PostgresApiKeyStore(
      ...
      bus as unknown as DzupEventBus
    );
...
// --- SEC-H-01: tenant isolation + valid-role for route-issued keys ---
describe("API key routes — SEC-H-01 tenant isolation + valid role", () => {
  /**
   * Build an app whose middleware simulates the server-trusted request state a
   * real deployment would populate: an owner identity, the tenant-scope
   * middleware's `forgeTenantId`, and the RBAC middleware's `forgeRole`.
   */
  function buildTenantScopedApp(
    store: PostgresApiKeyStore,
    ctx: { ownerId: string; tenantId?: string; role?: string }
  ): Hono<AppEnv> {
    const app = new Hono<AppEnv>();
    app.use("*", async (c, next) => {
      c.set("identity", { id: ctx.ownerId });
      if (ctx.tenantId !== undefined) {
        c.set("forgeTenantId", ctx.tenantId);
      }
      if (ctx.role !== undefined) {
        c.set("forgeRole", ctx.role as AppEnv["Variables"]["forgeRole"]);
      }
      return next();
    });
    app.route("/api/keys", createApiKeyRoutes({ store }));
  ...
  it("scopes a route-issued key to the CALLER's tenant, not the shared 'default'", async () => {
    const store = buildStore();
    const record = buildRecord({ tenantId: "tenant-a", role: "viewer" });
    (store.create as Mock).mockResolvedValue({ key: "k".repeat(64), record });

    const app = buildTenantScopedApp(store, {
      ownerId: "owner-1",
      tenantId: "tenant-a",
    });
    ...
    expect(res.status).toBe(201);
    // The tenant handed to the store is the caller's — never 'default'.
    const call = (store.create as Mock).mock.calls[0]!;
    const options = call[3] as { tenantId: string; role: string };
    expect(options.tenantId).toBe("tenant-a");
    expect(options.tenantId).not.toBe("default");
    // Default role is a valid ForgeRole.
    expect(options.role).toBe("viewer");
    // Response echoes the server-derived scope.
    const body = (await res.json()) as { tenantId: string; role: string };
    expect(body.tenantId).toBe("tenant-a");
    expect(body.role).toBe("viewer");
  });

  it("ignores a client-supplied tenantId for a non-admin caller (403)", async () => {
    const store = buildStore();
    const app = buildTenantScopedApp(store, {
      ownerId: "owner-1",
      tenantId: "tenant-a",
    ...
    const res = await app.request("/api/keys", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "sneaky", tenantId: "tenant-victim" }),
    });
  ...
  it("lets an admin caller set an explicit tenantId", async () => {
    const store = buildStore();
    const record = buildRecord({ tenantId: "tenant-b", role: "viewer" });
    (store.create as Mock).mockResolvedValue({ key: "k".repeat(64), record });
    ...
    const res = await app.request("/api/keys", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "cross", tenantId: "tenant-b" }),
    });

    expect(res.status).toBe(201);
    const options = (store.create as Mock).mock.calls[0]![3] as {
      tenantId: string;
    };
    expect(options.tenantId).toBe("tenant-b");
  });

  it("rejects an invalid role (not a ForgeRole) with 400", async () => {
    const store = buildStore();
    const app = buildTenantScopedApp(store, {
      ownerId: "owner-1",
      tenantId: "tenant-a",
    ...
    const res = await app.request("/api/keys", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "bad-role", role: "user" }),
    });
  ...
  it("forbids a non-admin caller from issuing a non-default role (403)", async () => {
    const store = buildStore();
    const app = buildTenantScopedApp(store, {
      ownerId: "owner-1",
      tenantId: "tenant-a",
    ...
    const res = await app.request("/api/keys", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "escalate", role: "admin" }),
    });
  ...
  it("lets an admin caller issue a valid non-default role", async () => {
    const store = buildStore();
    const record = buildRecord({ role: "operator", tenantId: "tenant-a" });
    (store.create as Mock).mockResolvedValue({ key: "k".repeat(64), record });
    ...
    const res = await app.request("/api/keys", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "op-key", role: "operator" }),
    });
  ...
  it("rotate preserves the ORIGINAL key's tenant and role (not 'default'/re-scoped)", async () => {
    const store = buildStore();
    const existing = buildRecord({
      id: "old",
      tenantId: "tenant-a",
    ...
    const res = await app.request("/api/keys/old/rotate", { method: "POST" });
