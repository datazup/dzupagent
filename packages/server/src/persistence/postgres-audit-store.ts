...
function entryContent(
  entry: Omit<ComplianceAuditEntry, "hash" | "previousHash" | "seq">
): string {
...
export class PostgresAuditStore implements ComplianceAuditStore {
  ...
  private buildConditions(filter: AuditFilter): SQL[] {
    ...
    if (filter.actorType !== undefined)
      conditions.push(eq(auditLog.actorType, filter.actorType));
    if (filter.action !== undefined)
      conditions.push(eq(auditLog.action, filter.action));
    if (filter.result !== undefined)
      conditions.push(eq(auditLog.result, filter.result));
    if (filter.fromDate !== undefined)
      conditions.push(gte(auditLog.ts, filter.fromDate));
    if (filter.toDate !== undefined)
      conditions.push(lte(auditLog.ts, filter.toDate));
    return conditions;
  }
  ...
  private async doFlush(): Promise<void> {
    ...
    try {
      // MC-1 (AGENT-H-02): the last-row read and the batch insert must be a
      // single atomic unit. Two concurrent PostgresAuditStore instances would
      // otherwise both read the same baseSeq/previousHash and produce duplicate
      // `seq` values, forking the hash chain. We wrap both in one transaction
      // and take a `FOR UPDATE` lock on the tail row so the second flusher
      // blocks until the first commits, then reads the freshly-inserted tail.
      const completed = await this.db.transaction(async (tx) => {
        ...
        const rows: ComplianceAuditEntry[] = [];
      ...
      this.flushing = false;
      // MC-1: level-triggered drain. If more entries arrived while we were
      // flushing (e.g. >batchSize appended at once, or new appends mid-flush),
      // the excess would otherwise strand in the queue until the next append
      // happens to re-trigger scheduleFlush. Re-schedule to drain the backlog.
