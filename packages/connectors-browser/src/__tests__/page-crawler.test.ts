...
describe("PageCrawler", () => {
  ...
  it("respects maxDepth limit", async () => {
    ...
    mockedExtractLinks.mockResolvedValue(["https://example.com/deep"]);
  ...
  it("continues crawling when a page fails to load", async () => {
    ...
    (failPage.goto as ReturnType<typeof vi.fn>).mockRejectedValue(
  ...
  it("crawls same-origin links by default and skips cross-origin links", async () => {
    ...
    mockedExtractLinks.mockImplementation(async () => {
      callIdx++;
      if (callIdx === 1) {
        return ["https://example.com/docs", "https://other.example/docs"];
      }
      return [];
    });
  ...
  it("crawls cross-origin links when the origin is explicitly allowlisted", async () => {
    ...
    const crawler = new PageCrawler(context, {
      ...
      // Deterministic DNS so the SSRF resolved-IP check resolves the
      // allowlisted cross-origin host to a public address instead of hitting
      // the real resolver (which would hang on this non-routable test host).
      navigationPolicy: {
