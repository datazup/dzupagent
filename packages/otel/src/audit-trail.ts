 ...
 *
 * Subscribes to DzupEventBus and maps events to audit categories:
...
interface EventMapping {
  category: AuditCategory;
  action: string;
  extractDetails: (event: DzupEvent) => Record<string, unknown>;
  extractIds: (event: DzupEvent) => { agentId?: string; runId?: string };
}

function mapEvent(event: DzupEvent): EventMapping | undefined {
  switch (event.type) {
    case "agent:started":
      return {
...
export class AuditTrail {
  ...
  /**
   * Attach to a DzupEventBus. Maps events to audit entries.
   */
    ...
    this._unsubscribes.push(
      eventBus.onAny((event) => {
        try {
          ...
          if (!mapping) return;
          if (this._categories && !this._categories.has(mapping.category))
            return;
  ...
  private async _appendEntry(
    ...
    if (this._retentionDays > 0 && seq % 100 === 99) {
      const cutoff = new Date(
        Date.now() - this._retentionDays * 24 * 60 * 60 * 1000,
      );
      await this._store.prune(cutoff).catch((err: unknown) => {
        // Non-fatal — a prune failure must not break entry appends, but a
        // silently failing retention prune lets the audit store grow unbounded.
        defaultLogger.warn("[otel] audit-trail retention prune failed", {
