  ...
  attach(eventBus: DzupEventBus): void {
    ...
    this._unsubscribe = eventBus.onAny((event) => {
      try {
        ...
        // Bridge errors must never propagate to the event bus (non-fatal), but
        // a silently failing metric/span export hides telemetry drift.
        defaultLogger.warn("[otel] event bridge export failed", {
          operation: "otelBridge.handleEvent",
