...
describe("ForgeSpanAttr", () => {
  ...
  it("defines GenAI semantic convention keys", () => {
    ...
    expect(ForgeSpanAttr.GEN_AI_USAGE_INPUT_TOKENS).toBe(
      "gen_ai.usage.input_tokens"
    );
    expect(ForgeSpanAttr.GEN_AI_USAGE_OUTPUT_TOKENS).toBe(
      "gen_ai.usage.output_tokens"
    );
    expect(ForgeSpanAttr.GEN_AI_USAGE_TOTAL_TOKENS).toBe(
  ...
  it("defines budget attribute keys", () => {
    ...
    expect(ForgeSpanAttr.BUDGET_COST_LIMIT).toBe(
      "forge.budget.cost_limit_cents"
    );
    expect(ForgeSpanAttr.BUDGET_ITERATIONS).toBe("forge.budget.iterations");
    expect(ForgeSpanAttr.BUDGET_ITERATIONS_LIMIT).toBe(
      "forge.budget.iterations_limit"
    );
  });
  ...
  it("all forge attributes use forge. prefix", () => {
    // Standard OTel/GenAI semconv keys and the codev.* execution-profile
    // namespace legitimately do NOT use the forge. prefix — they follow
    // upstream OpenTelemetry conventions. Everything else must be forge-prefixed.
    const NON_FORGE_PREFIXES = ["gen_ai.", "error.", "codev."];
    const forgeKeys = Object.entries(ForgeSpanAttr).filter(
      ([, value]) => !NON_FORGE_PREFIXES.some((p) => value.startsWith(p))
    );
  ...
  it("all gen_ai attributes use gen_ai. prefix", () => {
    const genAiKeys = Object.entries(ForgeSpanAttr).filter(([key]) =>
