...
describe("McpConnectionPool", () => {
  ...
  describe("backoff on failure", () => {
    it("returns false after max retries exhausted", async () => {
      ...
      // But still fails to connect. The pool gates retries on Date.now() elapsed
      // vs. a jittered backoff (≤2ms here); advancing fake time clears it
    ...
    it("resets retries on successful connection", async () => {
      ...
      // Advance fake time past the (≤2ms) backoff window deterministically.
      await vi.advanceTimersByTimeAsync(10);
      await p.acquire(config);
  ...
  describe("tool cache", () => {
    ...
    it("expires cache after TTL", () => {
      ...
      // TTL is a Date.now()-based comparison (500ms in test config). Advancing
      // fake time past it expires the cache deterministically with no real wait.
