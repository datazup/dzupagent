...
describe("Semaphore", () => {
  ...
  it("limits concurrency correctly", async () => {
    ...
    const task = async () => {
      ...
      // eslint-disable-next-line no-restricted-syntax -- simulated timed work, driven by fake timers (advanceTimersByTimeAsync); not a real wall-clock sleep
    ...
    // 5 tasks gated by 2 permits: each 20ms hold; advance generously to drain
    // all batches. advanceTimersByTimeAsync flushes microtasks between ticks,
    // so released permits are picked up by waiting acquirers.
...
describe("ConcurrencyPool", () => {
  ...
  it("limits global concurrency", async () => {
    ...
    const all = Promise.all([task("a"), task("b"), task("c"), task("d")]);
    await vi.advanceTimersByTimeAsync(100);
    await all;
    expect(maxConcurrent).toBe(2);
