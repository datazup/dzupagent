...
describe("runBeforeModelCall", () => {
  it("replaces messages when a hook returns a new array", async () => {
    ...
    const hook = vi.fn(async () => replacement);

    const result = await runBeforeModelCall(
      [hook],
      undefined,
      original,
      "gpt-4",
    ...
    expect(hook).toHaveBeenCalledWith(original, "gpt-4", ctx);
  });

  it("passes messages through unchanged when a hook returns void", async () => {
    const original: BaseMessage[] = [new HumanMessage("hi")];
    const hook = vi.fn(async () => undefined);
  ...
  it("swallows a throwing hook and passes messages through unchanged", async () => {
    const bus = createEventBus();
    const events: DzupEvent[] = [];
    bus.onAny((event) => {
      events.push(event);
  ...
  it("runs hooks sequentially, second sees first hook output array", async () => {
    const original: BaseMessage[] = [new HumanMessage("one")];
    const first = vi.fn(async (msgs: BaseMessage[]) => [
      ...msgs,
      new AIMessage("two"),
    ]);
    const seen: BaseMessage[][] = [];
    const second = vi.fn(async (msgs: BaseMessage[]) => {
      seen.push(msgs);
      return [...msgs, new AIMessage("three")];
    });
    ...
    // second saw the first hook's output (2 messages)
    expect(seen[0]).toHaveLength(2);
    expect(result).toHaveLength(3);
  });

  it("returns original when hook list is empty", async () => {
    const original: BaseMessage[] = [new HumanMessage("hi")];
    const result = await runBeforeModelCall(
      [],
      undefined,
...
describe("runAfterModelCall", () => {
  it("runs all after-hooks with response and modelId", async () => {
    const messages: BaseMessage[] = [new HumanMessage("hi")];
    const response = new AIMessage("yo");
    const hook = vi.fn(async () => {});
    ...
    expect(hook).toHaveBeenCalledWith(messages, response, "gpt-4", ctx);
  });

  it("swallows a throwing after-hook and continues", async () => {
    const bus = createEventBus();
    const events: DzupEvent[] = [];
    bus.onAny((event) => {
      events.push(event);
...
describe("runOnModelError", () => {
  it("runs all error-hooks with error and modelId", async () => {
    const err = new Error("model failed");
    const hook = vi.fn(async () => {});

    await runOnModelError([hook], undefined, err, "gpt-4", ctx);
    expect(hook).toHaveBeenCalledWith(err, "gpt-4", ctx);
  });

  it("swallows a throwing error-hook and continues", async () => {
    const bus = createEventBus();
    const events: DzupEvent[] = [];
    bus.onAny((event) => {
      events.push(event);
