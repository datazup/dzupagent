...
describe("ModelRegistry", () => {
  ...
  it("sorts providers by priority (lower = higher priority)", () => {
    registry.addProvider(
      makeProvider({ provider: "openai", priority: 5, apiKey: "k2" }),
    );
    registry.addProvider(
      makeProvider({ provider: "anthropic", priority: 1, apiKey: "k1" }),
    );
  ...
  it("throws for an unconfigured tier", () => {
    registry.addProvider(
      makeProvider({ models: { chat: { name: "x", maxTokens: 100 } } }),
    );
    expect(() => registry.getModel("reasoning")).toThrow(
      /No provider configured for tier "reasoning"/,
    );
  });
  ...
  it("decorates resolved models with provider-derived structured-output capabilities", () => {
    ...
    realRegistry.addProvider(
      makeProvider({ provider: "openai", apiKey: "oai" }),
    );

    const model = realRegistry.getModel("chat") as unknown as Record<
      string,
      unknown
    >;
  ...
  it("gets a model from a specific provider", () => {
    ...
    const model = registry.getModelFromProvider(
      "openai",
      "chat",
    ) as unknown as Record<string, unknown>;
  ...
  it("throws when provider lacks the requested tier", () => {
    registry.addProvider(
      makeProvider({ models: { chat: { name: "x", maxTokens: 1 } } }),
    );
  ...
  it("resolves a model by exact name", () => {
    registry.addProvider(makeProvider());
    const model = registry.getModelByName("claude-haiku") as unknown as Record<
      string,
      unknown
    >;
    expect(model["_model"]).toBe("claude-haiku");
  ...
  it("resolves a model by partial name match", () => {
    registry.addProvider(makeProvider());
    const model = registry.getModelByName("sonnet") as unknown as Record<
      string,
      unknown
    >;
  ...
  it("exposes selectable provider candidates for explicit run-level failover", () => {
    ...
    expect(
      candidates.map((candidate) => ({
        ...
        modelProvider: (candidate.model as unknown as Record<string, unknown>)[
          "_provider"
        ],
      })),
    ).toEqual([
  ...
  it("reports provider health", () => {
    ...
    // getProviderHealth() was extended additively with weight/successRate/samples
    // (provider health EMA). The original state/provider fields are unchanged.
    expect(health["anthropic"]).toMatchObject({
      state: "closed",
      provider: "anthropic",
    });
    expect(health["anthropic"]).toEqual({
      state: "closed",
      provider: "anthropic",
      weight: 1,
      successRate: 1,
