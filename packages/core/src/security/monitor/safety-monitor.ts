...
function normalizeInjectionPolicy(
  value: "off" | "warn" | "block" | undefined
): PolicyLevel {
  return value ?? "block";
}

function normalizePiiPolicy(
  value: "off" | "redact" | "block" | undefined
): PolicyLevel {
...
function buildBuiltInRules(config?: SafetyMonitorConfig): SafetyRule[] {
  ...
  const piiRule = applyPolicyToRule(
    createPIILeakRule(config?.piiScanner),
    piiLevel
  );
  ...
  const escalationRule = applyPolicyToRule(
    createEscalationRule(),
    escalationLevel
  );
...
export function createSafetyMonitor(
  ...
  function recordViolation(
    ...
    try {
      ...
      // Non-fatal: event emission failure should not crash the monitor, but a
      // dropped safety:violation / safety:kill_requested must be observable
      // (ERR-H-02) — a silently swallowed kill-switch alert is a security gap.
      defaultLogger.warn("[safety-monitor] violation emit failed", {
        category: violation.category,
        action: violation.action,
        error: String(err),
      });
  ...
  function scanContent(
    ...
    for (const rule of rules) {
      try {
        ...
        // Non-fatal: one throwing rule must not block the others — but a
        // silently skipped detection category is a no-op scan for that
        // content and must be observable (ERR-H-02). A rule that throws on
        // adversarial input (e.g. ReDoS) would otherwise let content pass
        // unscanned with zero signal.
        defaultLogger.error("[safety-monitor] rule check threw", {
          rule: rule.category,
          error: String(err),
        });
      }
