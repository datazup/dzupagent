...
export interface OutboundUrlSecurityPolicy {
  ...
  lookup?:
    | ((hostname: string) => Promise<readonly OutboundUrlResolvedAddress[]>)
    | undefined;
}
...
export function validateOutboundUrlSyntax(
  ...
  if (
    ...
    return {
      ok: false,
      reason:
        "URL protocol must be https unless trusted HTTP is explicitly allowed.",
    };
  }
  ...
  const blockedHosts = new Set([
    ...
    ...toSet(policy?.blockedHosts),
  ]);
  if (
    !isAllowlistedHost &&
    (blockedHosts.has(hostname) || hostname.endsWith(".localhost"))
  ) {
    return {
      ok: false,
      reason: `URL host "${parsedUrl.hostname}" is blocked.`,
    };
  }
  ...
  if (
    ipVersion !== 0 &&
    !isAllowlistedHost &&
    !isIpAllowlisted(hostname, policy) &&
    !isPublicIpAddress(hostname)
  ) {
    return {
      ok: false,
      reason: `URL host "${parsedUrl.hostname}" is not a public IP address.`,
    };
  }
  ...
  return {
    ...
    url: parsedUrl,
    resolvedAddresses:
      ipVersion === 0 ? [] : [{ address: hostname, family: ipVersion }],
  };
}
...
export async function validateOutboundUrl(
  ...
  try {
    ...
    return {
      ok: false,
      reason: `URL host "${syntax.url.hostname}" could not be resolved: ${message}`,
    };
  }
  ...
  if (resolved.length === 0) {
    return {
      ok: false,
      reason: `URL host "${syntax.url.hostname}" did not resolve to any addresses.`,
    };
  }
...
function createIpPinnedDispatcher(
  ...
  const pinnedFamily: 0 | 4 | 6 =
    address.family === 4 || address.family === 6 ? address.family : 0;
...
function getResolvedAddressForPinning(
  ...
  for (const entry of validation.resolvedAddresses) {
    ...
    if (isIP(normalized) !== 0)
      return { address: normalized, family: entry.family };
  }
  return undefined;
}
...
export async function fetchWithOutboundUrlPolicy(
  ...
  for (let redirectCount = 0; redirectCount <= maxRedirects; redirectCount++) {
    ...
    if (pinnedAddress) {
      ...
      // "invalid onRequestStart method"). We must route pinned requests through
