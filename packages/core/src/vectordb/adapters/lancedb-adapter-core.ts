...
export class LanceDBAdapter implements VectorStore {
  ...
  static async create(config?: LanceDBAdapterConfig): Promise<LanceDBAdapter> {
    ...
    try {
      ...
      // laziness is preserved by the dynamic import, and the module shape
      // comes from the local ambient declaration (optional-peer-modules.d.ts).
      lancedb = (await import(
        /* webpackIgnore: true */ "@lancedb/lancedb"
      )) as typeof lancedb;
    } catch {
      throw new ForgeError({
        ...
        recoverable: false,
        suggestion:
          "Install @lancedb/lancedb as a dependency or use a different vector store.",
      });
    }
