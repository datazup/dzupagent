...
describe("Embedding Providers", () => {
  ...
  describe("createOpenAIEmbedding", () => {
    ...
    it("retries on HTTP 429 and succeeds once the rate limit clears", async () => {
      const embeddings = makeMockEmbeddings(1, 4);
      const fetchImpl = vi
        .fn()
        ...
        .mockResolvedValueOnce({
          ...
          headers: new Headers(),
          json: () =>
            Promise.resolve({ data: [{ embedding: embeddings[0], index: 0 }] }),
        });
    ...
    it("does not retry a non-recoverable error (HTTP 401)", async () => {
