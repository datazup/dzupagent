...
function sanitizeProviderError(message: string): string {
  ...
  // Both patterns use mandatory `.` separators between bounded label groups, so
  // the nested quantifiers cannot overlap — linear-time, no ReDoS. The
  // detect-unsafe-regex heuristic over-flags them.
...
function toInvokeOptions(
  options?: ResilientInvokerOptions
  ...
  async invoke(messages: BaseMessage[]): Promise<BaseMessage> {
    if (this.candidates.length === 0) {
      throw new ForgeError({
        ...
        recoverable: false,
        suggestion:
          "Ensure ModelRegistry has at least one provider configured for the requested tier",
      });
    }
