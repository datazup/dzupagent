    ...
    if (!descriptor) {
      return {
        content: [
          {
            type: "text",
            text: `Tool "${toolName}" not found on any connected MCP server`,
          },
        ],
        isError: true,
      };
    ...
    switch (config.transport) {
      case "http": {
        ...
        try {
          ...
          return (
            data.result ?? { content: [{ type: "text", text: "No result" }] }
          );
        } finally {
          clearTimeout(timer);
        }
    ...
    return new Promise((resolve, reject) => {
      import("node:child_process")
        .then(({ spawn }) => {
          ...
          // Defence in depth: even an allowlisted executable can be driven into
          // host RCE via inline-eval args (`node -e …`, `bash -c …`). Default to
          // the strict policy when the config does not specify one.
          assertMcpCommandAllowed(
            config.url,
            config.args,
