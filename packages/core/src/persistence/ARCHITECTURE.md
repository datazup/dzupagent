...
- `@langchain/langgraph-checkpoint-postgres` is a runtime `dependency` (`^1.0.1`) imported by `checkpointer.ts`.
...
- `checkpointer.ts` is covered by `__tests__/checkpointer.test.ts` (memory path, missing-connection-string error, postgres setup).
