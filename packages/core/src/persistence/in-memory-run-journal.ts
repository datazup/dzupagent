  ...
  implements RunJournal<TState>
{
  ...
  async append(
    ...
    // Reserved for a future post-append hook mechanism; see RunJournalAppendOptions doc.
    // Currently a declared-but-no-op parameter (no hook mechanism exists yet).
    _options?: RunJournalAppendOptions
  ): Promise<number> {
    ...
    if (entry.type === "state_updated" && this.stateSchema) {
      try {
        this.stateSchema.parse((entry as StateUpdatedEntry<TState>).data.state);
      } catch (err) {
  ...
  async compact(runId: string): Promise<void> {
    ...
    this.entries.set(runId, [
      snapshot as RunJournalEntry<TState>,
      ...remaining,
    ]);
