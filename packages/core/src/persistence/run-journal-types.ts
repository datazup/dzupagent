...
export interface StateUpdatedEntry<TState = Record<string, unknown>>
  extends RunJournalEntryBase {
  type: "state_updated";
  data: {
...
export interface SnapshotEntry<TState = Record<string, unknown>>
  extends RunJournalEntryBase {
  type: "snapshot";
  data: {
...
/**
 * Options controlling how `RunJournal.append()` behaves for a single write.
 */
export interface RunJournalAppendOptions {
  /**
   * Controls whether registered post-append hooks are awaited before
   * append() resolves. Default 'async' matches pre-existing behavior
   * exactly. This does NOT make an in-memory journal crash-safe —
   * see the module doc in run-journal.ts for the relationship to
   * FlowDocumentV1.durability.mode / DurableNodeLedger.
   *
   * NOTE: as of this writing, no `RunJournal` implementation has a
   * post-append hook mechanism (no `onAppend()` registration exists on
   * `RunJournal` or `InMemoryRunJournal`). This option is therefore a
   * declared-but-currently-no-op field, accepted and stored for forward
   * compatibility with a future hook mechanism. It does not change
   * append() behavior today.
   */
  flushPolicy?: "sync" | "async";
}
...
export interface RunJournal<TState = Record<string, unknown>> {
   ...
   *
   * @param options - Optional per-write options. See {@link RunJournalAppendOptions}.
   *   Currently a no-op (see that type's doc) — accepted for forward
   *   compatibility only.
   */
  append(
    runId: string,
    entry: Omit<RunJournalEntry<TState>, "v" | "seq" | "ts" | "runId">,
    options?: RunJournalAppendOptions
  ): Promise<number>;
