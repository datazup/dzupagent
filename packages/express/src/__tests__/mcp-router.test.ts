...
describe("createMcpRouter", () => {
  it("serves MCP helper routes and request hooks through a mounted Express app", async () => {
    ...
    const toolsReq = createRequest("GET", "/mcp/tools", undefined, {
      "x-api-key": "secret",
    });
    ...
    const resourcesReq = createRequest("GET", "/mcp/resources", undefined, {
      "x-api-key": "secret",
    });
    ...
    const templatesReq = createRequest(
      "GET",
    ...
    expect(JSON.parse(templatesState.chunks.join(""))).toEqual({
      resourceTemplates: [
        { uriTemplate: "project://{projectId}/report", name: "Report" },
      ],
    });
  ...
  describe("auth scoping to basePath", () => {
    ...
    function buildServer(): DzupAgentMCPServer {
    ...
    it("rejects POST {basePath} without credentials (regression)", async () => {
      const app = express();
      app.use(express.json());
      app.use(createMcpRouter({ server: buildServer(), auth }));
    ...
    it("does not 401 sibling routes when mounted at a shared prefix", async () => {
      const app = express();
      app.use(express.json());
      app.use("/api/v1", createMcpRouter({ server: buildServer(), auth }));
      app.get("/api/v1/other", (_req: Request, res: Response) => {
        res.status(200).json({ ok: true });
      });

      const req = createRequest("GET", "/api/v1/other", undefined, {});
      const res = createResponse(app);
      const state = await dispatch(app, req, res);

      expect(state.statusCode).toBe(200);
      expect(JSON.parse(state.chunks.join(""))).toEqual({ ok: true });
    });

    it("still requires auth for GET {basePath}/tools under a shared-prefix mount", async () => {
      const app = express();
      app.use(express.json());
      app.use("/api/v1", createMcpRouter({ server: buildServer(), auth }));
      ...
      const authorizedReq = createRequest(
        "GET",
        "/api/v1/mcp/tools",
        undefined,
        { "x-api-key": "secret" },
      ...
      expect(authorizedState.statusCode).toBe(200);
      expect(JSON.parse(authorizedState.chunks.join(""))).toEqual({
        tools: [expect.objectContaining({ name: "echo" })],
      });
    });
  });
  ...
  it("supports request-scoped server resolution for tenant-aware publishers", async () => {
    ...
    app.use((req: Request, _res: Response, next: NextFunction) => {
      (req as Request & { tenantName?: string }).tenantName =
        req.headers["x-tenant"] === "beta" ? "beta" : "alpha";
      next();
    });
    app.use(
      createMcpRouter({
        server: async (req) => {
          const tenantName =
            (req as Request & { tenantName?: string }).tenantName ?? "alpha";
          return new DzupAgentMCPServer({
            name: `${tenantName}-mcp`,
            version: "1.0.0",
            tools: [
    ...
    expect(JSON.parse(alphaState.chunks.join(""))).toEqual({
      tools: [
        expect.objectContaining({ name: "echo-alpha", serverId: "alpha-mcp" }),
      ],
    });

    const betaReq = createRequest("GET", "/mcp/tools", undefined, {
      "x-tenant": "beta",
    });
    ...
    expect(JSON.parse(betaState.chunks.join(""))).toEqual({
      tools: [
        expect.objectContaining({ name: "echo-beta", serverId: "beta-mcp" }),
      ],
    });
  });
});
