...
export interface MemoryFileEntry {
  ...
  tags: string[];
  content: string;
  /** Estimated tokens via core `estimateTokens` (tokenizer-aware, chars/4 fallback). */
...
export interface DzupAgentMemoryLoaderOptions {
  ...
  onRecalled?: (
    entries: AgentMemoryRecalledEvent["entries"],
    totalTokens: number
  ) => void;
}
...
export class DzupAgentMemoryLoader implements MemoryServiceLike {
  ...
  constructor(options: DzupAgentMemoryLoaderOptions) {
    ...
    this.codexMemoryStrategy =
      options.codexMemoryStrategy ?? "inject-on-new-thread";
  ...
  async search(
    ...
    const slice =
      limit !== undefined && limit > 0 ? entries.slice(0, limit) : entries;
  ...
  async loadEntries(): Promise<MemoryFileEntry[]> {
    const globalEntries = await this.loadFromDir(
    ...
    const projectEntries = await this.loadFromDir(
