...
export class GeminiSDKAdapter
  implements AgentCLIAdapter, AdapterStreamSource<GeminiRawEvent>
{
  ...
  getCapabilities(): AdapterCapabilityProfile {
    return {
      ...
      // The network SDK surfaces function calls but this adapter does not
      // execute them or re-invoke Gemini with their results.
      emitsToolCalls: true,
  ...
  async *open(
    ...
    const model = sdk.getGenerativeModel({
      ...
      ...(input.systemPrompt !== undefined
        ? {
            systemInstruction: String(
              new SystemPromptBuilder(input.systemPrompt).buildFor("gemini-sdk")
            ),
          }
        : {}),
  ...
  private async loadSDK(): Promise<GeminiSDK> {
    ...
    // Resolve the API key before the SDK import — a missing key must surface as
    // ADAPTER_EXECUTION_FAILED, not be swallowed by the import catch block.
    const apiKey =
