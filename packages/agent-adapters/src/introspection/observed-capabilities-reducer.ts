...
export function reduceRunEventsToObservedCapabilities(
  ...
  const streaming = evidenceFor(
    runs.flatMap((run) =>
      run.accepted.filter((entry) => entry.event.type === 'adapter:stream_delta'),
    ),
  )
  ...
  const toolResults = evidenceFor(
    runs.flatMap((run) =>
      run.accepted.filter((entry) => entry.event.type === 'adapter:tool_result'),
    ),
  )
  ...
  const evidence: Record<EvidenceKey, ObservedCapabilityEvidence | null> = {
    ...
    toolLoopExecuted:
      toolResults ?? evidenceFor(toolCallsWithoutResult),
...
export class ObservedCapabilitiesLiveSubscriber {
  ...
  start(): void {
    ...
    this.unsubscribe = this.source.subscribe((event) => this.events.push(event))
  }
...
function groupRuns(events: CanonicalRunEvent[]): RunFacts[] {
  ...
  return [...byRun.entries()]
    ...
    .map(([runId, runEvents]) => {
      const terminalIndex = runEvents.findIndex((entry) => isTerminal(entry.event))
      const accepted = terminalIndex === -1
        ? runEvents
        : runEvents.slice(0, terminalIndex + 1)
      return {
        runId,
        started:
          accepted.find((entry) => entry.event.type === 'adapter:started') ?? null,
...
function compareEvents(left: CanonicalRunEvent, right: CanonicalRunEvent): number {
  return (
