...
/**
 * R3-ISO: resolve the task's owning tenant for routing-decision audit
 * attribution. Mirrors packages/server's run-pipeline resolveTenantId
 * convention (tenantId → metadata.tenantId → 'default'). Providers
 * themselves are shared infrastructure — this does not gate selection.
 */
export function resolveTaskTenantId(task: {
  tenantId?: string | undefined;
  metadata?: Record<string, unknown> | undefined;
}): string {
  if (typeof task.tenantId === "string" && task.tenantId.length > 0)
    return task.tenantId;
  const fromMeta = task.metadata?.["tenantId"];
  ...
  route(
    ...
    if (
      ...
      return {
        ...
        fallbackProviders: buildFallbacks(
          task.preferredProvider,
          availableProviders
        ),
        confidence: 0.95,
...
export class CompositeRouter implements TaskRoutingStrategy {
  ...
  route(
    ...
    for (const { strategy, weight } of this.strategies) {
      ...
      reasons.push(
        `${strategy.name}: ${decision.provider} (${(
    ...
    const normalizedConfidence =
      totalWeight > 0 ? Math.min(bestScore / totalWeight, 1) : 0;
