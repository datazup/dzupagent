...
function handleToolCall(
  ...
  if (state.stuckDetector) {
    const stuckStatus = state.stuckDetector.recordToolCall(event.toolName, event.input)
...
async function handleCompleted(
  ...
  if (state.stuckDetector) {
    const stuckStatus = state.stuckDetector.recordIteration(state.toolCallsInCurrentIteration)
  ...
  if (state.outputFilter && event.result) {
    ...
    if (filtered === null) {
      ...
      state.violations.push({ type: 'output_filtered', message, severity: 'critical' })
      return { abort: true, abortReason: message }
    }
