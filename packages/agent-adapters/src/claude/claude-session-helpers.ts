...
export function interruptClaudeConversation(
  ...
  try {
    if (conversation) {
      // SDK 0.3.x interrupt() may trigger an async transport write that rejects
      // when the process is already terminated. Absorb both sync throws and
      // any resulting async rejection to prevent unhandledRejection noise.
      const result =
        conversation.interrupt() as unknown as Promise<unknown> | void;
      if (
        result != null &&
...
export async function* openClaudeConversation({
  ...
  try {
    ...
    throw ForgeError.wrap(err, {
      code: "ADAPTER_EXECUTION_FAILED",
      suggestion:
        "Verify Claude Agent SDK is correctly installed and configured",
      context: {
        providerId: "claude",
