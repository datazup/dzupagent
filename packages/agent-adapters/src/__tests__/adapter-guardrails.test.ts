...
describe("AdapterGuardrails", () => {
  ...
  it("blocks forbidden tools and yields failed event", async () => {
    ...
    expect(
      (output[1]! as Extract<AgentEvent, { type: "adapter:failed" }>).error
    ).toContain("dangerous_tool");
  ...
  describe("output filter", () => {
    it("applies output filter on completion and modifies result", async () => {
      ...
      const completed = output[0] as Extract<
        AgentEvent,
        { type: "adapter:completed" }
      >;
  ...
  describe("destructive command deny hook", () => {
    it("aborts the stream when a destructive shell command is detected", async () => {
      const abortFn = vi.fn();
      const guardrails = new AdapterGuardrails({ maxIterations: 100 });

      const events = [
        makeStartedEvent(),
        makeToolCallEvent("bash", { command: "rm -rf /" }),
        makeCompletedEvent(),
      ];
      ...
      // Stream must abort: started passes, then failed (blocked), completed must NOT appear
      ...
      expect(failedEvent.error).toContain("Destructive");
    });
    ...
    it("passes through when the shell command is safe", async () => {
      ...
      // All 3 events should pass through
      expect(output).toHaveLength(3);
      expect(output[2]!.type).toBe("adapter:completed");
