...
describe("ConversationCompressor", () => {
  ...
  describe("recordEvent — turn detection", () => {
    ...
    it("stores tool summary from tool_call and tool_result events", () => {
      const c = new ConversationCompressor();
      c.recordEvents(
        makeTurn("List files", "Done.", ["list_files", "read_file"])
      );
  ...
  // -------------------------------------------------------------------------
  // Phase 5.2c — stripThinking (Qwen <think>-history hygiene, REQ-PREP-3)
  // -------------------------------------------------------------------------

  describe("stripThinking option", () => {
    it("strips <think> blocks from recorded assistant responses when enabled", () => {
      const c = new ConversationCompressor({ stripThinking: true });
      c.recordEvents(
        makeTurn("Q", "<think>reasoning here</think>\n\nFinal answer.")
      );
      expect(c.getTurns()[0]!.response).toBe("Final answer.");
    });

    it("preserves <think> blocks by default (no behavior change)", () => {
      const c = new ConversationCompressor();
      c.recordEvents(
        makeTurn("Q", "<think>reasoning here</think>\n\nFinal answer.")
      );
      expect(c.getTurns()[0]!.response).toBe(
        "<think>reasoning here</think>\n\nFinal answer."
      );
    });

    it("keeps the cleaned response in the built history", () => {
      const c = new ConversationCompressor({ stripThinking: true });
      c.recordEvents(makeTurn("Q", "<think>aside</think>Clean output."));
      const history = c.buildHistory();
      expect(history).toContain("[assistant]: Clean output.");
      expect(history).not.toContain("<think>");
    });
  });
});
