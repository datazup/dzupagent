...
describe("CodexAdapter — deep coverage", () => {
  ...
  describe("thread lifecycle", () => {
    ...
    it("resumes an existing thread via resumeThread on resumeSession", async () => {
      mockResumeThread.mockReturnValue(
        createMockThread([threadStarted("tid-resumed"), turnCompleted()]),
      );
    ...
    it("continues thread state across multiple sequential executions", async () => {
      ...
      mockStartThread.mockReturnValueOnce(
        createMockThread([threadStarted("t-1"), turnCompleted()]),
      );
      await collectEvents(adapter.execute(makeInput({ prompt: "turn1" })));
      // Second execute (different thread)
      mockStartThread.mockReturnValueOnce(
        createMockThread([threadStarted("t-2"), turnCompleted()]),
      );
    ...
    it("marks isResume=true on started event when resuming", async () => {
      mockResumeThread.mockReturnValue(
        createMockThread([threadStarted("t-r"), turnCompleted()]),
      );
      const events = await collectEvents(
        adapter.resumeSession("t-r", makeInput()),
      );
  ...
  describe("timeout handling", () => {
    ...
    it("falls back to adapter.config.timeoutMs when input has no override", async () => {
      ...
      (a2 as unknown as { config: Record<string, unknown> }).config[
        "timeoutMs"
      ] = 30;
    ...
    it("emits ADAPTER_TIMEOUT when stream itself takes too long mid-flight", async () => {
      ...
      const failed = events.find((e) => e.type === "adapter:failed") as
  ...
  describe("caller abort handling", () => {
    it('emits adapter:completed with "(interrupted)" when caller aborts mid-stream', async () => {
      ...
      const events = await collectEvents(
        adapter.execute(makeInput({ signal: ctrl.signal })),
      );
  ...
  describe("sandbox mode mapping", () => {
    it('maps "read-only" to SDK "read-only"', async () => {
      ...
      expect(mockStartThread.mock.calls[0]![0]["sandboxMode"]).toBe(
        "read-only",
      );
    });
    ...
    it("input.options.sandboxMode overrides config sandboxMode", async () => {
      ...
      await collectEvents(
        a.execute(
          makeInput({ options: { sandboxMode: "danger-full-access" } }),
        ),
      );
      expect(mockStartThread.mock.calls[0]![0]["sandboxMode"]).toBe(
        "danger-full-access",
  ...
  describe("provider config preservation", () => {
    it("preserves caller-supplied providerOptions.config keys alongside instructions", async () => {
      ...
      const ctorOpts = mockCodexCtor.mock.calls.at(-1)![0] as Record<
        string,
        unknown
      >;
    ...
    it("passes env from config to Codex constructor", async () => {
      const a = new CodexAdapter({
        env: { CODEX_DEBUG: "1", PATH: "/usr/bin" },
      });
      mockStartThread.mockReturnValue(
        createMockThread([threadStarted(), turnCompleted()]),
      );
  ...
  describe("skipped or default-handled item types", () => {
    it("silently skips todo_list items", async () => {
      ...
      expect(
        events.find((e) => e.type === "adapter:tool_call"),
      ).toBeUndefined();
      expect(
        events.find((e) => e.type === "adapter:tool_result"),
      ).toBeUndefined();
    });
  ...
  // ── turn.failed / error surfacing ─────────────────────
  //
  // Regression coverage for the "Reading prompt from stdin..." class of bug:
  // the Codex CLI emits the real failure (e.g. an API 400 "model not supported")
  // as a turn.failed / error stream event on stdout, then the SDK throws with a
  // generic stderr-only message. The adapter must surface the stream-event
  // payload, not the opaque stderr.
  describe("turn.failed / error event surfacing", () => {
    it("emits adapter:failed carrying the turn.failed error message (non-approval)", async () => {
      mockStartThread.mockReturnValue(
        createMockThread([
          threadStarted(),
          {
      ...
      expect(failed?.error).toContain("model is not supported");
      // Must NOT silently succeed with an empty completion.
      const completed = events.find((e) => e.type === "adapter:completed");
      expect(completed).toBeUndefined();
    });

    it("prefers the stream error payload over the SDK stderr-only throw", async () => {
      // Mirror the real failure mode: yield a turn.failed with the true cause,
      // then have the iterator throw the SDK's generic stderr-only error.
      mockStartThread.mockReturnValue({
        runStreamed: vi.fn().mockResolvedValue({
          events: (async function* () {
            yield threadStarted();
            yield {
              type: "turn.failed",
              error: {
                message: "API 400: model is not supported for this account",
              },
            };
            throw new Error(
      ...
      expect(failed).toBeDefined();
      expect(failed?.error).toContain("model is not supported");
      // The opaque stderr line should not be the only thing the caller sees.
      expect(failed?.error).not.toBe(
        "Codex Exec exited with code 1: Reading prompt from stdin...",
      );
    });

    it("surfaces a standalone error item as adapter:failed", async () => {
      mockStartThread.mockReturnValue(
        createMockThread([
          threadStarted(),
          {
            type: "error",
            message: "unexpected provider error: rate limit exceeded",
          },
        ]),
      );
      const events = await collectEvents(adapter.execute(makeInput()));
