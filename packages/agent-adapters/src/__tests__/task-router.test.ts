...
describe("TagBasedRouter", () => {
  ...
  it("respects preferredProvider when available", () => {
    const decision = router.route(makeTask({ preferredProvider: "codex" }), [
      "claude",
      "codex",
    ]);
  ...
  it("routes reasoning tags to claude when available", () => {
    const decision = router.route(makeTask({ tags: ["reasoning"] }), [
      "claude",
      "codex",
    ]);
    expect(decision.provider).toBe("claude");
  ...
  it("routes architecture tags to claude", () => {
    const decision = router.route(makeTask({ tags: ["architecture"] }), [
      "claude",
      "codex",
    ]);
    expect(decision.provider).toBe("claude");
  ...
  it("routes review tags to claude", () => {
    const decision = router.route(makeTask({ tags: ["review"] }), [
      "claude",
      "codex",
    ]);
    expect(decision.provider).toBe("claude");
  ...
  it("routes design/analysis/planning/refactor/explain tags to claude", () => {
    for (const tag of [
      ...
      const decision = router.route(makeTask({ tags: [tag] }), [
        "claude",
        "codex",
      ]);
      expect(decision.provider).toBe("claude");
  ...
  it("respects requiresReasoning flag even without reasoning tag", () => {
    const decision = router.route(makeTask({ requiresReasoning: true }), [
      "claude",
      "codex",
    ]);
    expect(decision.provider).toBe("claude");
  ...
  it("routes execution tags to codex when available", () => {
    const decision = router.route(makeTask({ tags: ["fix-tests"] }), [
      "claude",
      "codex",
    ]);
    expect(decision.provider).toBe("codex");
  ...
  it("respects requiresExecution flag even without execution tag", () => {
    const decision = router.route(makeTask({ requiresExecution: true }), [
      "claude",
      "codex",
    ]);
    expect(decision.provider).toBe("codex");
  ...
  it("falls through to default when execution is requested but codex is unavailable", () => {
    const decision = router.route(makeTask({ tags: ["implement"] }), [
      "claude",
      "gemini",
    ]);
    // codex not available
  ...
  it("routes local tags to crush when available", () => {
    const decision = router.route(makeTask({ tags: ["local"] }), [
      "claude",
      "crush",
      "qwen",
    ]);
  ...
  it("routes local tags to qwen when crush unavailable", () => {
    const decision = router.route(makeTask({ tags: ["offline"] }), [
      "claude",
      "qwen",
    ]);
  ...
  it("routes low budget to cheapest available", () => {
    const decision = router.route(makeTask({ budgetConstraint: "low" }), [
      "claude",
      "crush",
      "gemini",
    ]);
  ...
  it("does not use cheapest path for medium/high/unlimited budget", () => {
    const decision = router.route(makeTask({ budgetConstraint: "high" }), [
      "claude",
      "crush",
    ]);
  ...
  it("uses default highest-priority routing when nothing else matches", () => {
    const decision = router.route(makeTask(), ["gemini", "claude", "crush"]);
...
describe("CostOptimizedRouter", () => {
  ...
  it("respects preferredProvider override", () => {
    const router = new CostOptimizedRouter();
    const decision = router.route(makeTask({ preferredProvider: "claude" }), [
      "claude",
      "crush",
    ]);
    expect(decision.provider).toBe("claude");
...
describe("RoundRobinRouter", () => {
  ...
  it("reset() restores the counter", () => {
    ...
    router.route(makeTask(), [
      "a" as AdapterProviderId,
      "b" as AdapterProviderId,
    ]);
    router.route(makeTask(), [
      "a" as AdapterProviderId,
      "b" as AdapterProviderId,
...
describe("CompositeRouter", () => {
  ...
  it("picks highest weighted score across strategies", () => {
    ...
    const decision = composite.route(makeTask({ tags: ["reasoning"] }), [
      "claude",
      "crush",
    ]);
    expect(decision.provider).toBe("claude");
  ...
  it("normalizes confidence to [0, 1]", () => {
    const tagRouter = new TagBasedRouter();
    const composite = new CompositeRouter([{ strategy: tagRouter, weight: 1 }]);
    const decision = composite.route(makeTask({ tags: ["reasoning"] }), [
      "claude",
    ]);
...
describe("resolveTaskTenantId", () => {
  it("prefers the explicit tenantId field", () => {
    expect(
      resolveTaskTenantId(
        makeTask({ tenantId: "tenant-a", metadata: { tenantId: "tenant-b" } })
      )
    ).toBe("tenant-a");
  });

  it("falls back to metadata.tenantId when tenantId is absent", () => {
    expect(
      resolveTaskTenantId(makeTask({ metadata: { tenantId: "tenant-b" } }))
    ).toBe("tenant-b");
  });

  it("falls back to 'default' when neither is present", () => {
    expect(resolveTaskTenantId(makeTask())).toBe("default");
  });

  it("falls back to 'default' for an empty-string tenantId", () => {
    expect(resolveTaskTenantId(makeTask({ tenantId: "" }))).toBe("default");
  });

  it("falls back to 'default' for a non-string metadata.tenantId", () => {
    expect(resolveTaskTenantId(makeTask({ metadata: { tenantId: 42 } }))).toBe(
      "default"
    );
  });
});
