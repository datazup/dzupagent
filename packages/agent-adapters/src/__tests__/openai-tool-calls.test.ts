...
function toolCallEvents(events: AgentEvent[]): AgentToolCallEvent[] {
  return events.filter(
    (e): e is AgentToolCallEvent => e.type === "adapter:tool_call",
  );
}
...
describe("OpenAIAdapter — tool calling", () => {
  ...
  it("accumulates tool_call argument fragments across deltas and emits a single adapter:tool_call", async () => {
    ...
    expect(types.indexOf("adapter:tool_call")).toBeLessThan(
      types.indexOf("adapter:completed"),
    );
  });
  ...
  it("repairs a truncated tool-call JSON buffer before falling back to raw", async () => {
    // Simulate a stream that was cut off mid-object: an open brace, a complete
    // key/value pair, a trailing comma, and no closing brace. This is the
    // common streaming truncation failure mode and IS repairable.
    const sseLines = [
      'data: {"choices":[{"delta":{"tool_calls":[{"index":0,"type":"function","function":{"name":"buggy","arguments":"{\\"q\\":\\"cats\\","}}]}}]}',
      'data: {"choices":[{"delta":{},"finish_reason":"tool_calls"}]}',
      "data: [DONE]",
    ];
    vi.stubGlobal(
    ...
    const toolCalls = toolCallEvents(events);
    expect(toolCalls).toHaveLength(1);
    // Repaired into a valid object rather than surfaced as the raw string.
    expect(toolCalls[0]!.input).toEqual({ q: "cats" });
  });

  it("falls back to the raw argument string when accumulated JSON is unrepairable", async () => {
    const sseLines = [
  ...
  it("isolates pending tool-call state across consecutive execute() calls", async () => {
    ...
    const events1 = await collectEvents(
      adapter.execute({
        prompt: "a",
        options: { tools: [{ name: "first", parameters: {} }] },
      }),
    );
    const events2 = await collectEvents(
      adapter.execute({
        prompt: "b",
        options: { tools: [{ name: "second", parameters: {} }] },
      }),
    );
