...
describe("GeminiSDKAdapter", () => {
  ...
  it("execute yields started -> stream_delta -> completed events", async () => {
    ...
    const events = await collectEvents(
      adapter.execute({ prompt: "test prompt" })
    );
    ...
    const delta1 = events[1] as Extract<
      AgentEvent,
      { type: "adapter:stream_delta" }
    >;
    expect(delta1.content).toBe("Hello");

    const delta2 = events[2] as Extract<
      AgentEvent,
      { type: "adapter:stream_delta" }
    >;
  ...
  it("resumeSession throws ForgeError", async () => {
    ...
    await expect(
      collectEvents(
        adapter.resumeSession("some-session-id", { prompt: "test" })
      )
  ...
  it("passes native Gemini responseSchema config when outputSchema is provided", async () => {
    ...
    await collectEvents(
      adapter.execute({ prompt: "Return JSON.", outputSchema })
    );
  ...
  it("works with StructuredOutputAdapter on the Gemini SDK seam", async () => {
    ...
    const result = await adapter.execute(
      ...
      JsonOutputSchema.fromZod(z.object({ answer: z.number() }), {
    ...
    expect(result.result.schemaName).toBe(
      "gemini-sdk-adapter.generation.qa.answer"
    );
  ...
  it.each([
    ...
    },
    {
      ...
      response:
        '{"metadata":{"notes":null,"items":[{"id":"item-1","tags":["a","b"]}]}}',
      expectedValue: {
        metadata: {
          notes: null,
    ...
    "propagates native Gemini responseSchema for $label",
    async ({ schema, response, expectedValue, expectedSchema }) => {
      async function* streamChunks() {
        yield {
          text: () => response,
          functionCalls: () => undefined,
      ...
      const adapter = new StructuredOutputAdapter(createGeminiRegistry(), {
        maxRetries: 2,
      });
      const result = await adapter.execute(
