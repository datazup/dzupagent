...
describe("CodexAdapter", () => {
  ...
  describe("execute", () => {
    it("emits started event with enriched fields from thread.started", async () => {
      ...
      const events = await collectEvents(
        adapter.execute(
          makeInput({ systemPrompt: "Be helpful", workingDirectory: "/tmp" }),
        ),
      );
    ...
    it("maps todo_list items to adapter progress updates", async () => {
      ...
      const progress = events.find(
        (event) => event.type === "adapter:progress",
      );
    ...
    it("keeps turn.completed as provider raw while forwarding its usage onto adapter:completed", async () => {
      ...
      const completed = events.find(
        ...
        | Extract<(typeof events)[number], { type: "adapter:completed" }>
        | undefined;
      expect(completed?.usage).toEqual({
        inputTokens: 100,
        outputTokens: 200,
    ...
    it("passes systemPrompt from input as config.instructions to Codex constructor", async () => {
      ...
      await collectEvents(
        adapter.execute(
          makeInput({ systemPrompt: "You are a helpful assistant." }),
        ),
      );
    ...
    it("input.systemPrompt takes priority over providerOptions.systemPrompt", async () => {
      ...
      await collectEvents(
        staticAdapter.execute(
          makeInput({ systemPrompt: "Per-request prompt." }),
        ),
      );
    ...
    it("does not set config.instructions when no systemPrompt is provided", async () => {
      ...
      const ctorCall = mockCodexCtor.mock.calls[0]?.[0] as
    ...
    it("passes developerInstructions from providerOptions as config.developer_instructions", async () => {
      const devAdapter = new CodexAdapter({
        providerOptions: {
          developerInstructions: "Always use structured output.",
        },
      });
      const thread = createMockThread([
        threadStartedEvent(),
      ...
      expect(mockCodexCtor).toHaveBeenCalledWith(
        expect.objectContaining({
          config: expect.objectContaining({
            developer_instructions: "Always use structured output.",
          }),
        }),
      );
    });
    ...
    it("can set both instructions and developer_instructions together", async () => {
      ...
      await collectEvents(
        bothAdapter.execute(makeInput({ systemPrompt: "Be brief." })),
      );
  ...
  describe("signal handling", () => {
    ...
    it("handles pre-aborted external signal", async () => {
      ...
      const events = await collectEvents(
        adapter.execute(makeInput({ signal: ac.signal })),
      );
