...
/**
 * Adapter that parks mid-turn and only advances when its own `input.signal`
 * aborts. Because the generator cannot reach `adapter:completed` on its own,
 * any assertion downstream of the park proves the abort arrived *during* the
 * turn — not merely that a listener was attached after it finished.
 */
function createAbortParkingAdapter(providerId: AdapterProviderId = "codex"): {
  adapter: AgentCLIAdapter;
  /** Resolves once the adapter has parked, i.e. the turn is genuinely in flight. */
  whenParked: Promise<AbortSignal | undefined>;
  getAbortObservedInFlight: () => boolean;
} {
  let signalParked!: (signal: AbortSignal | undefined) => void;
  const whenParked = new Promise<AbortSignal | undefined>((resolve) => {
  ...
  async function* park(
    ...
    yield {
      type: "adapter:started",
      providerId,
      sessionId: "sess-abort-park",
      timestamp: Date.now(),
    };

    signalParked(input.signal);

    // Park until the adapter-side signal aborts. No timeout fallback: if the
    // signal never propagates, this never settles and the test fails by timing
    // out rather than passing vacuously.
    ...
    abortObservedInFlight = input.signal?.aborted === true;

    // Mirrors the real CLI/SDK adapters, which reject with AGENT_ABORTED once
    // the child process is killed rather than yielding an `adapter:failed`
    // event. The registry relies on this to distinguish a caller-initiated
    // cancellation (propagate) from a retryable provider failure (recover).
  ...
  return {
    adapter,
    whenParked,
    getAbortObservedInFlight: () => abortObservedInFlight,
  };
}
...
describe("OrchestratorFacade", () => {
  ...
  describe("run()", () => {
    ...
    it("throws when stream ends without adapter:completed event", async () => {
      ...
      )._registry.executeWithFallback = async function* (): AsyncGenerator<
        AgentEvent,
        void,
        undefined
      > {
  ...
  describe("mapReduce()", () => {
    it("surfaces cancellation at the public API boundary", async () => {
      ...
      const result = await facade.mapReduce("chunk-a\nchunk-b", {
        chunker: {
  ...
  describe("chatWithRaw()", () => {
    ...
    it("links ChatOptions.signal to the adapter-side attempt signal", async () => {
      const { adapter, getCapturedInput } =
        createPolicyCapturingRawAdapter("codex");
      const facade = createOrchestrator({
        adapters: [adapter],
        eventBus: bus,
      });
    ...
    it("propagates a mid-flight ChatOptions.signal abort to the adapter before the turn ends", async () => {
      const { adapter, whenParked, getAbortObservedInFlight } =
        createAbortParkingAdapter("codex");
      const facade = createOrchestrator({
        adapters: [adapter],
        eventBus: bus,
      });
      ...
      const events: AgentStreamEvent[] = [];
      const drained = (async () => {
        for await (const event of facade.chatWithRaw("Long-running turn", {
          provider: "codex",
          signal: controller.signal,
        })) {
          events.push(event);
      ...
      // Attach a no-op rejection handler now so aborting below cannot trip an
      ...
      // demonstrably in flight rather than already complete.
      const adapterSignal = await whenParked;
      expect(adapterSignal).toBeDefined();
      expect(adapterSignal?.aborted).toBe(false);
      ...
      // Only reachable if the abort crossed the facade -> registry -> adapter
      ...
      // laundered into a retryable ALL_ADAPTERS_EXHAUSTED by the fallback chain.
      expect(outcome.ok).toBe(false);
      const error = (outcome as { error: unknown }).error;
      expect(ForgeError.is(error)).toBe(true);
      expect((error as ForgeError).code).toBe("AGENT_ABORTED");
    });
    ...
    it("does not link an unrelated controller when ChatOptions carries no signal", async () => {
      ...
      const events: AgentStreamEvent[] = [];
      for await (const event of facade.chatWithRaw("Plain turn", {
        provider: "codex",
      })) {
        events.push(event);
      }
    ...
    it("roundtrip: toolCallId propagates through adapter emit and consume", async () => {
      ...
      try {
        ...
        const events: AgentStreamEvent[] = [];
        for await (const event of facade.chatWithRaw("Run lookup", {
          provider: "openai",
        })) {
          events.push(event);
        }
    ...
    it("applies policy overrides on raw-capable chat turns", async () => {
      const { adapter, getCapturedInput, configureSpy } =
        createPolicyCapturingRawAdapter("codex");
      const facade = createOrchestrator({
        adapters: [adapter],
        eventBus: bus,
  ...
  describe("event bus integration", () => {
    ...
    it("emits policy conformance violations on strict blocked-attempt fallback runs", async () => {
      const facade = createOrchestrator({
        adapters: [
          createMockAdapter("openai", "OpenAI result"),
    ...
    it("persists conformance governance records across fallback attempts when audit logger is attached", async () => {
      ...
      for await (const _event of facade.chatWithRaw(
        "Strict policy fallback with audit persistence",
        {
          approvedFallbackProviders: ["codex"],
          policy: { networkAccess: false },
          policyConformanceMode: "strict",
      ...
      expect(conformanceEntries[0]!.details).toEqual(
        expect.objectContaining({
          providerId: "openai",
          conformanceMode: "strict",
          fallbackBehavior: "blocked_attempt",
