...
describe("CrossProviderHandoff", () => {
  ...
  describe("buildHandoffContext", () => {
    ...
    it("includes default header", () => {
      ...
      expect(h.buildHandoffContext()).toContain(
        "## Partial progress from previous provider"
      );
    });
  ...
  describe("AGENT-M-07 sanitization (truncation + secret redaction)", () => {
    it("truncates an oversized tool_result with an explicit byte marker", () => {
      const h = new CrossProviderHandoff();
      const huge = "A".repeat(20_000);
      h.recordEvent(toolResultEvent("read_file", huge));
      const ctx = h.buildHandoffContext() ?? "";
      // The full 20KB payload must not survive into the handoff prompt.
      expect(ctx).not.toContain(huge);
      expect(ctx).toMatch(/…\[truncated \d+ bytes\]/);
      // Captured item is bounded (marker + ~4KB head), far under the original.
      expect(ctx.length).toBeLessThan(6_000);
    });

    it("does not truncate content within the size cap", () => {
      const h = new CrossProviderHandoff();
      h.recordEvent(toolResultEvent("grep", "match at line 42"));
      const ctx = h.buildHandoffContext() ?? "";
      expect(ctx).toContain("match at line 42");
      expect(ctx).not.toContain("[truncated");
    });

    it("redacts a secret-like token in a tool_result before handoff", () => {
      const h = new CrossProviderHandoff();
      const secret = "sk-1234567890abcdefghij1234567890";
      h.recordEvent(toolResultEvent("env_dump", `OPENAI_KEY=${secret} rest`));
      const ctx = h.buildHandoffContext() ?? "";
      expect(ctx).not.toContain(secret);
      expect(ctx).toContain("[REDACTED-");
    });

    it("redacts a github token (supplemental pattern) in a message", () => {
      const h = new CrossProviderHandoff();
      const ghp = "ghp_abcdefghijklmnopqrstuvwxyz0123456789";
      h.recordEvent(msgEvent(`token is ${ghp} done`));
      const ctx = h.buildHandoffContext() ?? "";
      expect(ctx).not.toContain(ghp);
      expect(ctx).toContain("[REDACTED-GITHUB-TOKEN]");
    });

    it("redacts an api_key assignment (supplemental pattern) in tool_call args", () => {
      const h = new CrossProviderHandoff();
      const val = "abcdefghijklmnopqrstuvwxyz012345";
      h.recordEvent(toolCallEvent("configure", { note: `api_key=${val}` }));
      const ctx = h.buildHandoffContext() ?? "";
      expect(ctx).not.toContain(val);
      expect(ctx).toContain("[REDACTED-");
    });

    it("redacts secrets before truncation so no partial secret leaks", () => {
      const h = new CrossProviderHandoff();
      const secret = "sk-1234567890abcdefghij1234567890";
      // Place a secret near the end of an oversized payload.
      const payload = "A".repeat(5000) + ` ${secret}`;
      h.recordEvent(toolResultEvent("dump", payload));
      const ctx = h.buildHandoffContext() ?? "";
      expect(ctx).not.toContain(secret);
    });
  });
  ...
  describe("CrossProviderHandoff.enrichInput", () => {
    ...
    it("appends to existing systemPrompt", () => {
      ...
      expect(sp.indexOf("Step A done.")).toBeLessThan(
        sp.indexOf("Be helpful.")
      );
    });
