...
import type { AdapterProviderId, AgentEvent, HealthStatus } from "../types.js";
...
function captureStreamedEvents(): {
  ...
  const spy = vi
    .spyOn(StreamingHandler.prototype, "serialize")
    .mockImplementation(async function* (source) {
...
describe("AdapterHttpHandler", () => {
  ...
  describe("routing", () => {
    ...
    it("POST /supervisor returns result", async () => {
      ...
      const json = asJsonResponse(result as HttpResponse);
      expect(json.status).toBe(200);
      expect(orchestrator.supervisor).toHaveBeenCalledWith(
        "Review the code",
        expect.any(Object),
      );
    });
    ...
    it("POST /supervisor stream uses provider identity from result data", async () => {
      ...
      try {
        ...
        const completed = capture.events.find(
          (event) => event.type === "adapter:completed",
        );
  ...
  describe("runtime fallback attribution", () => {
    it("prefers the requested provider, then explicit providers, then registry order, then unknown", () => {
      ...
      expect(resolveRuntimeFallbackProviderId(registry)).toBe("gemini");
      expect(
        resolveRuntimeFallbackProviderId({
          listAdapters: vi.fn().mockReturnValue([]),
        }),
  ...
  describe("validation", () => {
    ...
    it("invalid body (null) returns 400", async () => {
      const result = await handler.handle(makeRequest("POST", "/run", null));
  ...
  describe("auth", () => {
    it("missing API key returns 401 when validateApiKey is configured", async () => {
      ...
      const result = await authedHandler.handle(makeRequest("GET", "/health"));
    ...
    it("publicEndpoints does not bypass non-listed paths", async () => {
      ...
      // /cost is NOT public
      const result = await authedHandler.handle(makeRequest("GET", "/cost"));
  ...
  describe("approval", () => {
    ...
    it("POST /approve/:id grants request", async () => {
      ...
      expect(mockGate.grant).toHaveBeenCalledWith(
        "req-123",
        "admin",
        undefined,
      );
  ...
  describe("error handling", () => {
    it("returns 500 on orchestrator error", async () => {
      ...
      // ERR-C-02: the raw thrown message must NOT reach the client; a generic
      // operation-failed message is returned instead.
      expect(json.body["error"] as string).not.toContain("kaboom");
      expect(json.body["error"] as string).toContain("failed");
    });
