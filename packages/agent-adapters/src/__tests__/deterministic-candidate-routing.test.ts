...
describe("deterministic candidate routing", () => {
  it.each(["llm-rank"] as const)(
    ...
    (strategy) => {
      expect(() =>
        selectExecutionRoute(policy([candidate("one")], { strategy }), {
  ...
  it("selects the single candidate under a fixed strategy", () => {
    ...
    expect(decision.reasoningSummary).toContain(
      `Fixed route selected ${only.id}`,
    );
  });

  it("produces an identical fixed decision on repeated invocation", () => {
    const fixedPolicy = policy([candidate("codex:sdk:work")], {
      strategy: "fixed",
      fallback: "none",
    });

    expect(
      selectExecutionRoute(fixedPolicy, { decidedAt: DECIDED_AT }),
  ...
  it("denies a fixed policy carrying two candidates instead of guessing one", () => {
    expect(() =>
      selectExecutionRoute(
        policy(
          [
            candidate("codex:sdk:work"),
            candidate("codex:cli:work", { backend: "cli" }),
          ],
          {
            strategy: "fixed",
            fallback: "none",
  ...
  it("rejects duplicate candidate identities instead of silently collapsing them", () => {
    ...
    expect(() =>
      selectExecutionRoute(policy([candidate("same"), duplicate]), {
        decidedAt: DECIDED_AT,
      }),
    ).toThrow(
      expect.objectContaining({
  ...
  it("does not widen legacy fallback to unapproved healthy providers", () => {
    ...
    expect(buildFallbackOrder(decision, ["claude", "codex", "openai"])).toEqual(
      ["claude"],
    );
    expect(
      buildFallbackOrder(decision, ["claude", "codex", "openai"], ["codex"]),
  ...
  it("uses candidate ID as a deterministic tie-break independent of input order", () => {
    const first = candidate("gemini:sdk", {
      provider: "gemini-sdk",
      backend: "sdk",
    });
    const second = candidate("gemini:cli", {
      provider: "gemini",
      backend: "cli",
    });
    const forward = selectExecutionRoute(
      policy([first, second], { preferenceOrder: [] }),
      { decidedAt: DECIDED_AT },
    );
    const reverse = selectExecutionRoute(
      policy([second, first], { preferenceOrder: [] }),
      { decidedAt: DECIDED_AT },
    );
  ...
  it("represents Goose plus provider separately from direct Ollama", () => {
    ...
    const decision = selectExecutionRoute(
      policy([goose, direct], { preferenceOrder: [direct.id, goose.id] }),
      { decidedAt: DECIDED_AT },
    );
  ...
  it("returns stable rejection codes for auth, model, health, capability, and policy failures", () => {
    const candidates = [
      ...
      candidate("unhealthy", {
        health: { status: "unhealthy", reason: "probe failed" },
      }),
    ...
    expect(
      Object.fromEntries(
        decision.rejected.map((item) => [item.candidateId, item.codes]),
      ),
    ).toMatchObject({
  ...
  it("blocks local-to-remote, identity, privacy, and higher-cost fallback without approval", () => {
    ...
    expect(decision.fallbackCandidateIds).toEqual([]);
    expect(
      decision.rejected.find((item) => item.candidateId === remote.id)?.codes,
    ).toEqual(["TRANSITION_APPROVAL_REQUIRED"]);
  ...
  it("keeps same-candidate retry distinct from compatible fallback", () => {
    expect(
      planCandidateRecovery({
        candidateId: "ollama:local",
        failureCode: "ADAPTER_TIMEOUT",
        recoverable: true,
        attempt: 1,
        maxSameCandidateRetries: 1,
        compatibleFallbackCandidateIds: ["ollama:local-compatible"],
      }),
    ).toEqual({
    ...
    expect(
      ...
      kind: "fallback-candidate",
      candidateId: "ollama:local-compatible",
    });
    ...
    expect(
      planCandidateRecovery({
        ...
        recoverable: true,
        attempt: 1,
        maxSameCandidateRetries: 3,
        compatibleFallbackCandidateIds: ["openai:remote"],
      }),
  ...
  it("materializes live Ollama health, inventory, and model capabilities", async () => {
    ...
    expect(materialized?.capabilities).toEqual(
  ...
  it("keeps local OpenAI-compatible and remote API candidates separate", () => {
    ...
    const decision = selectExecutionRoute(
      policy([remoteApi, localCompatible], {
