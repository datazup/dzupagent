...
export interface PmTaskStore {
  ...
  update(
    id: string,
    patch: Partial<Omit<PmTask, "id" | "createdAt">>,
  ): PmTask | undefined;
  ...
  list(filter?: { status?: PmTaskStatus; assignee?: string }): PmTask[] {
    ...
    return all.filter((t) => {
      if (filter.status !== undefined && t.status !== filter.status)
        return false;
      if (filter.assignee !== undefined && t.assignee !== filter.assignee)
        return false;
      return true;
    });
  }
...
function buildPmGetTask(
  store: PmTaskStore,
): ExecutableDomainTool<GetTaskInput, GetTaskOutput> {
  const definition: DomainToolDefinition = {
    name: "pm.get_task",
...
export function buildPmTools(
  ...
  return [
    buildPmCreateTask(store, idFactory, now),
    buildPmUpdateTask(store),
    buildPmGetTask(store),
    buildPmListTasks(store),
  ];
}
