...
/**
 * Existential-unpack helper for tests: `buildTopicsTools` returns the
 * heterogeneous storage type `AnyExecutableDomainTool` (`execute(input: never)`).
 * To exercise `execute` with concrete inputs we recover a callable signature
 * via a single `as` (no `unknown` laundering), widening the parameter to
 * `Record<string, unknown>` — the same boundary the registry uses.
 */
function asCallable(
  tool: AnyExecutableDomainTool,
...
describe("topics.list — lenient filter behavior", () => {
  it("resolves with { topics: [] } when tag is a number (non-string never matches string arrays)", async () => {
    ...
    const listTool = asCallable(
      tools.find((t) => t.definition.name === "topics.list")!,
    );
    const result = await listTool.execute({ tag: 42 });
...
describe("topics.search — invalid input rejection", () => {
  it("throws when query is not a string (schema constraint violated)", async () => {
    const catalogMap = new Map(CATALOG.map((t) => [t.id, t]));
    const tools = buildTopicsTools(catalogMap);
    const searchTool = asCallable(
      tools.find((t) => t.definition.name === "topics.search")!,
    );
    await expect(searchTool.execute({ query: 42 })).rejects.toThrow();
  });
});
...
describe("topics.search — result shape matches TopicSearchResult schema exactly", () => {
  it("each result contains exactly {id, title, score} when topic has no summary", async () => {
    const noSummaryCatalog: TopicRecord[] = [
      { id: "t1", title: "Testing", tags: ["qa"] },
    ];
