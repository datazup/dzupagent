...
function getExecutor<TInput, TOutput>(
  ...
  // Existential unpack: recover the concrete tool generics from the
  // heterogeneous `AnyExecutableDomainTool` storage type, keyed by name.
  return exec as ExecutableDomainTool<TInput, TOutput>;
}
...
describe("project_docs.read", () => {
  ...
  it("refuses paths that escape rootDir", async () => {
    ...
    await expect(exec.execute({ path: "../etc/passwd" })).rejects.toThrow(
      /outside rootDir/,
    );
  });
});
...
describe("pm.*", () => {
  ...
  it("update_task mutates state and list_tasks filters by status", async () => {
    ...
    const { task: updated } = await update.execute({
      id: t1.id,
      status: "done",
    });
  ...
  it("update_task throws when the task does not exist", async () => {
    const { executors } = createBuiltinToolRegistry();
    const update = getExecutor<{ id: string }, { task: PmTask }>(
      executors,
      "pm.update_task",
    );
    await expect(update.execute({ id: "missing" })).rejects.toThrow(
      /task not found/,
    );
  });
...
describe("workflow.*", () => {
  ...
  it("workflow.run transitions a run to succeeded and exposes it via workflow.status", async () => {
    ...
    const status = getExecutor<
      { runId: string },
      { run: WorkflowRunRecord | null }
    >(executors, "workflow.status");
...
describe("topics.*", () => {
  ...
  it("topics.search limit restricts output when multiple topics match", async () => {
    ...
    const limited = await search.execute({
      query: "index login invoices",
      limit: 1,
    });
...
describe("record.*", () => {
  ...
  describe("record.append — file-backed persistence", () => {
    ...
    it("writes entries as JSONL lines when recordsDir is set", async () => {
      ...
      const defaultLines = (await fs.readFile(defaultFile, "utf8"))
        .trim()
        .split("\n");
      const otherLines = (await fs.readFile(otherFile, "utf8"))
        .trim()
        .split("\n");
...
describe("toToolResolver", () => {
  it("resolve returns compiler-grade metadata for a registered tool", async () => {
    ...
    await expect(
      resolved?.handle.execute({ entry: "hello" }, {}),
