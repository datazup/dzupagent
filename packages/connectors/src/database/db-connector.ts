 ...
 * forms and rejects multi-statement/query-shape bypasses (see
 * {@link ./db-sql-safety.js} / {@link ./db-sql-lexer.js}).
 *
 * This module is orchestration only: it composes the connection pool
 * ({@link ./db-connection.js}), query executors ({@link ./db-query.js}),
 * the programmatic operations surface ({@link ./db-operations.js}), and
 * error containment ({@link ./db-errors.js}) into LangChain tools.
 */
import { z } from "zod";
...
import { handleDbToolError } from "./db-errors.js";
import type { DatabaseConnectorConfig, QueryExecutor } from "./db-types.js";
import type { DatabaseOperations } from "./db-operations.js";

// ---------------------------------------------------------------------------
// Re-exported public surface (symbols physically live in siblings, but the
// `@dzupagent/connectors` package and its tests import them from this module,
// so the historical export shape is preserved here).
// ---------------------------------------------------------------------------
...
export { DbToolError } from "./db-errors.js";
