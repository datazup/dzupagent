...
function enforceReadOnlyStatement(sql: string): string {
  ...
  if (statements.length === 0) {
    throw new Error(
      "Write operations not allowed (read-only mode). SQL query is empty."
    );
  }
  if (statements.length > 1) {
    throw new Error(
      "Write operations not allowed (read-only mode). Multiple SQL statements are not permitted."
    );
  }
  ...
  if (root === "WITH" && isDataModifyingWithStatement(masked)) {
    throw new Error(
      "Write operations not allowed (read-only mode). Data-modifying CTEs are not permitted."
    );
  }
  ...
  if (root === "EXPLAIN") {
    if (EXPLAIN_ANALYZE_RE.test(masked)) {
      throw new Error(
        "Write operations not allowed (read-only mode). EXPLAIN ANALYZE is not permitted."
      );
    }
    ...
    if (explainedRoot && WRITE_ROOT_KEYWORDS.has(explainedRoot)) {
      throw new Error(
        "Write operations not allowed (read-only mode). EXPLAIN of write statements is not permitted."
      );
    }
    if (
      explainedRoot === "WITH" &&
      isDataModifyingWithStatement(explainedStatement)
    ) {
      throw new Error(
        "Write operations not allowed (read-only mode). EXPLAIN of data-modifying CTEs is not permitted."
      );
    }
  }
...
async function createPool(config: DatabaseConnectorConfig): Promise<PgPool> {
  ...
  try {
    ...
    PgPool =
      ...
      (pgModule as unknown as { Pool: typeof PgPool }).Pool;
  } catch {
    throw new Error(
...
export function createDatabaseOperations(
  ...
  return {
    async query(sql: string, params?: unknown[]): Promise<QueryResult> {
      ...
      if (
        shouldApplyAutoLimit(maskedCandidate) &&
        !LIMIT_RE.test(maskedCandidate)
      ) {
    ...
    async describeTable(
      ...
      return result.rows.map((row) => {
        ...
        return {
          ...
          isPrimaryKey:
            row["is_primary_key"] === true || row["is_primary_key"] === "t",
        };
      });
    },
...
// ---------------------------------------------------------------------------
// Error containment (DZUPAGENT-ERR-M-06)
// ---------------------------------------------------------------------------
...
export function createDatabaseConnector(
  ...
  const all: DynamicStructuredTool[] = [
    ...
    new DynamicStructuredTool({
      name: "db-query",
      description: `Execute a parameterized SQL query against ${dbName}. ${
        readOnly
          ? "Read-only: allows SELECT/WITH/SHOW/VALUES and safe EXPLAIN only; blocks multi-statement and data-modifying CTE/query shapes."
          : "Read-write access."
      } Results limited to ${maxRows} rows.`,
      schema: z.object({
        sql: z
          .string()
          .describe("Parameterized SQL query (use $1, $2... for parameters)"),
        params: z
          .array(z.unknown())
          .optional()
      ...
      func: async ({ sql, params }) => {
        try {
          ...
          return handleDbToolError("query", "Query error", err);
        }
      },
    }),
    ...
    new DynamicStructuredTool({
      ...
      func: async ({ schema }) => {
        try {
          ...
          return handleDbToolError("list_tables", "Error listing tables", err);
        }
      },
    }),
    ...
    new DynamicStructuredTool({
      ...
      func: async ({ table, schema }) => {
        try {
          ...
          const rowEstimate =
            info.rowCount != null ? `Estimated rows: ${info.rowCount}` : "";
          return [
            header,
            rowEstimate,
            "",
 ...
 */
export function createDatabaseConnectorToolkit(
  config: DatabaseConnectorConfig
): ConnectorToolkit {
  return {
    name: "database",
