...
describe("Database connector — extended", () => {
  describe("connection error handling", () => {
    ...
    it("reports authentication failure", async () => {
      ...
      expect(result).toContain("Query error");
      expect(result).not.toContain("password authentication failed");
    });
    ...
    it("reports SSL connection error", async () => {
      const query = vi
        .fn()
        .mockRejectedValue(
          new Error("SSL connection error: self signed certificate")
        );
      const tools = createDatabaseConnector(makeConfig({ query }));
      const dbQuery = tools.find((t) => t.name === "db-query")!;
      const result = await dbQuery.invoke({ sql: "SELECT 1" });
      ...
      expect(result).toContain("Query error");
      expect(result).not.toContain("SSL connection error");
    });
  ...
  describe("query timeout", () => {
    it("reports timeout error through db-query tool", async () => {
      ...
      expect(result).not.toContain("Query read timeout");
    });
    ...
    it("reports statement timeout from pg", async () => {
      ...
      expect(result).toContain("Query error");
      expect(result).not.toContain("statement timeout");
    });
  });
  ...
  describe("transaction error paths", () => {
    ...
    it("reports foreign key violation", async () => {
      ...
      expect(result).toContain("Query error");
      expect(result).not.toContain("foreign key constraint");
    });
  ...
  describe("multiple consecutive queries", () => {
    ...
    it("handles a mix of successes and failures", async () => {
      ...
      expect(r2).not.toContain("connection lost");
  ...
  describe("parameter sanitization", () => {
    ...
    it("handles numeric parameter values", async () => {
      ...
      expect(query).toHaveBeenCalledWith(expect.any(String), [42, 18]);
    });
    ...
    it("handles boolean parameter values", async () => {
      ...
      expect(query).toHaveBeenCalledWith(expect.any(String), [true]);
    });
  });
  ...
  describe("createDatabaseOperations — additional", () => {
    ...
    it("query blocks EXPLAIN of write statements in read-only mode", async () => {
      ...
      const ops = createDatabaseOperations(executor, { readOnly: true });
      await expect(
        ops.query("EXPLAIN UPDATE users SET name = $1")
  ...
  describe("db-describe-table — additional cases", () => {
    ...
    it("passes custom schema to describe", async () => {
      ...
      const tools = createDatabaseConnector(makeConfig({ query }));
      const describeTool = tools.find((t) => t.name === "db-describe-table")!;
      const result = await describeTool.invoke({
        table: "events",
        schema: "analytics",
      });
  ...
  describe("read-only edge cases", () => {
    ...
    it("blocks case-insensitive write keywords", async () => {
      ...
      const r1 = await dbQuery.invoke({
        sql: "insert into users (name) values ($1)",
      });
      expect(r1).toContain("not permitted");

      const r2 = await dbQuery.invoke({ sql: "  DELETE FROM users" });
      expect(r2).toContain("not permitted");
    });
  ...
  describe("transaction-like sequences", () => {
    ...
    it("reports failure mid-transaction (caller can issue ROLLBACK)", async () => {
      ...
      expect(r3).not.toContain("duplicate key");
      expect(r4).not.toContain("Query error");
    });
  });
  ...
  describe("concurrent queries", () => {
    ...
    it("handles mixed parallel success/failure", async () => {
      ...
      expect(results[1]).not.toContain("deadlock");
  ...
  describe("PostgreSQL-specific error codes (ERR-M-06: sanitized categories)", () => {
    it("classifies relation-not-found without leaking the table name", async () => {
      const query = vi
        .fn()
        .mockRejectedValue(
      ...
      // Raw driver phrasing + object name never leak.
      expect(result).not.toContain("nonexistent_table");
      expect(result).not.toContain("does not exist");
    });

    it("classifies column-not-found without leaking the column name", async () => {
      const query = vi
        .fn()
        .mockRejectedValue(new Error('column "missing_col" does not exist'));
      const tools = createDatabaseConnector(makeConfig({ query }));
      const dbQuery = tools.find((t) => t.name === "db-query")!;
      const result = await dbQuery.invoke({ sql: "SELECT missing_col FROM t" });
      expect(result).toContain("was not found");
      expect(result).not.toContain("missing_col");
    });

    it("classifies syntax errors without echoing the raw driver text", async () => {
      const query = vi
        .fn()
      ...
      expect(result).not.toContain("at or near");
    });
  });
});
