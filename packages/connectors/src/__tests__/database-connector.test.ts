...
describe("Database connector", () => {
  describe("read-only mode", () => {
    ...
    for (const sql of writeStatements) {
      ...
      it(`blocks ${keyword} in read-only mode`, async () => {
        ...
        const tools = createDatabaseConnector(
          makeConfig({ query, readOnly: true })
        );
        const dbQuery = tools.find((t) => t.name === "db-query")!;
        const result = await dbQuery.invoke({ sql });
        // ERR-M-06: the rejection is reported as a sanitized error and the
        // write is NOT executed against the DB.
    ...
    it("blocks multi-statement payloads in read-only mode", async () => {
      ...
      expect(result).toContain("Query error");
      expect(result).toContain("not permitted");
      expect(result).not.toContain("Multiple SQL statements");
      expect(query).not.toHaveBeenCalled();
    });
    ...
    it("blocks data-modifying CTEs in read-only mode", async () => {
      ...
      expect(result).toContain("Query error");
      expect(result).toContain("not permitted");
      expect(result).not.toContain("Data-modifying CTEs");
      expect(query).not.toHaveBeenCalled();
    });
    ...
    it("blocks EXPLAIN ANALYZE in read-only mode", async () => {
      ...
      expect(result).toContain("Query error");
      expect(result).toContain("not permitted");
      expect(result).not.toContain("EXPLAIN ANALYZE");
      expect(query).not.toHaveBeenCalled();
    });
    ...
    it("defaults to read-only when readOnly is omitted", async () => {
      ...
      // Default read-only blocks the write → sanitized rejection, query not run.
      expect(result).toContain("Query error");
      expect(result).toContain("not permitted");
      expect(query).not.toHaveBeenCalled();
    });
  ...
  describe("error handling", () => {
    it("catches query errors and returns a SANITIZED message (ERR-M-06)", async () => {
      const query = vi
        .fn()
      ...
      expect(result).toContain("Query error");
      // Sanitized category summary reaches the LLM; raw driver text does not.
      expect(result).toContain("was not found");
      expect(result).not.toContain('relation "users" does not exist');
    ...
    it("handles non-Error thrown values", async () => {
      ...
      expect(result).toContain("Query error");
      expect(result).not.toContain("unexpected string error");
    });
  });
  ...
  describe("tool metadata", () => {
    it("uses custom database name in descriptions", () => {
      const tools = createDatabaseConnector(
        makeConfig({ databaseName: "analytics_db" })
      );
  ...
  describe("tool filtering", () => {
    it("filters to only enabled tools", () => {
      const tools = createDatabaseConnector(
        makeConfig({ enabledTools: ["db-query"] })
      );
  ...
  describe("createDatabaseOperations", () => {
    ...
    it("query throws in read-only mode for write statements", async () => {
      ...
      await expect(ops.query("DROP TABLE users")).rejects.toThrow(
        "not allowed"
      );
    });
