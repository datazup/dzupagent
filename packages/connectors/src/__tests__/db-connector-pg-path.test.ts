...
const { createDatabaseConnector, createDatabaseOperations } = await import(
...
describe("Database connector — pg executor path (branch coverage)", () => {
  ...
  describe("createPool: configuration branches", () => {
    it("passes connectionString when provided", async () => {
      mockPoolQuery.mockResolvedValue(
        pgResult([{ id: 1 }], [{ name: "id", dataTypeID: 23 }])
      );
      ...
      expect(poolCtorCalls[0]?.["connectionString"]).toBe(
  ...
  describe("createPgExecutor: result mapping", () => {
    ...
    it("falls back to oid:NNN for unknown type IDs", async () => {
      mockPoolQuery.mockResolvedValue(
        pgResult([{ weird: "xyz" }], [{ name: "weird", dataTypeID: 99999 }])
      );
      const tools = createDatabaseConnector({});
      const dbQuery = tools.find((t) => t.name === "db-query")!;
    ...
    it("uses rows.length when rowCount is null", async () => {
      mockPoolQuery.mockResolvedValue(
        pgResult([{ a: 1 }, { a: 2 }], [{ name: "a", dataTypeID: 23 }], null)
      );
      const tools = createDatabaseConnector({});
      const result = await tools
        .find((t) => t.name === "db-query")!
        .invoke({ sql: "SELECT a FROM t" });
      expect(result).toContain("2 rows");
    });

    it("uses explicit rowCount when provided", async () => {
      mockPoolQuery.mockResolvedValue(
        pgResult([{ a: 1 }], [{ name: "a", dataTypeID: 23 }], 42)
      );
      const tools = createDatabaseConnector({});
      const result = await tools
        .find((t) => t.name === "db-query")!
    ...
    it("contains DB errors: sanitized summary to the LLM, full detail logged (ERR-M-06)", async () => {
      // A raw driver error that leaks table/object internals.
      const rawDriverError = new Error(
        'relation "secret_accounts" does not exist'
      );
      mockPoolQuery.mockRejectedValue(rawDriverError);
      const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
      try {
        const tools = createDatabaseConnector({});
        const result = await tools
          .find((t) => t.name === "db-query")!
          .invoke({ sql: "SELECT * FROM secret_accounts" });

        // LLM-facing output is a sanitized, category-based summary.
        expect(result).toContain("Query error");
        expect(result).toContain("was not found"); // object_not_found category summary
        // Raw driver text (table name, exact driver phrasing) NEVER leaks.
        expect(result).not.toContain("secret_accounts");
        expect(result).not.toContain('relation "secret_accounts"');

        // Full detail IS logged server-side (structured).
        expect(errorSpy).toHaveBeenCalled();
        const logged = JSON.parse(errorSpy.mock.calls[0]?.[0] as string);
        expect(logged.component).toBe("db-connector");
        expect(logged.operation).toBe("query");
        expect(logged.category).toBe("object_not_found");
        expect(logged.error.message).toContain("secret_accounts");
      } finally {
    ...
    it("uses transaction-level read-only execution when pool.connect is available", async () => {
      const originalConnect = (
        MockPool.prototype as unknown as { connect?: unknown }
      ).connect;
      ...
      (MockPool.prototype as unknown as { connect?: unknown }).connect =
        connectSpy;
      ...
      try {
        ...
        const result = await tools
          .find((t) => t.name === "db-query")!
          .invoke({ sql: "SELECT 1 AS n LIMIT 1" });
        expect(result).toContain("1 rows");
        ...
        if (typeof originalConnect === "function") {
          ...
          delete (MockPool.prototype as unknown as { connect?: unknown })
            .connect;
        }
      }
    });
  ...
  describe("lazy pool initialisation", () => {
    it("creates only one pool across multiple tool invocations", async () => {
      mockPoolQuery.mockResolvedValue(
        pgResult([{ v: 1 }], [{ name: "v", dataTypeID: 23 }])
      );
      const tools = createDatabaseConnector({});
