...
function makeStubConnector(
  overrides: Partial<SQLConnector> = {},
): SQLConnector {
  return {
    getDialect: () => "postgresql",
    testConnection: async () =>
      ({ ok: true, latencyMs: 5 }) as ConnectionTestResult,
    executeQuery: async () =>
      ({
        columns: [],
        rows: [],
        ...
        makeTable({
          columns: [
            makeColumn({
              isPrimaryKey: true,
              dataType: "bigserial",
              defaultValue: "nextval('x_seq'::regclass)",
            }),
          ],
        }),
        "mysql",
  ...
  describe("Foreign key branches", () => {
    it("emits FK constraint with schema prefix when referencedSchema set", () => {
      const ddl = generateDDL(
        makeTable({
          foreignKeys: [
            makeFK({ referencedSchema: "other", referencedTable: "users" }),
          ],
        }),
        "postgresql",
      );
    ...
    it("emits FK constraint without schema prefix when referencedSchema empty", () => {
      const ddl = generateDDL(
        makeTable({
          foreignKeys: [
            makeFK({ referencedSchema: "", referencedTable: "users" }),
          ],
        }),
        "postgresql",
      );
...
describe("createSQLTools — branch coverage", () => {
  describe("sql-query tool", () => {
    ...
    it("sanitizes Error instance message in JSON", async () => {
      const connector = makeStubConnector({
        executeQuery: async () => {
      ...
      const result = await tools
        ...
        .invoke({ sql: "SELECT 1" });
      const parsed = JSON.parse(result);
      expect(parsed.error).toBe("Query error: the operation failed.");
    });

    it("sanitizes non-Error rejection via String()", async () => {
      const connector = makeStubConnector({
        executeQuery: async () => {
          throw "raw string";
        },
  ...
  describe("sql-list-tables tool", () => {
    ...
    it("sanitizes Error message on discovery failure", async () => {
      const connector = makeStubConnector({
        discoverSchema: async () => {
          throw new Error("schema denied");
        },
      });
      const tools = createSQLTools({ connector });
      const result = await tools
      ...
      expect(parsed.error).toBe(
        "Error listing tables: the operation was not permitted for this connection.",
      );
    });
    ...
    it("sanitizes non-Error rejection via String()", async () => {
      ...
      const result = await tools
        ...
        .invoke({});
      const parsed = JSON.parse(result);
      expect(parsed.error).toBe("Error listing tables: the operation failed.");
    });
    ...
    it("passes schemaName to discoverSchema when provided", async () => {
      ...
      await tools
        .find((t) => t.name === "sql-list-tables")!
        .invoke({ schemaName: "custom" });
      expect(spy).toHaveBeenCalledWith(
        expect.objectContaining({ schemaName: "custom" }),
      );
    });
  });
  ...
  describe("sql-describe-table tool", () => {
    it("returns DDL + table metadata", async () => {
      ...
      const tools = createSQLTools({ connector });
      const result = await tools
        .find((t) => t.name === "sql-describe-table")!
        .invoke({ tableName: "u" });
      const parsed = JSON.parse(result);
    ...
    it("uses default sampleValueLimit of 5 when not specified", async () => {
      ...
      await tools
        ...
        .invoke({ tableName: "t" });
      expect(spy).toHaveBeenCalledWith(
        expect.objectContaining({ sampleValueLimit: 5 }),
      );
    });
    ...
    it("honours explicit sampleValueLimit", async () => {
      ...
      expect(spy).toHaveBeenCalledWith(
        expect.objectContaining({ sampleValueLimit: 10 }),
      );
    });

    it("sanitizes Error message when discovery fails", async () => {
      const connector = makeStubConnector({
        discoverSchema: async () => {
      ...
      const parsed = JSON.parse(result);
      expect(parsed.error).toBe(
        "Error describing table: the operation failed.",
      );
    });
  ...
  describe("sql-discover-schema tool", () => {
    it("passes includeTables and excludeTables through", async () => {
      ...
      const tools = createSQLTools({ connector });
      await tools
        .find((t) => t.name === "sql-discover-schema")!
        .invoke({
          includeTables: ["a", "b"],
          excludeTables: ["c"],
          sampleValueLimit: 0,
    ...
    it("sanitizes Error message on failure", async () => {
      const connector = makeStubConnector({
        discoverSchema: async () => {
      ...
      const parsed = JSON.parse(result);
      expect(parsed.error).toBe(
        "Error discovering schema: the operation failed.",
      );
    });
  ...
  describe("sql-generate-ddl tool", () => {
    it("returns DDL string for existing table", async () => {
      ...
      const tools = createSQLTools({ connector });
      const result = await tools
        .find((t) => t.name === "sql-generate-ddl")!
        .invoke({ tableName: "x" });
    ...
    it("sanitizes Error JSON on discovery failure", async () => {
      const connector = makeStubConnector({
        discoverSchema: async () => {
          throw new Error("lost conn");
        },
      });
      const tools = createSQLTools({ connector });
      const result = await tools
        ...
        .invoke({ tableName: "t" });
      const parsed = JSON.parse(result);
      expect(parsed.error).toBe("Error generating DDL: the operation failed.");
    });
  ...
  describe("sql-test-connection tool", () => {
    it("returns test result on success", async () => {
      ...
      const tools = createSQLTools({ connector });
      const result = await tools
        .find((t) => t.name === "sql-test-connection")!
        .invoke({});
      const parsed = JSON.parse(result);
    ...
    it("returns sanitized failure JSON when testConnection rejects", async () => {
      const connector = makeStubConnector({
        testConnection: async () => {
      ...
      expect(parsed.error).toBe("Connection error: the operation failed.");
...
describe("createDatabaseOperations — LIMIT wrapping", () => {
  ...
  it("enforces readOnly default (true) when not specified", async () => {
    const executor = makeExec({ rows: [], rowCount: 0 });
    const ops = createDatabaseOperations(executor, {});
    await expect(ops.query("DELETE FROM t")).rejects.toThrow(
