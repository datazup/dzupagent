...
describe('parseFlow — worker.dispatch node', () => {
  ...
  it('rejects a missing dispatchId', () => {
    ...
    expect(
      result.errors.some((e) => e.message.includes('dispatchId')),
    ).toBe(true)
  })
  ...
  it('rejects a missing instructions field', () => {
    ...
    expect(result.ast).toBeNull()
    expect(
      result.errors.some((e) => e.message.includes('instructions')),
    ).toBe(true)
  })
  ...
  it('rejects a missing outputKey', () => {
    ...
    expect(result.ast).toBeNull()
    expect(
      result.errors.some((e) => e.message.includes('outputKey')),
    ).toBe(true)
  })
  ...
  it('rejects a non-object input', () => {
    ...
    expect(result.ast).toBeNull()
    expect(result.errors.some((e) => e.message.includes('input'))).toBe(
      true,
    )
  })
  ...
  it('rejects a non-array commandAllowlist', () => {
    const result = parseFlow(
      baseDispatch({ commandAllowlist: 'git status' }),
    )
    expect(result.ast).toBeNull()
    expect(
  ...
  it('rejects an invalid resultFormat', () => {
    ...
    expect(result.ast).toBeNull()
    expect(
      result.errors.some((e) => e.message.includes('resultFormat')),
    ).toBe(true)
  })
})
