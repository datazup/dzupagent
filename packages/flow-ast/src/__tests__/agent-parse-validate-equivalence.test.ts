...
describe('equivalence — output (required object)', () => {
  ...
  it('both accept output with inline schema', () => {
    const node = { ...VALID_BASE, output: { key: 'out', schema: { type: 'object' } } }
    expect(parseAccepts(node)).toBe(true)
    expect(validateAccepts(node)).toBe(true)
  })
  ...
  it('both reject output.schema that is not an object', () => {
    const node = { ...VALID_BASE, output: { key: 'out', schema: 'not-object' } }
    expect(parseRejects(node)).toBe(true)
    expect(validateRejects(node)).toBe(true)
  })
})
...
describe('equivalence — input (optional object)', () => {
  it('both accept a valid input object', () => {
    const node = { ...VALID_BASE, input: { topic: 'flow dsl', maxResults: 10 } }
    expect(parseAccepts(node)).toBe(true)
    expect(validateAccepts(node)).toBe(true)
  })
...
// ── parse divergence note: stop.maxToolCalls validation differs ───────────────
//
// DRIFT DETECTED: parse/agent.ts requires maxToolCalls to be a positive
// integer (Number.isFinite + Number.isInteger + > 0), while validate/agent.ts
// only requires typeof === 'number' (no integer or positive guard). This
// divergence is captured as a test expectation so it does not regress silently.

describe('equivalence — stop.maxToolCalls (drift documented)', () => {
  ...
  it('parse rejects zero maxToolCalls; validate accepts it (known drift)', () => {
    const node = { ...VALID_BASE, stop: { maxToolCalls: 0 } }
    // Parser enforces > 0 positive integer.
    expect(parseRejects(node)).toBe(true)
    // Validator does NOT enforce positive — it just checks typeof number.
    expect(validateAccepts(node)).toBe(true)
  })
})
...
describe('equivalence — onInvalidOutput', () => {
  it('both accept a valid onInvalidOutput', () => {
    const node = {
      ...VALID_BASE,
      onInvalidOutput: { retry: 2, repairPrompt: true, failAfterRetries: false },
    }
    expect(parseAccepts(node)).toBe(true)
    expect(validateAccepts(node)).toBe(true)
  })
  ...
  it('parse accepts non-boolean repairPrompt (silently ignores); validate rejects it (known drift)', () => {
    const node = { ...VALID_BASE, onInvalidOutput: { retry: 1, repairPrompt: 'yes' } }
...
describe('equivalence — retry', () => {
  ...
  it('both reject retry.onInvalidOutput with missing attempts', () => {
    const node = { ...VALID_BASE, retry: { onInvalidOutput: { repairPrompt: true } } }
    expect(parseRejects(node)).toBe(true)
    expect(validateRejects(node)).toBe(true)
  })

  it('both reject retry.onValidationFailure with negative attempts', () => {
    const node = { ...VALID_BASE, retry: { onValidationFailure: { attempts: -1 } } }
    expect(parseRejects(node)).toBe(true)
    expect(validateRejects(node)).toBe(true)
  })
})
...
describe('equivalence — policy', () => {
  ...
  // DRIFT DETECTED: policy.maxToolCalls has opposite enforcement to
  // policy.timeoutMs. The parser uses `numberField` (no positive/integer
  // guard), while the validator enforces positive integer. Zero is accepted by
  // the parser but rejected by the validator.
  it('parse accepts zero policy.maxToolCalls; validate rejects it (known drift)', () => {
    const node = { ...VALID_BASE, policy: { maxToolCalls: 0 } }
    // Parser uses `numberField` — no positive/integer check.
    expect(parseAccepts(node)).toBe(true)
    // Validator enforces positive integer.
    expect(validateRejects(node)).toBe(true)
  })

  it('both reject policy.approval.requiredFor that is not an array of strings', () => {
    const node = { ...VALID_BASE, policy: { approval: { requiredFor: 'all' } } }
    expect(parseRejects(node)).toBe(true)
    expect(validateRejects(node)).toBe(true)
  })
})
