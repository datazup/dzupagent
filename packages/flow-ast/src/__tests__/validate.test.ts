...
import {
  FLOW_NODE_VALIDATOR_DESCRIPTORS,
  KNOWN_NODE_TYPES,
} from "../validation-descriptors.js";
    ...
    if (!result.success) {
      expect(
        result.error.issues.some((i) =>
          i.message.includes("route.provider is required")
        )
      ).toBe(true);
    }
  });
...
describe("flowNodeSchema.parse", () => {
  ...
  it("throws SchemaValidationError on invalid input", () => {
    expect(() => flowNodeSchema.parse({ type: "action", id: "x" })).toThrow(
      SchemaValidationError
    );
  });
...
describe("flowDocumentSchema", () => {
      ...
      expect(result.error.issues).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            ...
            message:
              "input spec.default must be a JSON-like value when present",
          }),
        ])
      );
    }
  ...
  it("rejects a document whose canonical nodes are missing ids", () => {
    ...
    if (!result.success) {
      expect(
        result.error.issues.some((issue) => issue.path.endsWith(".id"))
      ).toBe(true);
    }
  });
  ...
  it("rejects duplicate canonical node ids", () => {
    ...
    expect(errors.some((error) => error.code === "DUPLICATE_NODE_ID")).toBe(
      true
    );
  });
});
...
describe("validation extraction seams", () => {
  it("keeps node validator descriptors exhaustive with public node kinds", () => {
    expect(
      FLOW_NODE_VALIDATOR_DESCRIPTORS.map(
        (descriptor) => descriptor.type
      ).sort()
    ).toEqual([...FLOW_NODE_KINDS].sort());
...
describe("flowNodeSchema.safeParse — AdapterRunNode", () => {
  it("accepts an explicit-provider adapter.run node", () => {
    const result = flowNodeSchema.safeParse({
      type: "adapter.run",
      id: "summarize",
      provider: "claude",
    ...
    expect(result.success).toBe(true);
    if (result.success) expect(result.data.type).toBe("adapter.run");
  });

  it("accepts a tags-routed adapter.run node without a provider", () => {
    const result = flowNodeSchema.safeParse({
      type: "adapter.run",
      id: "route",
      tags: ["reasoning"],
  ...
  it("rejects adapter.run with neither provider nor tags", () => {
    ...
    if (!result.success) {
      expect(result.error.issues[0]!.code).toBe("MISSING_REQUIRED_FIELD");
      expect(result.error.issues.map((i) => i.path)).toContain("root.provider");
    }
  });
