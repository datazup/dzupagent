import type { AdapterParallelNode } from "../types.js";
import {
  type ParseContext,
  describeJsType,
  joinPointer,
...
export function parseAdapterParallel(
  obj: Record<string, unknown>,
  pointer: string,
  ctx: ParseContext
): AdapterParallelNode | null {
  ...
  const providers = obj.providers as ParallelProvider[];

  const instructions = obj.instructions;
  if (typeof instructions !== "string" || instructions.length === 0) {
    ctx.errors.push({
      code: "WRONG_FIELD_TYPE",
      message: `adapter.parallel.instructions must be a non-empty string, received ${describeJsType(
        instructions
      )}`,
  ...
  if (typeof output !== "string" || output.length === 0) {
    ctx.errors.push({
      code: "WRONG_FIELD_TYPE",
      message: `adapter.parallel.output must be a non-empty string, received ${describeJsType(
        output
      )}`,
  ...
  if (obj.merge !== undefined) {
    if (
      ...
      ctx.errors.push({
        ...
        pointer: joinPointer(pointer, "merge"),
      });
      return null;
    }
  }
