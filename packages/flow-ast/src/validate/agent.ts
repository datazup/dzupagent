...
export function validateAgent(
  ...
  if (typeof agentId !== 'string' || agentId.length === 0) {
    issues.push({
      ...
      message: `agent.agentId is required (non-empty string), received ${describeJsType(agentId)}`,
    })
    ok = false
  }
  ...
  const output = validateAgentOutput(obj['output'], joinPath(path, 'output'), issues)
  ...
  const retry = validateAgentRetry(obj['retry'], joinPath(path, 'retry'), issues)
  ...
  const policy = validateAgentPolicy(obj['policy'], joinPath(path, 'policy'), issues)
  ...
  if ('repair' in obj && obj['repair'] !== undefined) {
    ...
    if (!isPlainObject(repairRaw)) {
      issues.push({
        ...
        code: 'MISSING_REQUIRED_FIELD',
        message: `validate.repair must be an object, received ${describeJsType(repairRaw)}`,
      })
    } else {
...
function validateAgentStop(
  ...
  if (!isPlainObject(raw)) {
    issues.push({
      path,
      code: 'MISSING_REQUIRED_FIELD',
      message: `agent.stop must be an object when present, received ${describeJsType(raw)}`,
    })
    return undefined
  }
...
function validateOnInvalidOutput(
  ...
  if (!isPlainObject(raw)) {
    issues.push({
      path,
      code: 'MISSING_REQUIRED_FIELD',
      message: `agent.onInvalidOutput must be an object, received ${describeJsType(raw)}`,
    })
    return undefined
  }
  ...
  if (isPlainObject(onInvalidOutput)) {
    ...
    if (typeof attempts !== 'number' || attempts < 0) {
      issues.push({
        ...
        code: 'MISSING_REQUIRED_FIELD',
        message: 'retry.onInvalidOutput.attempts is required (non-negative number)',
      })
    } else {
  ...
  if (isPlainObject(onValidationFailure)) {
    ...
    if (typeof attempts !== 'number' || attempts < 0) {
      issues.push({
        ...
        code: 'MISSING_REQUIRED_FIELD',
        message: 'retry.onValidationFailure.attempts is required (non-negative number)',
      })
    } else {
      const branch: NonNullable<AgentRetry['onValidationFailure']> = { attempts }
  ...
  if (isPlainObject(onModelUnavailable)) {
    ...
    if (typeof attempts !== 'number' || attempts < 0) {
      issues.push({
        ...
        code: 'MISSING_REQUIRED_FIELD',
        message: 'retry.onModelUnavailable.attempts is required (non-negative number)',
      })
    } else {
      const branch: NonNullable<AgentRetry['onModelUnavailable']> = { attempts }
...
function validateAgentValidation(
  ...
  if (!isPlainObject(raw)) {
    issues.push({
      path,
      code: 'MISSING_REQUIRED_FIELD',
      message: `agent.validation must be an object, received ${describeJsType(raw)}`,
    })
    return undefined
  }
...
function validateAgentPolicy(
  ...
  if (!isPlainObject(raw)) {
    issues.push({
      path,
      code: 'MISSING_REQUIRED_FIELD',
      message: `agent.policy must be an object, received ${describeJsType(raw)}`,
    })
    return undefined
  }
