import type { FlowDocumentPolicy, FlowDocumentV1 } from '../types.js'
import {
  describeJsType,
  isFlowValue,
  isPlainObject,
  ...
  if ('budgetCents' in value && value['budgetCents'] !== undefined) {
    ...
    if (typeof v !== 'number' || !Number.isFinite(v)) {
      issues.push({
        ...
        code: 'MISSING_REQUIRED_FIELD',
        message: 'document.policy.budgetCents must be a finite number when present',
      })
    } else if (v <= 0) {
      issues.push({
        path: joinPath(joinPath(path, 'policy'), 'budgetCents'),
  ...
  if ('timeoutMs' in value && value['timeoutMs'] !== undefined) {
    ...
    if (typeof v !== 'number' || !Number.isFinite(v)) {
      issues.push({
        ...
        code: 'MISSING_REQUIRED_FIELD',
        message: 'document.policy.timeoutMs must be a finite number when present',
      })
    } else if (v <= 0) {
      issues.push({
        path: joinPath(joinPath(path, 'policy'), 'timeoutMs'),
