import type { AdapterRaceNode, FlowNode } from "../types.js";
...
export function validateAdapterRace(
  ...
  const providers = providersValue as RaceProvider[];
  ...
  if (typeof instructions !== "string" || instructions.length === 0) {
    issues.push({
      ...
      code: "MISSING_REQUIRED_FIELD",
      message: `adapter.race.instructions is required (non-empty string), received ${describeJsType(
        instructions
      )}`,
    });
    return null;
  ...
  if (typeof output !== "string" || output.length === 0) {
    issues.push({
      ...
      code: "MISSING_REQUIRED_FIELD",
      message: `adapter.race.output is required (non-empty string), received ${describeJsType(
        output
      )}`,
    });
    return null;
