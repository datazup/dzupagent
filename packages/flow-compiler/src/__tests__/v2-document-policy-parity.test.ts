...
describe("DSL-MVP-POLICY-01 compiler document-policy parity", () => {
  it.each<[string, FlowDocumentPolicy | undefined]>([
    ...
    "preserves %s policy across every document frontend",
    async (_name, policy) => {
      ...
      const results = [
        await compiler.compileDsl(v1),
        await compiler.compileDsl(v2),
        await compiler.compileDocument(canonical.document),
        await compileTextInput(compiler, v2),
        await compileTextInput(compiler, JSON.stringify(canonical.document)),
      ];
      const baseline = results[0];
      if (baseline === undefined || "errors" in baseline)
      ...
      for (const result of results) {
        ...
        // Pipeline artifact/node IDs are generated per compile. Pin canonical
        // node lineage and executable payload without erasing generated IDs.
      ...
      if (
        ...
        throw new Error("missing successful frontends");
  ...
  it.each([0, -1])(
    "rejects invalid policy %s before artifact emission",
    async (budgetCents) => {
      const compiler = createFlowCompiler({ toolResolver: resolver });
      ...
      expect(result.errors).toContainEqual(
        expect.objectContaining({ nodePath: "root.policy.budgetCents" }),
      );
    },
  );
});
