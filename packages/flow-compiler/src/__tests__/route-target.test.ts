...
// ---------------------------------------------------------------------------
// MPCO P1 — RUNTIME_LEAF helpers and tests.
// ---------------------------------------------------------------------------

const adapterRun = (instructions = "do it"): FlowNode =>
  ({
    type: "adapter.run",
    provider: "codex",
    instructions,
...
describe("RUNTIME_LEAF routing (MPCO P1)", () => {
  ...
  it("T2: for_each + adapter.run stays on pipeline (leaf-safe), not diverted", () => {
    const result = routeTarget(forEachLeaf(adapterRun()));
    expect(result.target).toBe("pipeline");
