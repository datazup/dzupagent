...
describe("computeFeatureBitmask", () => {
  ...
  it("returns BRANCH bit for branch node", () => {
    ...
    expect(computeFeatureBitmask(ast) & FEATURE_BITS.BRANCH).toBe(
      FEATURE_BITS.BRANCH
    );
  });
  ...
  it("returns FOR_EACH bit for for_each node", () => {
    ...
    expect(computeFeatureBitmask(ast) & FEATURE_BITS.FOR_EACH).toBe(
...
describe("validateShape", () => {
  ...
  it("emits MISSING_REQUIRED_FIELD when action.input is missing", () => {
    ...
    expect(
      errors.some(
        (e) =>
          e.code === "MISSING_REQUIRED_FIELD" && e.message.includes("input")
      )
    ).toBe(true);
  });
  ...
  it("OI-4: emits error for on_error in skill-chain-routed flow", () => {
    ...
    expect(
      errors.some(
        (e) =>
          e.code === "MISSING_REQUIRED_FIELD" && e.message.includes("on_error")
      )
    ).toBe(true);
  });
  ...
  it("does NOT emit OI-4 error for on_error in pipeline-routed flow", () => {
    ...
    //
    // Deliberately NOT guarded with a non-empty length assertion (cf.
    // ORCH-DSL-TEST-H-05, which added such guards in compile.test.ts): this is
    // a negative assertion, so an empty `errors` array is a legitimate pass,
    // not a vacuous one. Requiring errors here would assert the opposite of
...
describe("semanticResolve", () => {
  ...
  it("emits UNRESOLVED_TOOL_REF for missing ref", async () => {
    ...
    const result = await semanticResolve(ast, { toolResolver: resolver });
    expect(result.errors.some((e) => e.code === "UNRESOLVED_TOOL_REF")).toBe(
      true
    );
  });
  ...
  it("emits UNRESOLVED_PERSONA_REF when no personaResolver is provided", async () => {
    ...
    expect(result.errors.some((e) => e.code === "UNRESOLVED_PERSONA_REF")).toBe(
      true
    );
  });
  ...
  it("emits RESOLVER_INFRA_ERROR when resolver throws", async () => {
    ...
    const ast: FlowNode = makeAction("skill:a");
    const result = await semanticResolve(ast, { toolResolver: resolver });
    expect(result.errors.some((e) => e.code === "RESOLVER_INFRA_ERROR")).toBe(
      true
    );
...
describe("UNIMPLEMENTED_AT_RUNTIME", () => {
  it("warns that an emit node will not be published at runtime", async () => {
    ...
    const warning = result.warnings.find(
      (w) => w.code === "UNIMPLEMENTED_AT_RUNTIME"
    );
  ...
  it("does not warn on a spawn that leaves waitForCompletion unset", async () => {
    ...
    const result = await semanticResolve(ast, { toolResolver: resolver });
    expect(
      result.warnings.some((w) => w.code === "UNIMPLEMENTED_AT_RUNTIME")
    ).toBe(false);
  });
});
