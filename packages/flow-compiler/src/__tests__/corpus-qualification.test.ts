...
describe("flow corpus qualification", () => {
  ...
  describe("formatter round trip", () => {
    ...
    describe("lossless ratchet", () => {
      // Varying the floor proves the comparison, but no fixture here is
      // FORMATTER-lossy (see the note below on why one cannot be authored).
      // That half was proven by mutation instead: dropping the `outputKey`
      // emitter in format-interaction-nodes.ts took the corpus to 10/26
      // lossless, where ungated qualification still exited 0 - the silent
      // regression this gate exists to stop - while `--min-lossless 26`
      // exited 1. Re-run that mutation if you change this gate.
