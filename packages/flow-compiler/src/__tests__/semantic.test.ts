...
describe("semanticResolve — tool resolution", () => {
  ...
  it("multiple unresolved refs: errors emitted in document order", async () => {
    ...
    expect(result.errors.every((e) => e.code === "UNRESOLVED_TOOL_REF")).toBe(
      true
    );
  });
  ...
  it('suggestionDistance: 0 disables "Did you mean" even on close misses', async () => {
    ...
    expect(result.errors[0]?.message).toBe(
      'Unresolved tool reference: "pm.creat_task".'
    );
  });
...
describe("semanticResolve — condition expression validation", () => {
  ...
  it("rejects branch condition with syntax error", async () => {
    ...
    expect(condErr?.message).toContain(
      "branch.condition is not a valid expression"
    );
  });
  ...
  // TEST-N-01: for_each.source must reject the UNSAFE_PATTERNS
  // (eval / dynamic Function / import) just like branch conditions do.
  it.each([
    ["eval", 'eval("rm -rf /")'],
    ["Function", 'new Function("return items")'],
    ["import", 'import("node:fs")'],
  ])(
    "rejects for_each source containing %s as a disallowed construct",
    async (_label, source) => {
      const resolver = makeResolver(["pm.task"]);
      const ast = sequence(forEach(source, "item", action("pm.task")));
      const result = await semanticResolve(ast, { toolResolver: resolver });
      const condErr = result.errors.find((e) => e.code === "INVALID_CONDITION");
      expect(condErr).toBeDefined();
      expect(condErr?.nodeType).toBe("for_each");
  ...
  it("rejects condition with Function() constructor", async () => {
    const resolver = makeResolver(["pm.task"]);
    const ast = sequence(
      branch('new Function("return 1")', [action("pm.task")])
    );
    const result = await semanticResolve(ast, { toolResolver: resolver });
    const condErr = result.errors.find((e) => e.code === "INVALID_CONDITION");
    expect(condErr).toBeDefined();
  ...
  it("collects condition error alongside unresolved tool ref", async () => {
    ...
    const result = await semanticResolve(ast, { toolResolver: resolver });
    expect(result.errors.some((e) => e.code === "INVALID_CONDITION")).toBe(
      true
    );
