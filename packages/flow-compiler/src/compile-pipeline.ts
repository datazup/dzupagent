/**
 * @dzupagent/flow-compiler — core compilation pipeline.
 *
 * Houses the `createFlowCompiler` factory plus the four-stage pipeline
 * (parse → shape-validate → semantic-resolve → route+lower) and its
 * private helpers. `index.ts` is the public API barrel that re-exports
 * `createFlowCompiler` and the stage/type surface.
 *
 * Stage pipeline:
...
export function createFlowCompiler(opts: CompilerOptions): FlowCompiler {
  ...
  async function compileDsl(
    ...
    const documentPolicy = canonicalized.ok
      ? extractDocumentPolicy(canonicalized.document)
      : undefined;
    const result = await compile(prepared.flowInput, {
