...
import {
  ...
  DEFAULT_WINDOW_OVERLAP_LINES,
  estimateTokens,
  MIN_CHUNK_TOKENS,
  splitByHeadings,
  uuidFromSeed,
  type HeadingSectionLine,
} from "./chunker-internals.js";
...
export class SmartChunker {
  ...
  chunkText(text: string, sourceId: string): ChunkResult[] {
    ...
    const rawChunks: Array<{
      ...
      startOffset: number;
      endOffset: number;
      boundaryType: ChunkMetadata["boundaryType"];
    }> = [];
    let start = 0;
    ...
    return rawChunks.map((raw, index) => {
      ...
      return {
        id: uuidFromSeed(`${sourceId}:${index}`),
        text: raw.content,
        tokenCount,
  ...
  computeChunkQuality(
    ...
    const textToNoiseRatio = Math.min(
      1,
      densityRaw < 0.5 ? densityRaw / 0.5 : 1
    );
    ...
    const avgSentenceLength =
      sentences.length > 0
        ? sentences.reduce((sum, s) => sum + s.trim().split(/\s+/).length, 0) /
          sentences.length
        : 0;
    const meaningfulSentencesScore =
      sentences.length >= 2 ? 1 : sentences.length === 1 ? 0.5 : 0.1;
    ...
    const tokenRatio =
      tokenRatioRaw < 0.3
        ? tokenRatioRaw / 0.3
        : Math.min(1, 1 / tokenRatioRaw);

    // --- Position penalty ---
    const positionScore =
      chunkIndex === totalChunks - 1 && totalChunks > 1 ? 0.7 : 1.0;
    ...
    const boilerplateScore =
      boilerplateRatio > 0.3 ? 0 : Math.max(0, 1 - boilerplateRatio * 3);
    ...
    const structureScore =
      (hasHeaders ? 0.4 : 0) + (hasLists ? 0.3 : 0) + (hasCodeBlocks ? 0.3 : 0);
  ...
  private findBestBreakpoint(
    ...
    for (const boundary of BOUNDARY_PRIORITIES) {
      ...
      const regex = new RegExp(
        boundary.pattern.source,
        boundary.pattern.flags + "g"
      );
