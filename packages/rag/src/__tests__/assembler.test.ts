...
function makeChunk(
  overrides: Partial<ScoredChunk> & { id: string }
): ScoredChunk {
  return {
...
describe("ContextAssembler", () => {
  ...
  describe("context modes", () => {
    it('excludes sources with mode "off"', () => {
      ...
      const offBreakdown = assembled.sourceBreakdown.find(
        (b) => b.sourceId === "src-off"
      );
    ...
    it('uses summary for sources with mode "insights"', () => {
      ...
      const result = makeRetrievalResult(chunks);
      const summary =
        "This is a detailed summary of the document that is long enough.";
      const sourceMeta = new Map([
    ...
    it("sorts insights before full chunks", () => {
      ...
      expect(assembled.contextText.indexOf(summary)).toBeLessThan(
        assembled.contextText.indexOf("Content of c1")
      );
    });
  });
  ...
  describe("token budget", () => {
    ...
    it("preserves insights summaries when trimming for budget", () => {
      ...
      const insightsCitation = assembled.citations.find(
        (c) => c.sourceId === "src-insights"
      );
  ...
  describe("citations", () => {
    ...
    it("formats context text with numbered source references", () => {
      const chunks = [makeChunk({ id: "c1", sourceId: "src-1", score: 0.9 })];
      const result = makeRetrievalResult(chunks);
      const sourceMeta = new Map([
        makeSourceMeta("src-1", { title: "My Source" }),
      ]);
  ...
  describe("buildGroundedPrompt", () => {
    ...
    it("applies custom template with {{source_context}} placeholder", () => {
      ...
      expect(prompt).toBe(
        'PREFIX: <untrusted_content source="retrieved_content">\nmy-context\n</untrusted_content> :SUFFIX'
      );
    });
  });
  ...
  describe("source breakdown", () => {
    it("aggregates token and chunk counts per source", () => {
      ...
      const assembled = assembler.assembleContext(result, sourceMeta);
      const bd1 = assembled.sourceBreakdown.find(
        (b) => b.sourceId === "src-1"
      )!;
      const bd2 = assembled.sourceBreakdown.find(
        (b) => b.sourceId === "src-2"
      )!;
