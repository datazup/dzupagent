...
describe("QualityBoostedRetriever — deep branches", () => {
  ...
  it("forces qualityBoosting=false on underlying retriever (double-boost prevention)", async () => {
    ...
    // The options are the *fourth* argument; passing them third put them in
    // the `sourceQualities` slot, so this assertion used to hold no matter
    // what the override did.
    await retriever.retrieve(
      "q",
      {},
      {},
      { mode: "vector", qualityBoosting: true }
    );
    expect(base.retrieve).toHaveBeenCalledWith(
      "q",
      {},
