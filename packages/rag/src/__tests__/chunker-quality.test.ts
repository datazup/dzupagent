...
describe("SmartChunker", () => {
  ...
  describe("chunkText", () => {
    ...
    it("assigns sequential chunk indices", () => {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        expect(result[i]!.id).toMatch(/^[0-9a-f-]{36}$/);
      }
    });
    ...
    it("generates overlapping chunks", () => {
      ...
      for (let i = 1; i < result.length; i++) {
        expect(result[i]!.metadata.startOffset).toBeLessThan(
  ...
  describe("boundary detection", () => {
    it("splits at markdown header boundaries", () => {
      ...
      expect(
        result.find((c) => c.metadata.boundaryType === "header")
      ).toBeDefined();
    });
    ...
    it("splits at paragraph boundaries (double newline)", () => {
      ...
      expect(result.length).toBeGreaterThanOrEqual(2);
      expect(
        result.find((c) => c.metadata.boundaryType === "paragraph")
      ).toBeDefined();
    });
  ...
  describe("computeChunkQuality", () => {
    ...
    it("returns positive quality for meaningful content", () => {
      const content =
        "The quick brown fox jumps over the lazy dog. This is a test of the quality scoring system. It should produce a reasonable quality score.";
    ...
    it("penalizes the last chunk in a multi-chunk document", () => {
      const content =
        "The quick brown fox jumps over the lazy dog. This is a meaningful sentence with enough words.";
    ...
    it("detects boilerplate content and lowers quality", () => {
      const boilerplate =
        "Please subscribe to our newsletter. Accept cookies. Follow us on social media. Sign up for our service. Privacy policy applies. Copyright reserved. All rights reserved.";
      const clean =
        "Machine learning models process data through multiple layers. Neural networks use activation functions to transform inputs.";
    ...
    it("assigns vocabulary diversity score", () => {
      ...
      expect(diverseQ.vocabularyDiversity).toBeGreaterThan(
        repetitiveQ.vocabularyDiversity
      );
    });
    ...
    it("assigns structure score for headers, lists, and code blocks", () => {
      const structured =
        "# Header\n- List item one\n- List item two\n```\ncode block\n```";
      const plain =
        "Just some plain text without any structural elements at all.";
    ...
    it("clamps overall score between 0 and 1", () => {
      const content =
        "Some content for testing. This is a real sentence with words.";
      const quality = chunker.computeChunkQuality(content, 0, 1);
