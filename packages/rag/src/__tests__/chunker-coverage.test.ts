...
describe("SmartChunker — coverage", () => {
  ...
  describe("boundary-aware splitting", () => {
    ...
    it("falls back to token boundary when no sentence breaks found", () => {
      ...
      const tokenBoundaries = chunks.filter(
        (c) => c.metadata.boundaryType === "token"
      );
  ...
  describe("overlap", () => {
    it("produces overlapping chunks when overlapFraction > 0", () => {
      ...
      // Overlap means the tail of chunk N is re-emitted at the head of N+1.
      // The offset bound below is satisfied by non-overlapping chunks too, so
      // assert on the shared text itself — that is what "overlap" means.
      const lastWords = chunks[0]!.text.trim().split(/\s+/).slice(-3).join(" ");
      expect(chunks[1]!.text).toContain(lastWords);
  ...
  describe("quality scoring", () => {
    it("assigns quality scores between 0 and 1", () => {
      ...
      const text =
        "This is a well-structured paragraph with good vocabulary diversity. ".repeat(
          20
        );
    ...
    it("gives higher quality to text with structure (headers, lists)", () => {
      ...
      if (structuredChunks.length > 0 && plainChunks.length > 0) {
        expect(structuredChunks[0]!.quality).toBeGreaterThan(
          plainChunks[0]!.quality
        );
      }
    });
  });
  ...
  describe("token estimation", () => {
    it("estimates token count for each chunk", () => {
      ...
      for (const chunk of chunks) {
        ...
        expect(chunk.tokenCount).toBeCloseTo(
