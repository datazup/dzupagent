...
## Purpose
`@dzupagent/agent` is the orchestration layer that turns lower-level core primitives into a production agent runtime. Its central type is `DzupAgent`, which supports direct generation, streaming, tool-calling loops, workflow composition, orchestration patterns, and safety guardrails.

## Main Responsibilities
- Provide `DzupAgent` for `generate()`, `stream()`, and tool-wrapped usage.
...
- `agent/`: `DzupAgent`, tool loop, dynamic tool registry, state serialization helpers.
...
1. Construct `DzupAgent` with model, tools, guardrails, and optional hooks/config.
...
- Depends on `@dzupagent/core`, `@dzupagent/context`, and `@dzupagent/memory-ipc`.
- Commonly hosted by `@dzupagent/server`.
- Often combined with `@dzupagent/codegen` tools and `@dzupagent/connectors` integrations.
