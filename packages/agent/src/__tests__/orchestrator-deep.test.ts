...
describe("AgentOrchestrator.parallel — deep branches", () => {
  it("actually runs agents concurrently (timing test)", async () => {
    ...
    const a1 = createAgentWithModel(
      "p1",
      createDelayedModel("r1", delayMs, recordStart)
    );
    const a2 = createAgentWithModel(
      "p2",
      createDelayedModel("r2", delayMs, recordStart)
    );
    const a3 = createAgentWithModel(
      "p3",
      createDelayedModel("r3", delayMs, recordStart)
    );
  ...
  it("results preserved in agent array order regardless of completion order", async () => {
    ...
    const merged = await AgentOrchestrator.parallel([a1, a2], "input", (r) =>
      r.join("|")
    );
  ...
  it("throws OrchestrationError when all agents are excluded by breaker", async () => {
    const a1 = createAgentWithModel(
      "x",
      createMockModel([{ content: "unused" }])
    );
    const a2 = createAgentWithModel(
      "y",
      createMockModel([{ content: "unused" }])
  ...
  it("merge strategy returns partial result when some agents fail (allSettled path)", async () => {
    const a1 = createAgentWithModel(
      "ok",
      createMockModel([{ content: "success-data" }])
    );
  ...
  it("JSON-stringifies non-string merge strategy output", async () => {
    const agent = createAgentWithModel(
      "obj",
      createMockModel([{ content: "data" }])
    );
  ...
  it('emits "Merge status: X (no output)" when strategy omits output', async () => {
    const agent = createAgentWithModel(
      "no-out",
      createMockModel([{ content: "ignored" }])
    );
  ...
  it("legacy merge path (no options) uses Promise.all and rejects on any failure", async () => {
    const good = createAgentWithModel(
      "good",
      createMockModel([{ content: "ok" }])
    );
    ...
    await expect(
      AgentOrchestrator.parallel([good, bad], "input")
  ...
  it("FirstWins strategy: returns first successful output", async () => {
    const a1 = createAgentWithModel(
      "first",
      createMockModel([{ content: "first-value" }])
    );
    const a2 = createAgentWithModel(
      "second",
      createMockModel([{ content: "second-value" }])
    );
...
// ===========================================================================
// Parallel — maxConcurrency + signal options
// ===========================================================================

describe("AgentOrchestrator.parallel — maxConcurrency and signal", () => {
  it("maxConcurrency=1 serialises agents (peak active never exceeds 1)", async () => {
    let active = 0;
    let peak = 0;
    const makeThrottledAgent = (id: string, content: string): DzupAgent =>
      createAgentWithModel(id, {
        invoke: vi.fn(async () => {
          active++;
    ...
    const a1 = makeThrottledAgent("t1", "r1");
    const a2 = makeThrottledAgent("t2", "r2");
    const a3 = makeThrottledAgent("t3", "r3");
  ...
  it("maxConcurrency=2 allows up to 2 simultaneous agents", async () => {
    let active = 0;
    let peak = 0;
    const makeAgent = (id: string): DzupAgent =>
      createAgentWithModel(id, {
        invoke: vi.fn(async () => {
          ...
          active--;
          return new AIMessage({ content: id, response_metadata: {} });
        }),
        bindTools: vi.fn(function (this: BaseChatModel) {
          return this;
        }),
    ...
    await AgentOrchestrator.parallel(
      [makeAgent("a"), makeAgent("b"), makeAgent("c"), makeAgent("d")],
      "go",
      undefined,
      { maxConcurrency: 2 }
    );
  ...
  it("maxConcurrency does not affect result ordering", async () => {
    const a1 = createAgentWithModel("mc1", createDelayedModel("first", 30));
    const a2 = createAgentWithModel("mc2", createDelayedModel("second", 5));
    const a3 = createAgentWithModel("mc3", createDelayedModel("third", 15));
  ...
  it("pre-aborted signal rejects parallel immediately (no agents invoked)", async () => {
    const controller = new AbortController();
    controller.abort();

    const m = createMockModel([{ content: "never" }]);
    const agent = createAgentWithModel("abort-test", m);

    await expect(
      AgentOrchestrator.parallel([agent], "input", undefined, {
  ...
  it("external signal cancels in-flight parallel agents", async () => {
    const controller = new AbortController();
    const invoked = vi.fn();

    const slowModel: BaseChatModel = {
      invoke: vi.fn(async () => {
        invoked();
        ...
        return new AIMessage({ content: "never", response_metadata: {} });
      }),
      bindTools: vi.fn(function (this: BaseChatModel) {
        return this;
      }),
    ...
    const agent = createAgentWithModel("slow", slowModel);
    const parallel$ = AgentOrchestrator.parallel([agent], "input", undefined, {
      signal: controller.signal,
    });

    setTimeout(() => controller.abort(), 20);

    await expect(parallel$).rejects.toBeDefined();
...
describe("AgentOrchestrator.supervisor — deep branches", () => {
  ...
  it("throws OrchestrationError when all specialists tripped in circuit breaker", async () => {
    const manager = createAgentWithModel(
      "mgr",
  ...
  it("does not record specialist timeout when manager errors before invoking tools", async () => {
    ...
    const spec = createAgentWithModel(
      "s1",
      createMockModel([{ content: "ok" }])
    );
  ...
  it("routing policy narrows specialist selection", async () => {
    const managerModel = createMockModel([{ content: "done" }]);
    const specA = createAgentWithModel(
      "sa",
      createMockModel([{ content: "a" }])
    );
    const specB = createAgentWithModel(
      "sb",
...
describe("OrchestrationError", () => {
  ...
  it("accepts arbitrary context", () => {
    const err = new OrchestrationError("x", "debate", {
      attempt: 3,
      ids: ["a"],
    });
...
describe("orchestration-telemetry helpers", () => {
  ...
  it("recordCircuitBreakerEvent handles each event type", () => {
    ...
    expect(
      (console.debug as unknown as ReturnType<typeof vi.fn>).mock.calls
    ).toHaveLength(4);
  });
...
describe("agent state isolation — parallel lanes", () => {
  it("two agent instances with same id do not interfere in parallel", async () => {
    ...
    const result = await AgentOrchestrator.parallel([a1, a2], "shared", (rs) =>
      rs.join(",")
    );
