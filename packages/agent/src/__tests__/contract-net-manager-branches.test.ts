...
function makeModel(
  respondWith: string | ((i: number) => string)
): BaseChatModel {
  let i = 0;
  return {
    invoke: vi.fn(async (_messages: BaseMessage[]) => {
      const content =
        typeof respondWith === "function" ? respondWith(i) : respondWith;
      i++;
      ...
      invoke: vi.fn(async (msgs: BaseMessage[]) => {
        ...
        return new AIMessage({
          content: JSON.stringify({
            ...
            confidence: 0.9,
            approach: "A",
            // Declared so the bid clears the capability filter and execution
            // reaches the prompt assertions below; this test is about CFP
            // prompt content, not eligibility.
