...
function makeToolLoopResult(
  overrides: Partial<ToolLoopResult> = {},
): ToolLoopResult {
  return {
...
describe("Session S — plugin.trackPhase() wiring", () => {
  it('charges the "prompt" phase during prepareRunState with the estimated prepared-transcript token count', async () => {
  ...
  it('charges the "tool-result" phase via runToolLoop\'s onToolResult hook during executeGenerateRun', async () => {
    const plugin = makeSpyPlugin();
  ...
  it("per-phase breakdown appears in the lifecycle report when a real manager is attached", async () => {
