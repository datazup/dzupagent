/**
 * Regression suite for the memory write-back stop-reason guard shared by all
 * generate and streaming run coordinators.
 *
 ...
 *
 * Generate, the non-streaming fallback used by `stream()`, and native
 * streaming consume one shared policy. Native streaming additionally retains
 * the last streamed response because ceiling stops occur outside the
 * final-answer branch and compression can replace the accumulated transcript.
 *
...
/** Native-stream counterpart that emits one tool-calling response per turn. */
function makeAlwaysToolCallingStreamingModel(content: string): BaseChatModel {
  return {
    ...
    bindTools: vi.fn().mockReturnThis(),
    model: "memory-write-back-stream-model",
  } as unknown as BaseChatModel;
}
...
function makeHaltingTokenPlugin(): AgentLoopPlugin {
  return {
    onUsage: vi.fn(),
    trackPhase: vi.fn(),
    maybeCompress: vi.fn(async (_messages: BaseMessage[]) => ({
      messages: [new HumanMessage("compressed history without the final AI turn")],
...
describe("agent memory write-back stop-reason guard", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  for (const [stopReason, shouldWriteBack] of rows) {
    ...
    it(`stream() fallback ${verb} on stopReason="${stopReason}"`, async () => {
      const memory = makeMockMemoryService();
      const content = `fallback content from a run that stopped with ${stopReason}`;
      ...
      const agent = new DzupAgent(
        memoryAgentConfig(
          `fallback-wb-${stopReason}`,
          makeMockModel("unused"),
          memory
        )
      );

      for await (const _event of agent.stream([new HumanMessage("hi")])) {
        // drain
      }
  ...
  // -------------------------------------------------------------------------
  // Native streaming must derive partial content at a ceiling stop.
  // -------------------------------------------------------------------------

  it("keeps native streamed partial work on iteration_limit", async () => {
    const partial = "partial answer produced before the ceiling";
    const memory = makeMockMemoryService();
    const agent = new DzupAgent({
      ...memoryAgentConfig(
        "native-stream-iteration-limit",
        makeAlwaysToolCallingStreamingModel(partial),
        memory
      ),
      tools: [loopTool],
    });
    ...
    for await (const event of agent.stream([new HumanMessage("go")], {
      maxIterations: 2,
    })) {
      if (event.type === "done") {
        doneStopReason = event.data["stopReason"] as string | undefined;
      }
    }

    expect(doneStopReason).toBe("iteration_limit");
    expect(memory.put).toHaveBeenCalledTimes(1);
    expect(memory.put.mock.calls[0]![3]).toMatchObject({
      text: partial,
      agentId: "native-stream-iteration-limit",
    });
  });

  it("keeps native streamed partial work on budget_exceeded", async () => {
    const partial = "partial answer that consumed the token budget";
    const memory = makeMockMemoryService();
    const agent = new DzupAgent({
      ...memoryAgentConfig(
        "native-stream-budget-exceeded",
        makeAlwaysToolCallingStreamingModel(partial),
        memory
      ),
      tools: [loopTool],
      guardrails: { maxIterations: 5, maxTokens: 1 },
    });
    ...
    expect(doneStopReason).toBe("budget_exceeded");
    expect(memory.put).toHaveBeenCalledTimes(1);
    expect(memory.put.mock.calls[0]![3]).toMatchObject({
      text: partial,
      agentId: "native-stream-budget-exceeded",
    });
  });

  it("keeps native streamed partial work on token_exhausted", async () => {
    const partial = "partial answer produced before token exhaustion";
    const memory = makeMockMemoryService();
    const agent = new DzupAgent({
      ...memoryAgentConfig(
        "native-stream-token-exhausted",
        makeAlwaysToolCallingStreamingModel(partial),
        memory
      ),
      tools: [loopTool],
      tokenLifecyclePlugin: makeHaltingTokenPlugin(),
    });
    ...
    expect(doneStopReason).toBe("token_exhausted");
    expect(memory.put).toHaveBeenCalledTimes(1);
    expect(memory.put.mock.calls[0]![3]).toMatchObject({
      text: partial,
      agentId: "native-stream-token-exhausted",
    });
  });
});
