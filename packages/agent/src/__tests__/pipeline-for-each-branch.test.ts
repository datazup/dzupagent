...
describe("one normal for_each branch through public runtime", () => {
  ...
  it.each(["absent", "released", "invalid-evidence"] as const)("advances a denied retry with %s authority and no invented hold", async (status) => {
    const run = await releasedBranchAttempt("failed");
    const reserve = run.host.reserve, reconcile = run.host.reconcile;
    const attempted: number[] = [];
    run.host.reserve = async (input) => {
      attempted.push(input.attempt ?? 0);
      if (status === "invalid-evidence") {
        await reserve(input);
        return { status: "reserved", reservedCostCents: 10, evidence: {} as never };
      }
      throw new Error("no reserve acknowledgement");
    };
    run.host.reconcile = (input) => run.rows.has(input.reservationId)
      ? reconcile(input) : { status: status === "invalid-evidence" ? "released" : status };
    const store = new InMemoryPipelineCheckpointStore();
    await store.save(run.cp);
    const failed = await new PipelineRuntime({ definition: branchDefinition(), predicates,
      checkpointStore: store, nodeExecutor: run.plain, loopIterationBudgetReservation: run.host }).resume(run.cp);
    ...
    expect([...run.rows.values()]).toEqual(status === "invalid-evidence" ? ["released", "released"] : ["released"]);
    run.host.reserve = reserve;
    run.calls.length = 0;
    const resumed = await new PipelineRuntime({ definition: branchDefinition(), predicates: { choose: () => false },
      checkpointStore: store, nodeExecutor: run.plain, loopIterationBudgetReservation: run.host }).resume(denied);
    ...
    expect(run.reservations.map(({ attempt }) => attempt)).toEqual(status === "invalid-evidence" ? [0, 1, 2] : [0, 2]);
    expect(run.calls).toEqual(["yes:0", "after:0", "done:outer"]);
    expect([...run.rows.values()]).toEqual(status === "invalid-evidence" ? ["released", "released", "settled"] : ["released", "settled"]);
  });
