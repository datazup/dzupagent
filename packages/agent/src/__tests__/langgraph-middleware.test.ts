...
function makeConfig(
  store: MockStore,
  overrides?: Partial<LangGraphLearningConfig>,
...
describe("LangGraphLearningMiddleware", () => {
  ...
  describe("wrapNode", () => {
    ...
    it("applies enrichment to state when store has data", async () => {
      ...
      expect(calledState._learningContext as string).toContain(
    ...
    it("applies enrichment to systemPromptAddendum if present in state", async () => {
      ...
      const original = vi.fn(async (_state: Record<string, unknown>) => ({
        output: "done",
      }));
      ...
      await wrapped({
        input: "test",
        systemPromptAddendum: "existing context",
      });
      ...
      expect(calledState["systemPromptAddendum"] as string).toContain(
        "existing context",
      );
      expect(calledState["systemPromptAddendum"] as string).toContain(
        "Use ESM imports",
    ...
    it("records trajectory step on success", async () => {
      ...
      const trajectoryCalls = store.put.mock.calls.filter((call) => {
    ...
    it("records errors on failure and re-throws", async () => {
      ...
      await expect(wrapped({ input: "test" })).rejects.toThrow(
        "compilation failed",
      );
    });
  ...
  describe("metrics", () => {
    ...
    it("tracks totalDurationMs", async () => {
      ...
      expect(middleware.getMetrics().totalDurationMs).toBeGreaterThanOrEqual(
        15,
      );
    });
  ...
  describe("recommendFixStrategy", () => {
    it("returns a strategy recommendation", async () => {
      const result = await middleware.recommendFixStrategy(
        "type_error",
        "gen_backend",
      );
