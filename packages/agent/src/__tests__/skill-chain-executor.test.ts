...
import { ChainValidationError } from "../skill-chain-executor/errors.js";
...
class MockSkillStepResolver implements SkillStepResolver {
  ...
  async resolve(skillId: string): Promise<WorkflowStep> {
    ...
    if (!fn)
      throw new Error(
        `MockSkillStepResolver: skill "${skillId}" not registered`,
      );
    return {
      id: skillId,
...
describe("SkillChainExecutor", () => {
  ...
  describe("validation errors", () => {
    it("throws ChainValidationError when resolver cannot resolve a skill", async () => {
      ...
      await expect(executor.execute(chain, {})).rejects.toThrow(
  ...
  describe("per-step retry", () => {
    it("retries a failing step up to maxAttempts then throws StepExecutionError", async () => {
      ...
      const chain = createSkillChain("retry-chain", [
        {
          skillName: "flaky",
          retryPolicy: { maxAttempts: 3, initialBackoffMs: 1 },
        },
      ]);
    ...
    it("emits step:retrying events during retry", async () => {
      ...
      const first = retryEvents[0] as Extract<
        WorkflowEvent,
        { type: "step:retrying" }
      >;
    ...
    it("step-level retryPolicy overrides config defaultRetry", async () => {
      ...
      const chain = createSkillChain("override-retry", [
        {
          skillName: "override",
          retryPolicy: { maxAttempts: 2, initialBackoffMs: 1 },
        },
      ]);
  ...
  describe("event bus bridge", () => {
    it("emits pipeline:node_started when step starts", async () => {
      ...
      const started = bus.emitted.filter(
        (e) => e.type === "pipeline:node_started",
      );
      expect(started.length).toBeGreaterThanOrEqual(1);
      const first = started[0] as Extract<
        DzupEvent,
    ...
    it("emits pipeline:node_completed when step completes", async () => {
      ...
      const first = completed[0] as Extract<
        DzupEvent,
    ...
    it("emits pipeline:run_completed when chain completes", async () => {
      ...
      const runCompleted = bus.emitted.filter(
        (e) => e.type === "pipeline:run_completed",
      );
      expect(runCompleted).toHaveLength(1);
      const evt = runCompleted[0] as Extract<
        DzupEvent,
    ...
    it("emits pipeline:run_failed when chain fails", async () => {
      ...
      const runFailed = bus.emitted.filter(
        (e) => e.type === "pipeline:run_failed",
      );
      expect(runFailed).toHaveLength(1);
      const evt = runFailed[0] as Extract<
        DzupEvent,
    ...
    it("emits pipeline:node_skipped for condition-gated step", async () => {
      ...
      const skipped = bus.emitted.filter(
        (e) => e.type === "pipeline:node_skipped",
      );
      expect(skipped).toHaveLength(1);
      const evt = skipped[0] as Extract<
        DzupEvent,
    ...
    it("emits pipeline:node_retry during retry", async () => {
      ...
      const retryEvents = bus.emitted.filter(
        (e) => e.type === "pipeline:node_retry",
      );
      expect(retryEvents).toHaveLength(1);
      const evt = retryEvents[0] as Extract<
        DzupEvent,
