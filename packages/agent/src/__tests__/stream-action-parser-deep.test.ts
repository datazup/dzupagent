...
describe("StreamActionParser", () => {
  ...
  describe("non-streaming tool_calls", () => {
    ...
    it("does not re-execute already-fired tool call ids", async () => {
      let callCount = 0;
      const tool = mockTool("test", async () => {
        callCount++;
        return "ok";
      });
      const parser = new StreamActionParser([tool]);
      ...
      await parser.processChunk({
        tool_calls: [{ id: "dup", name: "test", args: {} }],
      });
      await parser.processChunk({
        tool_calls: [{ id: "dup", name: "test", args: {} }],
      });
    ...
    it("handles tool execution errors gracefully", async () => {
      const tool = mockTool("broken", async () => {
      ...
      const err = events.find((e) => e.type === "error");
      expect(err).toBeDefined();
      // ERR-L-07: client-safe message is bounded; raw detail on internalError.
      expect(err!.data.error).toBe("Tool execution failed");
      expect(err!.data.code).toBe("TOOL_EXECUTION_FAILED");
      expect(err!.data.internalError).toBe("tool broke");
    });
    ...
    it("handles non-Error tool execution errors", async () => {
      ...
      const err = events.find((e) => e.type === "error");
      // ERR-L-07: bounded client message; raw string detail on internalError.
      expect(err!.data.error).toBe("Tool execution failed");
      expect(err!.data.code).toBe("TOOL_EXECUTION_FAILED");
      expect(err!.data.internalError).toBe("string error");
    });
