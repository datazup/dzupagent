...
describe("decomposeGoal", () => {
  it('splits on commas, semicolons, newlines, and "and"', () => {
    expect(
      decomposeGoal("build api, write tests; update docs\nand deploy"),
    ).toEqual(["build api", "write tests", "update docs", "deploy"]);
  });
...
describe("scoreMatch", () => {
  it("boosts matches against metadata tags more than tool overlap", () => {
    ...
    expect(
      scoreMatch("write a database migration", "db-agent", def),
    ).toBeGreaterThan(0);
  });
  ...
  it("matches via the built-in keyword map when both subtask and specialist agree", () => {
    ...
    expect(
      scoreMatch("add rbac authorization", "security-agent", def),
    ).toBeGreaterThan(0);
  });
});
    ...
    expect(result).toEqual([
      {
        ...
        specialistId: "db",
        input: { subtask: "add database migration" },
      },
      {
        task: "expose REST endpoint",
        specialistId: "api",
        input: { subtask: "expose REST endpoint" },
      },
    ]);
  });
...
describe("toAgentSpecs", () => {
  it("projects specialists with metadata.tags into AgentSpec[]", () => {
    const specialists = new Map<string, AgentExecutionSpec>([
      [
        "db",
        spec({ id: "db", name: "DB", metadata: { tags: ["database", "sql"] } }),
      ],
    ]);
    ...
    expect(assignments).toEqual([
      {
        task: "migrate db",
        specialistId: "db",
        input: { subtask: "migrate db" },
      },
    ]);
    expect(events.some((e) => e.type === "supervisor:routing_decision")).toBe(
      true,
    );
  });
