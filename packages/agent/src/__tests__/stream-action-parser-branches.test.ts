...
describe("StreamActionParser — tool_calls (non-streaming) branches", () => {
  ...
  it("emits error event when tool invoke throws", async () => {
    ...
    const errEvents = events.filter((e) => e.type === "error");
    expect(errEvents).toHaveLength(1);
    // ERR-L-07: bounded client message + stable code; raw detail internal-only.
    expect(errEvents[0]?.data.error).toBe("Tool execution failed");
    expect(errEvents[0]?.data.code).toBe("TOOL_EXECUTION_FAILED");
    expect(errEvents[0]?.data.internalError).toBe("bang");
  });

  it("ERR-L-07: raw internal error detail never leaks onto the client-facing field", async () => {
    // A future consumer that forwards `data.error` blindly must be safe by
    // construction — the sensitive provider detail lives only on internalError.
    const secret = "DB connection failed: postgres://user:pw@host/db";
    const tool = makeTool("leaky", () => {
      throw new Error(secret);
    });
    const p = new StreamActionParser([tool]);
    const events = await p.processChunk({
      tool_calls: [{ id: "s1", name: "leaky", args: {} }],
    });
    const err = events.find((e) => e.type === "error");
    expect(err).toBeDefined();
    // The client-safe message is bounded and carries none of the raw detail.
    expect(err?.data.error).toBe("Tool execution failed");
    expect(err?.data.error).not.toContain("postgres");
    expect(err?.data.code).toBe("TOOL_EXECUTION_FAILED");
    // Raw detail is preserved for server-side logging only.
    expect(err?.data.internalError).toBe(secret);
  });

  it("emits error with stringified non-Error rejection value", async () => {
    const tool = makeTool("rej", () => {
      throw "plain-string";
    });
    const p = new StreamActionParser([tool]);
    const events = await p.processChunk({
    ...
    const err = events.find((e) => e.type === "error");
    // ERR-L-07: bounded client message; raw string rejection on internalError.
    expect(err?.data.error).toBe("Tool execution failed");
    expect(err?.data.code).toBe("TOOL_EXECUTION_FAILED");
    expect(err?.data.internalError).toBe("plain-string");
  });
