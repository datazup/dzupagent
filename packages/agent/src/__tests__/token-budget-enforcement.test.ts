...
describe("token budget enforcement — tracking", () => {
  it("records input tokens after an LLM call", () => {
    ...
    budget.recordUsage({ model: "test-model", inputTokens: 100, outputTokens: 0 });
  ...
  it("records output tokens after an LLM call", () => {
    const budget = makeBudget({ maxTokens: 10_000 });
    budget.recordUsage({ model: "test-model", inputTokens: 0, outputTokens: 50 });
    const state = budget.getState();
  ...
  it("increments llmCalls counter on each recordUsage call", () => {
    const budget = makeBudget({ maxTokens: 10_000 });
    budget.recordUsage({ model: "test-model", inputTokens: 10, outputTokens: 5 });
    budget.recordUsage({ model: "test-model", inputTokens: 20, outputTokens: 10 });
    expect(budget.getState().llmCalls).toBe(2);
  });

  it("tracks input and output tokens independently", () => {
    const budget = makeBudget({ maxTokens: 10_000 });
    budget.recordUsage({ model: "test-model", inputTokens: 300, outputTokens: 50 });
    const state = budget.getState();
...
describe("token budget enforcement — cumulative accumulation", () => {
  it("accumulates tokens across multiple LLM calls", () => {
    const budget = makeBudget({ maxTokens: 10_000 });
    budget.recordUsage({ model: "test-model", inputTokens: 200, outputTokens: 50 });
    budget.recordUsage({ model: "test-model", inputTokens: 300, outputTokens: 75 });
    budget.recordUsage({ model: "test-model", inputTokens: 100, outputTokens: 25 });
  ...
  it("total used = sum of all input + output across all calls", () => {
    const budget = makeBudget({ maxTokens: 10_000 });
    budget.recordUsage({ model: "test-model", inputTokens: 100, outputTokens: 10 });
    budget.recordUsage({ model: "test-model", inputTokens: 200, outputTokens: 20 });
  ...
  it("isExceeded reflects cumulative total not per-call total", () => {
    ...
    budget.recordUsage({ model: "test-model", inputTokens: 200, outputTokens: 100 }); // 300, not exceeded
    expect(budget.isExceeded().exceeded).toBe(false);
    budget.recordUsage({ model: "test-model", inputTokens: 150, outputTokens: 60 }); // 510, exceeded
...
describe("token budget enforcement — hard stop on exhaustion", () => {
  it("isExceeded returns false when under the token limit", () => {
    ...
    budget.recordUsage({ model: "test-model", inputTokens: 400, outputTokens: 100 });
    expect(budget.isExceeded().exceeded).toBe(false);
  });

  it("isExceeded returns true when token limit is exactly reached", () => {
    const budget = makeBudget({ maxTokens: 500 });
    budget.recordUsage({ model: "test-model", inputTokens: 300, outputTokens: 200 });
  ...
  it("isExceeded returns true when token limit is exceeded", () => {
    const budget = makeBudget({ maxTokens: 500 });
    budget.recordUsage({ model: "test-model", inputTokens: 400, outputTokens: 200 });
    expect(budget.isExceeded().exceeded).toBe(true);
  });
  ...
  it("reason includes tokens used and budget limit", () => {
    const budget = makeBudget({ maxTokens: 1000 });
    budget.recordUsage({ model: "test-model", inputTokens: 800, outputTokens: 300 });
    const result = budget.isExceeded();
  ...
  it("hard stop on cost limit exceeded", () => {
    ...
    for (let i = 0; i < 100; i++) {
      budget.recordUsage({ model: "test-model", inputTokens: 1000, outputTokens: 1000 });
    }
    expect(budget.isExceeded().exceeded).toBe(true);
  });
  ...
  it("isExceeded reason mentions cost when cost limit exceeded", () => {
    ...
    for (let i = 0; i < 100; i++) {
      budget.recordUsage({ model: "test-model", inputTokens: 10000, outputTokens: 10000 });
    }
    const result = budget.isExceeded();
    expect(result.exceeded).toBe(true);
...
describe("token budget enforcement — soft warning threshold", () => {
  it("emits a warning when usage crosses 70% of token budget", () => {
    ...
    const warnings = budget.recordUsage({ model: "test-model", inputTokens: 700, outputTokens: 0 });
  ...
  it("emits a warning when usage crosses 90% of token budget", () => {
    ...
    const warnings = budget.recordUsage({ model: "test-model", inputTokens: 900, outputTokens: 0 });
    expect(warnings.length).toBeGreaterThanOrEqual(1);
  ...
  it("uses default warning thresholds [0.7, 0.9] when not configured", () => {
    ...
    const w1 = budget.recordUsage({ model: "test-model", inputTokens: 700, outputTokens: 0 });
  ...
  it("emits warnings at multiple configurable thresholds", () => {
    ...
    const w1 = budget.recordUsage({ model: "test-model", inputTokens: 500, outputTokens: 0 });
    expect(w1.some((w) => w.threshold === 0.5)).toBe(true);

    const w2 = budget.recordUsage({ model: "test-model", inputTokens: 300, outputTokens: 0 });
  ...
  it("emits cost warning when cost crosses threshold", () => {
    ...
    for (let i = 0; i < 30; i++) {
      const ws = budget.recordUsage({ model: "test-model", inputTokens: 1000, outputTokens: 1000 });
      if (ws.length > 0) {
...
describe("token budget enforcement — warning payload", () => {
  ...
  it("warning includes the threshold that was crossed", () => {
    ...
    const warnings = budget.recordUsage({ model: "test-model", inputTokens: 750, outputTokens: 0 });
  ...
  it("warnings are stored in budget state", () => {
    const budget = makeBudget({ maxTokens: 1000, budgetWarnings: [0.7] });
    budget.recordUsage({ model: "test-model", inputTokens: 700, outputTokens: 0 });
    const state = budget.getState();
...
describe("token budget enforcement — warning deduplication", () => {
  it("does not emit the same threshold warning twice", () => {
    const budget = makeBudget({ maxTokens: 1000, budgetWarnings: [0.7] });
    const w1 = budget.recordUsage({ model: "test-model", inputTokens: 700, outputTokens: 0 });
    const w2 = budget.recordUsage({ model: "test-model", inputTokens: 10, outputTokens: 0 });
  ...
  it("emits 0.7 and 0.9 warnings exactly once each across many small steps", () => {
    ...
    for (let i = 0; i < 20; i++) {
      const ws = budget.recordUsage({ model: "test-model", inputTokens: 50, outputTokens: 0 });
      ws.forEach((w) => {
  ...
  it("state.warnings accumulates all unique warnings that fired", () => {
    const budget = makeBudget({ maxTokens: 1000 });
    for (let i = 0; i < 20; i++) {
      budget.recordUsage({ model: "test-model", inputTokens: 50, outputTokens: 0 });
    }
    const state = budget.getState();
...
describe("token budget enforcement — normal completion", () => {
  it("does not flag exceeded when well under token budget", () => {
    const budget = makeBudget({ maxTokens: 10_000 });
    budget.recordUsage({ model: "test-model", inputTokens: 500, outputTokens: 100 });
    budget.recordUsage({ model: "test-model", inputTokens: 300, outputTokens: 50 });
    expect(budget.isExceeded().exceeded).toBe(false);
  });
  ...
  it("produces no warnings when usage stays below all thresholds", () => {
    ...
    const w1 = budget.recordUsage({ model: "test-model", inputTokens: 100, outputTokens: 50 });
    const w2 = budget.recordUsage({ model: "test-model", inputTokens: 200, outputTokens: 50 });
...
describe("token budget enforcement — budget reporting", () => {
  it("getState returns total input tokens across all calls", () => {
    const budget = makeBudget({ maxTokens: 10_000 });
    budget.recordUsage({ model: "test-model", inputTokens: 100, outputTokens: 0 });
    budget.recordUsage({ model: "test-model", inputTokens: 200, outputTokens: 0 });
    budget.recordUsage({ model: "test-model", inputTokens: 300, outputTokens: 0 });
    expect(budget.getState().totalInputTokens).toBe(600);
  });

  it("getState returns total output tokens across all calls", () => {
    const budget = makeBudget({ maxTokens: 10_000 });
    budget.recordUsage({ model: "test-model", inputTokens: 0, outputTokens: 25 });
    budget.recordUsage({ model: "test-model", inputTokens: 0, outputTokens: 50 });
...
describe("token budget enforcement — input vs output tracking", () => {
  it("tracks input and output tokens separately across multiple calls", () => {
    ...
    budget.recordUsage({ model: "test-model", inputTokens: 100, outputTokens: 10 });
    budget.recordUsage({ model: "test-model", inputTokens: 200, outputTokens: 20 });
    budget.recordUsage({ model: "test-model", inputTokens: 300, outputTokens: 30 });
  ...
  it("combined input + output is used for the maxTokens hard stop", () => {
    ...
    budget.recordUsage({ model: "test-model", inputTokens: 60, outputTokens: 50 });
    expect(budget.isExceeded().exceeded).toBe(true);
  });
  ...
  it("combined input + output is used for budget warning thresholds", () => {
    ...
    const warnings = budget.recordUsage({ model: "test-model", inputTokens: 400, outputTokens: 300 });
    expect(warnings.some((w) => w.threshold === 0.7)).toBe(true);
  });

  it("zero input tokens with non-zero output still tracked", () => {
    const budget = makeBudget({ maxTokens: 10_000 });
    budget.recordUsage({ model: "test-model", inputTokens: 0, outputTokens: 500 });
    const state = budget.getState();
    expect(state.totalInputTokens).toBe(0);
  ...
  it("zero output tokens with non-zero input still tracked", () => {
    const budget = makeBudget({ maxTokens: 10_000 });
    budget.recordUsage({ model: "test-model", inputTokens: 500, outputTokens: 0 });
    const state = budget.getState();
...
describe("token budget enforcement — tool call iteration overhead", () => {
  ...
  it("dynamically blocked tool does not affect token budget state", () => {
    ...
    budget.recordUsage({ model: "test-model", inputTokens: 100, outputTokens: 50 });
...
describe("token budget enforcement — budget reset between runs", () => {
  it("a new IterationBudget instance starts fresh with zero state", () => {
    ...
    budget1.recordUsage({ model: "test-model", inputTokens: 500, outputTokens: 100 });
  ...
  it("forked child shares cumulative state by reference", () => {
    ...
    child.recordUsage({ model: "test-model", inputTokens: 600, outputTokens: 0 });
  ...
  it("forked child and parent share the same exceeded state", () => {
    ...
    child.recordUsage({ model: "test-model", inputTokens: 300, outputTokens: 0 });
    parent.recordUsage({ model: "test-model", inputTokens: 300, outputTokens: 0 });
  ...
  it("forked child has independent emitted-thresholds set", () => {
    ...
    const pWarnings = parent.recordUsage({ model: "test-model", inputTokens: 700, outputTokens: 0 });
    ...
    const cWarnings = child.recordUsage({ model: "test-model", inputTokens: 0, outputTokens: 0 });
...
describe("token budget enforcement — edge cases", () => {
  it("maxTokens of 0 is treated as no limit (falsy guard in isExceeded)", () => {
    ...
    budget.recordUsage({ model: "test-model", inputTokens: 1, outputTokens: 0 });
    expect(budget.isExceeded().exceeded).toBe(false);
  });
  ...
  it("no token limit means isExceeded never flags token excess", () => {
    ...
    const budget = makeBudget({});
    for (let i = 0; i < 1000; i++) {
      budget.recordUsage({ model: "test-model", inputTokens: 100_000, outputTokens: 50_000 });
    }
  ...
  it("very large token budget never triggered for moderate usage", () => {
    ...
    for (let i = 0; i < 100; i++) {
      budget.recordUsage({ model: "test-model", inputTokens: 10_000, outputTokens: 5_000 });
    }
    expect(budget.isExceeded().exceeded).toBe(false);
  });
  ...
  it("budget with all zero usage reports no excess", () => {
    const budget = makeBudget({ maxTokens: 1000 });
    budget.recordUsage({ model: "test-model", inputTokens: 0, outputTokens: 0 });
    expect(budget.isExceeded().exceeded).toBe(false);
  });

  it("budget with no warnings configured uses default 0.7 and 0.9", () => {
    const budget = makeBudget({ maxTokens: 1000 }); // no budgetWarnings
    const ws = budget.recordUsage({ model: "test-model", inputTokens: 700, outputTokens: 0 });
...
describe("token budget enforcement — configurable per-run budget", () => {
  it("different runs can have different maxTokens limits", () => {
    ...
    tight.recordUsage({ model: "test-model", inputTokens: 90, outputTokens: 20 });
    loose.recordUsage({ model: "test-model", inputTokens: 90, outputTokens: 20 });
  ...
  it("different runs have independent warning threshold sets", () => {
    ...
    const w1 = b1.recordUsage({ model: "test-model", inputTokens: 500, outputTokens: 0 });
    const w2 = b2.recordUsage({ model: "test-model", inputTokens: 500, outputTokens: 0 });
  ...
  it("warns at different thresholds for tokens and iterations independently", () => {
    ...
    const tw = budget.recordUsage({ model: "test-model", inputTokens: 800, outputTokens: 0 });
