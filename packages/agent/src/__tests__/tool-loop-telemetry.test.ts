...
describe("Tool Loop Telemetry", () => {
  it("populates toolStats in ToolLoopResult after execution", async () => {
    ...
    const model = createMockModel([
      ...
      aiWithToolCalls([{ name: "read_file", args: { path: "c.ts" } }]),
  ...
  it('returns stopReason "aborted" when signal is aborted', async () => {
    ...
    const result = await runToolLoop(model, [new HumanMessage("test")], [], {
      maxIterations: 10,
  ...
  it("counts tool errors in toolStats.errors", async () => {
    ...
    const result = await runToolLoop(model, [new HumanMessage("try")], [tool], {
      maxIterations: 10,
      onToolLatency: (name, durationMs, error) => {
        latencies.push({ name, durationMs, error });
      },
  ...
  it("fires onIteration after tool results are appended to message history", async () => {
    ...
    await runToolLoop(model, [new HumanMessage("read a file")], [tool], {
      maxIterations: 5,
      onIteration: (info) => {
        snapshots.push(info);
      },
  ...
  it("propagates LLM invocation errors to the caller", async () => {
    ...
    await expect(
      runToolLoop(model, [new HumanMessage("crash")], [], {
        maxIterations: 10,
      }),
