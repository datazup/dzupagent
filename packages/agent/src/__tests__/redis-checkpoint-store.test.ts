...
function makeInteractionState(runId = "run-1") {
  ...
  const definitionDigest = digestPipelineInteractionValue({ id: "pipeline-1" });
...
describe("RedisPipelineCheckpointStore", () => {
  ...
  it("round-trips pending and committed interaction checkpoints exactly", async () => {
    ...
    expect((await store.loadVersion("run-1", 1))?.pendingInteraction).toEqual(
      pending,
    );
  ...
  it("rejects malformed and schema-invalid stored interaction payloads", async () => {
    ...
    await expect(store.load("run-1")).rejects.toThrow(
      "Invalid pipeline checkpoint payload",
    );
  });
  ...
  it("load() returns the highest version", async () => {
    ...
    await store.save(
      makeCheckpoint({ version: 3, completedNodeIds: ["a", "b", "c"] }),
    );
    await store.save(
      makeCheckpoint({ version: 2, completedNodeIds: ["a", "b"] }),
    );
  ...
  it("listVersions() returns all versions sorted ascending", async () => {
    await store.save(
      makeCheckpoint({ version: 3, createdAt: "2026-04-24T00:00:03Z" }),
    );
    await store.save(
      makeCheckpoint({ version: 1, createdAt: "2026-04-24T00:00:01Z" }),
    );
    await store.save(
      makeCheckpoint({ version: 2, createdAt: "2026-04-24T00:00:02Z" }),
    );
  ...
  it("prune() drops entries older than the threshold", async () => {
    ...
    await store.save(
      makeCheckpoint({ pipelineRunId: "old-run", version: 1, createdAt: old }),
    );
    await store.save(
      makeCheckpoint({ pipelineRunId: "old-run", version: 2, createdAt: old }),
    );
    await store.save(
      makeCheckpoint({
  ...
  it("keyPrefix option namespaces all keys", async () => {
    const scoped = new RedisPipelineCheckpointStore({
      client,
      keyPrefix: "tenant-a:cp",
    });
    await scoped.save(makeCheckpoint({ version: 1 }));

    expect([...client.strings.keys()]).toContain("tenant-a:cp:run-1:1");
    expect([...client.sortedSets.keys()]).toContain(
      "tenant-a:cp:run-1:versions",
    );
  ...
  describe("saveIfVersion (CAS)", () => {
    it("commits when the observed version matches and indexes the version", async () => {
      ...
      expect(await client.zrange("checkpoint:run-1:versions", 0, -1)).toEqual([
        "1",
      ]);
    ...
    it("rejects a stale expected version without writing", async () => {
      await store.saveIfVersion(
        makeCheckpoint({ version: 1, state: { by: "first" } }),
        0,
      );
    ...
    it("SET NX is the arbiter: a key claimed between the read and the write loses", async () => {
      ...
      client.strings.set(
        "checkpoint:run-1:1",
        JSON.stringify(makeCheckpoint({ version: 1, state: { by: "winner" } })),
      );
      ...
      expect(
        JSON.parse(client.strings.get("checkpoint:run-1:1")!).state,
      ).toEqual({ by: "winner" });
    });
