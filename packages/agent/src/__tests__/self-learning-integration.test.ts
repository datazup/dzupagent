...
function createMemoryStore(): BaseStore {
  const data = new Map<
    string,
    Map<string, { key: string; value: Record<string, unknown> }>
  >();
...
async function mockGenerateNode(
  _state: TestState,
...
async function mockValidateNode(
  _state: TestState,
): Promise<Partial<TestState>> {
...
describe("Self-Learning Integration", () => {
  ...
  describe("single run learning", () => {
    it("records trajectory steps for each wrapped node", async () => {
      ...
      await runPipeline(middleware, "run-1", standardPipeline(), {
        input: "auth feature",
      });
    ...
    it("error detection captures node failures", async () => {
      ...
      // Generate node fails
      const wrappedGenerate = middleware.wrapNode<TestState>(
        "generate",
        mockFailingNode,
      );
      await expect(wrappedGenerate(state)).rejects.toThrow(
  ...
  describe("cross-run learning", () => {
    ...
    it("quality scores improve across sequential runs (simulated)", async () => {
      ...
      await runPipeline(mw2, "run-2", standardPipeline(), {
        input: "feature v2",
      });
  ...
  describe("feedback integration", () => {
    it("FeedbackCollector stores plan rejection feedback", async () => {
      ...
      const record = await collector.recordPlanFeedback({
        ...
        approved: false,
        feedback:
          "The plan should include authentication. Missing error handling.",
    ...
    it("FeedbackCollector extracts action items from feedback text", async () => {
      ...
      const record = await collector.recordPublishFeedback({
        ...
        approved: false,
        feedback:
          "The code should use TypeScript strict mode. Need to add input validation. The UI looks fine.",
      });
      ...
      expect(
        record.actionItems.some((item) =>
          item.toLowerCase().includes("should"),
        ),
      ).toBe(true);
      expect(
        record.actionItems.some((item) =>
          item.toLowerCase().includes("need to"),
        ),
      ).toBe(true);
    });
    ...
    it("feedback-derived rules appear in enrichment on next run", async () => {
      ...
      const record = await collector.recordPublishFeedback({
        ...
        approved: false,
        feedback:
          "Must add error handling for all API endpoints. Should validate request bodies.",
        featureCategory: "crud",
        riskClass: "standard",
      });
      ...
      expect(
        enrichment.includes("error handling") ||
          enrichment.includes("validate"),
      ).toBe(true);
    });
  ...
  describe("tenant isolation", () => {
    it("two tenants cannot see each other's learned data", async () => {
      ...
      await runPipeline(mwA, "run-a1", standardPipeline(), {
        input: "feature A",
      });
      ...
      await runPipeline(mwB, "run-b1", standardPipeline(), {
        input: "feature B",
      });
  ...
  describe("dashboard", () => {
    it("returns accurate counts after runs", async () => {
      ...
      expect(overview.lessonCount + overview.ruleCount).toBeGreaterThanOrEqual(
        0,
      );
  ...
  describe("error resilience", () => {
    ...
    it("wrapped node re-throws original errors without modification", async () => {
      ...
      const wrapped = middleware.wrapNode<TestState>(
        "failing",
        mockFailingNode,
      );
    ...
    it("multiple failing nodes do not corrupt middleware state", async () => {
      ...
      const wrappedFail1 = middleware.wrapNode<TestState>(
        "fail1",
        mockFailingNode,
      );
      const wrappedFail2 = middleware.wrapNode<TestState>(
        "fail2",
        mockFailingNode,
      );
  ...
  describe("full learning loop", () => {
    it("run 1 generates artifacts, run 2 consumes them via enrichment", async () => {
      ...
      await runPipeline(mw1, "run-1", standardPipeline(), {
        input: "crud feature",
      });
    ...
    it("feedback between runs influences enrichment", async () => {
      ...
      const fbRecord = await collector.recordPublishFeedback({
        ...
        approved: false,
        feedback:
          "Must include retry logic for external API calls. Should add rate limiting.",
      });
