...
function createTestBus(): DzupEventBus {
  return createEventBus();
}

function emitSequence(bus: DzupEventBus, events: DzupEvent[]): void {
  for (const event of events) {
    bus.emit(event);
  }
}

function makeSampleEvents(): DzupEvent[] {
  return [
...
function makeSampleTrace(): CapturedTrace {
  const events: ReplayEvent[] = [
    ...
    },
    {
      ...
      type: "tool:result",
      nodeId: "read_file",
      data: { toolName: "read_file", durationMs: 50 },
    },
    {
      index: 3,
      timestamp: 1300,
      type: "tool:called",
      nodeId: "write_file",
      data: { toolName: "write_file", input: {} },
    },
    {
      index: 4,
      timestamp: 1400,
      type: "tool:result",
      nodeId: "write_file",
      data: { toolName: "write_file", durationMs: 20 },
    },
    {
      index: 5,
      timestamp: 1500,
      type: "tool:error",
      nodeId: "lint",
      data: { toolName: "lint", error: "Lint failed" },
    },
    {
      index: 6,
      timestamp: 1600,
    ...
    },
    {
      ...
      type: "tool:result",
      nodeId: "lint",
      data: { toolName: "lint", durationMs: 100 },
    },
    {
      index: 8,
      timestamp: 1800,
      type: "agent:completed",
      data: { agentId: "a1", runId: "r1", durationMs: 800 },
      stateSnapshot: { step: 8, done: true },
    },
  ];
...
describe("TraceSerializer", () => {
  ...
  it("sanitize also redacts state snapshots", () => {
    ...
    expect(sanitized.events[0]!.stateSnapshot!["secretToken"]).toBe(
      "[REDACTED]",
    );
  ...
  it("rejects wrong schema version", () => {
    ...
    expect(() => serializer.deserialize(wrongVersion, "json")).toThrow(
      "schema version",
    );
  });
  ...
  it("rejects binary with bad magic bytes", () => {
    ...
    expect(() => serializer.deserialize(badMagic, "binary")).toThrow(
      "magic bytes",
    );
  });
});
