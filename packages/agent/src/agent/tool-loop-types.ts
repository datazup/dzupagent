/**
 * Public type surface of the ReAct tool-calling loop.
 *
 * Extracted from `tool-loop.ts` so the implementation file can stay focused
 * on the orchestration logic. All names below are re-exported from
 * `tool-loop.ts` to preserve the legacy import paths.
 */
