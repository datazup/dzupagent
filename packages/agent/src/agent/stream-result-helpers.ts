...
/**
 * MC-3 (AGENT-H-06 / DZUPAGENT-AGENT-C-22) — wrap a successful tool result's
 * CONTEXT content in an `<untrusted_content source="tool_result">` delimiter
 * before it enters the model's message history. Returns the raw result
 * unchanged when wrapping is disabled. The emitted `tool_result` event payload
 * (`eventResult`) keeps the raw output — only the ToolMessage content is
 * wrapped.
 *
 * Delegates to the shared `fenceToolResult` primitive so the streaming path
 * stays byte-identical to the generate() path (stream/generate parity,
 * MJ-AGENT-02) without a second copy of the fencing logic.
 */
function wrapToolResultContent(
  result: string,
  guard: ToolResultGuardLike | undefined,
  wrapToolResults: boolean | undefined
): string {
  return fenceToolResult(result, {
    ...(guard ? { guard } : {}),
    ...(wrapToolResults === false ? { enabled: false as const } : {}),
  });
}
