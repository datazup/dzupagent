...
3. Non-streaming run (`generate` -> `executeGenerateRun`): `setupModelCall` builds `ToolLoopConfig` with policy/telemetry/token-lifecycle wiring, `runToolLoop` executes model+tool turns, `processGeneratedRun` emits terminal telemetry and post-filters output, and only `complete` runs write back memory.
