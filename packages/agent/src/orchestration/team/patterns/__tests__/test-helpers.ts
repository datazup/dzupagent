...
export function buildContext(
  ...
  options?: {
    ...
    teamId?: string;
    runId?: string;
    /** Runtime plumbing — defaults to undefined, matching a runtime with none. */
    signal?: AbortSignal;
  ...
  const recorder = options?.hooks
    ? {
        hooks: options.hooks,
        calls: { starts: [], completes: [], policyApplied: [] },
      }
    : makeHooksRecorder();
  ...
  const ctx: TeamPatternContext = {
    ...
    contractNet: options?.contractNet,
