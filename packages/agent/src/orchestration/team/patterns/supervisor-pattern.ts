...
export const supervisorPattern: TeamPattern = {
  ...
  async execute(ctx: TeamPatternContext): Promise<TeamPatternResult> {
    ...
    try {
      ...
      ctx.hooks.emitParticipantComplete(managerEntry.participant, true, durationMs)
      ...
      const durationMs = Date.now() - startTime
      ctx.hooks.emitParticipantComplete(managerEntry.participant, false, durationMs, message)
      for (const s of specialists) {
