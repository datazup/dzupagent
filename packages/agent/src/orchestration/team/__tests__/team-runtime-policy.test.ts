...
describe("TeamRuntime execution policy", () => {
  ...
  it("accepts supported timeout and retry policy fields at construction", () => {
    const definition = buildDefinition("supported-policy", [
      { id: "p1", role: "worker" },
    ]);
  ...
  it("rejects malformed or misplaced timeout and retry policy fields", () => {
    const definition = buildDefinition("malformed-policy", [
      { id: "p1", role: "worker" },
    ]);
    ...
    const supervisorDefinition: TeamDefinition = {
      ...buildDefinition("retry-outside-peer", [{ id: "p1", role: "worker" }]),
...
describe("TeamRuntime blackboard memory policy", () => {
  ...
  it("accepts consolidateOnComplete policy without throwing", () => {
    const blackboardDefinition =
      buildBlackboardDefinition("memory-consolidate");
