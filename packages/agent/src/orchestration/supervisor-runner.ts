...
export async function runSupervisor(
  ...
  if (circuitBreaker) {
    ...
    if (specialists.length < before) {
      const removedIds = config.specialists
        ...
        .map((s) => s.id);
      const cbRejectionReasons: Record<string, string> = {};
      ...
      if (!eventBus) {
        defaultLogger.debug(
          "[AgentOrchestrator] Circuit breaker filtered agents",
          { removedIds }
        );
      }
    }
  ...
  if (routingPolicy) {
    ...
    const filteredSpecialists = candidateSpecialists.filter(
    ...
    const routingEvent = omitUndefined({
      ...
      selectedSpecialists,
      selectedCandidates:
        decision.diagnostics?.selectedIds ?? selectedSpecialists,
      filteredSpecialists,
      candidateSpecialists,
      source: "direct-supervisor",
      routingDecisionId: decision.routingDecisionId,
      rejectionReasons: decision.diagnostics?.rejectionReasons,
    } as const);
  ...
  const canonicalSpecialists = [...specialists].sort((a, b) =>
  ...
  if (!managerWithTools) {
    ...
    if (cacheKey) {
      const cache =
        ...
        new Map<
          string,
          { agent: DzupAgent; specialists: readonly DzupAgent[] }
        >();
      cache.set(cacheKey, {
        agent: managerWithTools,
        specialists: canonicalSpecialists,
      });
      if (!managerCache) {
