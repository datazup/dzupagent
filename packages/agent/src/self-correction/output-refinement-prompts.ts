...
import {
  estimateCostCents as estimateCostCentsSplit,
...
export const DOMAIN_CRITIQUE_PROMPTS: Readonly<
  Record<RefinementDomain, string>
> = {
...
export function estimateTokens(chars: number): number {
  return estimateTokensShared(chars);
}

/**
 * Estimate cost in cents for a given number of tokens.
 *
 * This call site historically had only a single blended token total (no
 * input/output split), so it routes the whole total through the input side of
 * the shared split-pricing helper (CODE-M-10). This preserves its prior
 * single-bucket behavior while unifying on one pricing source of truth. See
 * ./cost.ts for the reconciliation rationale.
 */
export function estimateCostCents(totalTokens: number): number {
  return estimateCostCentsSplit(totalTokens, 0);
}
...
export function parseCritiqueResponse(response: string): {
  ...
  return {
    score: 0.5,
    feedback: response.trim() || "No specific feedback provided.",
  };
}
 ...
 */
export function detectRefinementDomain(
  task: string,
  output: string
): RefinementDomain {
  const combined = `${task}\n${output}`.toUpperCase();
