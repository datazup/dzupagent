...
import {
  estimateCostCents,
  estimateTokens as estimateTokensFromChars,
} from "./cost.js";
...
export function parseCriticResponse(response: string): ScoreResult {
  ...
  const feedback = feedbackMatch
    ...
    : response.replace(/SCORE:\s*\d+(?:\.\d+)?/i, "").trim(); // eslint-disable-line security/detect-unsafe-regex -- fixed literal stripping score prefix from LLM critic output
...
export class ReflectionLoop {
  ...
  async execute(
    ...
    if (!currentDraft) {
      try {
        ...
        const response = await this.drafter.invoke([new HumanMessage(task)]);
        currentDraft =
    ...
    for (let i = 0; i < this.config.maxIterations; i++) {
      ...
      if (!scoreFn) {
        const criticInputChars =
          task.length +
          currentDraft.length +
          (this.config.criticPrompt ?? DEFAULT_CRITIC_PROMPT).length;
        accumulatedCostCents += estimateCostCents(
      ...
      try {
        ...
        const revisionInputChars =
          task.length +
          currentDraft.length +
          scoreResult.feedback.length +
          REVISION_SYSTEM_PROMPT.length;
        accumulatedCostCents += estimateCostCents(
  ...
  private async scoreDraft(task: string, draft: string): Promise<ScoreResult> {
    const criticSystemPrompt =
      this.config.criticPrompt ?? DEFAULT_CRITIC_PROMPT;
