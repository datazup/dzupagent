...
import type { PipelineExecutorCoordinator } from "../pipeline-executor.js";
import { PipelineExecutor } from "../pipeline-executor.js";
import {
  findAdmittedRecursiveForkGraph,
  type AdmittedForkBranchGraph,
  type AdmittedRecursiveForkGraph,
...
import type { RunFrame } from "./stage-dispatch.js";

export interface RecursiveForkRuntimeDeps {
  readonly config: PipelineRuntimeConfig;
  readonly coordinator: PipelineExecutorCoordinator;
  readonly Executor: typeof PipelineExecutor;
}
...
function checkpointFrameFromJson(
...
function checkpointDefinitionFor(
  config: PipelineRuntimeConfig,
            ...
            const result = await executeScopedGraph(
              ...
              outerFrame,
              {
                ...
                onCheckpoint: async (checkpoint) => {
                  ...
                  await persistCheckpoint(checkpointFrameToJson(checkpoint));
                },
              },
              SCOPED_FRAME_CODEC
            );
            ...
            return {
              ...
              commit: {
                ...
                results: toJsonObject(
                  Object.fromEntries(result.nodeResults),
                  "results"
                ),
