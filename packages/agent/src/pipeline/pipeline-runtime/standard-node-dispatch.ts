...
export async function dispatchStandardNode(
  ...
  const {
    config, outgoingEdges, errorEdges, emit, recoveryCounter, budgetTracker,
    setState, pipelineId, node, runId, runState, nodeResults, completedNodeIds, startTime,
    saveCheckpoint,
  } = input
  ...
  try {
    ...
    if (finalResult.error) {
      ...
      const stuckAbort = recordFailureInStuckDetector(config, emit, node.id, finalResult.error, context)
      if (stuckAbort) return fail(stuckAbort)

      const errorNext = getErrorTarget(node.id, errorEdges, extractErrorCode(finalResult.error))
      if (errorNext) return { kind: 'continue', nextNodeId: errorNext }

      const recovered = await attemptRecovery(
        config, emit, recoveryCounter,
        node.id, node.type, finalResult.error, runId,
      )
      if (recovered) {
    ...
    const stuckAbort = recordSuccessInStuckDetector(config, emit, node.id, finalResult, context)
    if (stuckAbort) return fail(stuckAbort)

    await recordCalibration(config, emit, node.id, finalResult, runId)
    recordIterationBudget(config, emit, budgetTracker, node.id, finalResult, completedNodeIds.length)

    completedNodeIds.push(node.id)
    await saveCheckpoint()

    const nextIds = getNextNodeIds(node.id, outgoingEdges, config.predicates, runState)
    ...
    const errorNext = getErrorTarget(node.id, errorEdges, extractErrorCode(err))
    if (errorNext) return { kind: 'continue', nextNodeId: errorNext }

    const recovered = await attemptRecovery(
      config, emit, recoveryCounter,
      node.id, node.type, errorMessage, runId,
    )
    if (recovered) {
      nodeResults.delete(node.id)
      return { kind: 'continue', nextNodeId: node.id }
