...
export class PostgresEffectJournalStore<
  T extends EffectJsonValue = EffectJsonValue,
> implements EffectJournalStore<T> {
  ...
  async #requiredExisting(
    intent: EffectIntent,
  ): Promise<EffectJournalRecord<T>> {
    const existing = await this.#load(intent.idempotencyKey);
    if (existing === undefined) {
      throw new PostgresEffectJournalError(
        "record-missing",
        "Effect journal record is missing.",
      );
    }
  ...
  ): Promise<EffectJournalRecord<T> | undefined> {
...
function rowToRecord<T extends EffectJsonValue>(
  row: EffectJournalRow,
): EffectJournalRecord<T> {
  ...
  throw new PostgresEffectJournalError(
    "record-corrupt",
    "Stored effect status is unsupported.",
  );
}

function parseJson(value: unknown): unknown {
  try {
    // Non-string values arrive pre-parsed from the pg driver (jsonb columns).
    // Clone with structuredClone rather than a JSON round-trip: the old
    // JSON.parse(JSON.stringify(...)) silently dropped explicit `undefined`
    // properties and coerced Date instances to strings, letting a rewritten
    // record slip past receipt custody. structuredClone preserves such values
    // so custody validation rejects them as corrupt.
    ...
    throw new PostgresEffectJournalError(
      "record-corrupt",
      "Stored effect JSON is unreadable.",
    );
  }
}
...
function assertIntent(value: unknown): asserts value is EffectIntent {
  if (!validateEffectIntent(value).valid) {
    throw new PostgresEffectJournalError(
      "invalid-input",
      "Effect intent failed canonical custody.",
    );
  }
}
...
function assertReceipt<T extends EffectJsonValue>(
  ...
  if (!validateEffectReceipt(value, intent).valid) {
    throw new PostgresEffectJournalError(
      "record-corrupt",
      "Effect receipt failed canonical custody.",
    );
  }
}
