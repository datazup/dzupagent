...
export class PipelineAnalytics {
  ...
  getReport(pipelineId: string): PipelineAnalyticsReport {
    ...
    const successRate =
      acc.totalRuns > 0 ? acc.successCount / acc.totalRuns : 0;
    ...
    for (const nodeAcc of acc.nodes.values()) {
      ...
      const minDur =
        nodeAcc.durations.length > 0 ? Math.min(...nodeAcc.durations) : 0;
      const maxDur =
        nodeAcc.durations.length > 0 ? Math.max(...nodeAcc.durations) : 0;
  ...
  private computeBottlenecks(nodeMetrics: NodeMetrics[]): BottleneckEntry[] {
    ...
    const withCost = nodeMetrics.filter(
      (n) => n.totalCostCents !== undefined && n.totalCostCents > 0,
    );
    if (withCost.length > 0) {
      const expensive = withCost.sort(
        (a, b) => (b.totalCostCents ?? 0) - (a.totalCostCents ?? 0),
      )[0];
      if (expensive) {
        bottlenecks.push({
    ...
    if (withEnoughRuns.length > 0) {
      const highestFailure = withEnoughRuns.sort(
