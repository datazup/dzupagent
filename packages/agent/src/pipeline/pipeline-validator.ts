...
export function validatePipeline(definition: PipelineDefinition): PipelineValidationResult {
  ...
  const forkIds = new Set([
    ...forkNodes.map(fork => fork.forkId),
    ...joinNodes.map(join => join.forkId),
  ])
  for (const forkId of forkIds) {
    const matchingForks = forkNodes.filter(fork => fork.forkId === forkId)
    const matchingJoins = joinNodes.filter(join => join.forkId === forkId)
    if (matchingForks.length > 1 || matchingJoins.length > 1) {
      const owner = matchingForks[1] ?? matchingJoins[1] ?? matchingForks[0] ?? matchingJoins[0]
      errors.push({
        code: 'UNBALANCED_FORK_JOIN',
        message:
          `forkId "${forkId}" must identify exactly one ForkNode and one JoinNode; ` +
          `found ${matchingForks.length} fork(s) and ${matchingJoins.length} join(s)`,
        ...(owner === undefined ? {} : { nodeId: owner.id }),
      })
    }
  }

  // --- Fork branches are leaf-only until a durable recursive frame exists ---
  for (const fork of forkNodes) {
    const matchingForks = forkNodes.filter(candidate => candidate.forkId === fork.forkId)
    const matchingJoins = joinNodes.filter(candidate => candidate.forkId === fork.forkId)
    const join = matchingJoins[0]
    if (matchingForks.length !== 1 || matchingJoins.length !== 1 || join === undefined) continue
    const unsupported = findUnsupportedForkBranchShape(
      fork.id,
      join.id,
      nodeMap,
      definition.edges,
