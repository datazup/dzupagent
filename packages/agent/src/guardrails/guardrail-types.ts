...
export interface GuardrailConfig {
  ...
  /**
   * Wall-clock ceiling for a whole run, in milliseconds (ORCH-DSL-L1-H-02).
   *
   * Iteration, token and cost caps only advance when a call *returns*, so a
   * provider that accepts the connection and then stalls is never caught by
   * them. This bound is time-based and therefore fires regardless of progress.
   *
   * When set, the run's `AbortSignal` is derived from a `CascadingTimeout`, so
   * child agents inherit the remaining budget and a parent abort cascades down.
   */
  maxDurationMs?: number;
  /**
   * Per-call deadline for a single model invocation, in milliseconds
   * (ORCH-DSL-L1-C-01).
   *
   * Mirrors the per-tool deadline discipline in `toolTimeouts`. Without it the
   * model call is the one unbounded await in the hot loop.
   */
