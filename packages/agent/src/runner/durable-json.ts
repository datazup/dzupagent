...
function assertDurableValue(value: unknown, location: string, ancestors: Set<object>): void {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) {
      throw new AgentRunnerSerializationError('non-finite-number', location)
    }
    return
  }

  if (typeof value !== 'object') {
    throw new AgentRunnerSerializationError('unsupported-value', location)
  }

  if (ancestors.has(value)) {
    throw new AgentRunnerSerializationError('cycle', location)
  }
