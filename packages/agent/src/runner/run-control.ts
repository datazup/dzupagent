...
import {
  ...
  type AiExecutionRequest, type AiResolvedTargetSnapshot,
} from '@dzupagent/runtime-contracts/ai-execution'
import type { ExecutionRouteDecision } from '@dzupagent/runtime-contracts'
import type { AgentPendingInteraction } from '@dzupagent/agent-types/run'
import type { AgentRunnerResult, InMemoryAgentRunner } from './in-memory-agent-runner.js'
...
export class RunControl {
  ...
  observeAtSafePoint(safePoint: AgentRunnerSafePoint): RunControlSafePointDecision {
    ...
    const waiter = this.#observations.get(request.requestId)
    waiter?.resolve({
...
class AgentRunnerInlineHandle implements InlineAiExecutionHandle {
  ...
  readonly #runner: InMemoryAgentRunner
  readonly #now: () => string
