...
import { generateSessionId } from "./replay-engine.js";
...
export class ReplayController {
  ...
  private readonly engine: ReplayEngine | undefined;

  constructor(session: ReplaySession, engine?: ReplayEngine) {
  ...
  onBreakpointHit(callback: BreakpointHitCallback): () => void {
    ...
    return () => {
      this.breakpointCallbacks = this.breakpointCallbacks.filter(
        (cb) => cb !== callback
      );
    };
  }
  ...
  removeBreakpoint(breakpointId: string): boolean {
    const idx = this.session.breakpoints.findIndex(
  ...
  /**
   * Fork this controller's bound session at a given event index, producing
   * a brand-new, independently-steppable `ReplaySession` truncated to
   * `[0, atIndex]` and registered with the engine this controller was
   * constructed with.
   *
   * The fork is a deep-enough copy that subsequent mutations (seeking,
   * breakpoints) on either the original or the fork do not affect the
   * other: `events` and `breakpoints` are copied into new arrays.
   *
   * Requires the controller to have been constructed with a `ReplayEngine`
   * reference (`new ReplayController(session, engine)`); throws
   * `ReplaySessionNotFoundError` if no engine was supplied or the engine's
   * registry no longer contains this controller's session.
   *
   * @throws {ReplaySessionNotFoundError} if the engine is missing or the
   *   bound session is not (or no longer) registered in it.
   * @throws {ReplayIndexOutOfBoundsError} if `atIndex` is outside
   *   `[0, events.length - 1]`.
   */
  fork(atIndex: number): ReplaySession {
    if (!this.engine || !this.engine.getSession(this.session.id)) {
      throw new ReplaySessionNotFoundError(this.session.id);
    }
    if (atIndex < 0 || atIndex >= this.session.events.length) {
      throw new ReplayIndexOutOfBoundsError(
        atIndex,
        this.session.events.length
      );
    }

    const forked: ReplaySession = {
      id: generateSessionId(),
      runId: this.session.runId,
      events: this.session.events.slice(0, atIndex + 1),
      currentIndex: atIndex,
      status: "paused",
      breakpoints: [...this.session.breakpoints],
      speed: this.session.speed,
    };

    this.engine.registerSession(forked);
    return forked;
  }
