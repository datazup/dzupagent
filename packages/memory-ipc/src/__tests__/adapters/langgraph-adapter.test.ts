...
import {
  LangGraphAdapter,
  type LangGraphStoreItem,
} from "../../adapters/langgraph-adapter.js";
...
function makeStoreItem(
  overrides?: Partial<LangGraphStoreItem>
): LangGraphStoreItem {
  return {
    key: "item-1",
...
describe("LangGraphAdapter", () => {
  ...
  describe("canAdapt", () => {
    ...
    it("rejects missing key", () => {
      expect(
        adapter.canAdapt({ namespace: [], value: {}, createdAt: new Date() })
      ).toBe(false);
    });
    ...
    it("rejects missing value", () => {
      expect(
        adapter.canAdapt({ key: "k", namespace: [], createdAt: new Date() })
      ).toBe(false);
    });
  ...
  describe("validate", () => {
    ...
    it("warns when namespace tuple has fewer than 2 elements", () => {
      ...
      expect(at(result.warnings, 0).field).toBe("namespace");
    });

    it("reports shape mismatch for invalid records", () => {
      const result = adapter.validate([42]);
      expect(result.invalid).toBe(1);
      expect(at(result.warnings, 0).field).toBe("*");
    });
  ...
  describe("toFrame", () => {
    it("converts a single store item to an Arrow table with correct columns", () => {
      ...
      expect(table.getChild("text")?.get(0)).toBe(
    ...
    it("extracts category from value", () => {
      const item = makeStoreItem({
        value: { text: "test", category: "architecture" },
      });
      const table = adapter.toFrame([item]);
    ...
    it("stores remaining value fields in payload_json", () => {
      ...
      const payload = JSON.parse(
        table.getChild("payload_json")?.get(0) as string
      );
    ...
    it("maps createdAt to system_created_at as bigint", () => {
      ...
      const table = adapter.toFrame([item]);
      expect(table.getChild("system_created_at")?.get(0)).toBe(
        BigInt(date.getTime())
      );
    });
  ...
  describe("fromFrame", () => {
    it("converts frame back to store items", () => {
      ...
      expect(at(result, 0).key).toBe("item-1");
      expect(at(result, 0).namespace).toEqual([
        "tenant-a",
        "decisions",
        "project-x",
      ]);
      expect(at(result, 0).value["text"]).toBe(
        "Use PostgreSQL for the database"
      );
    });
    ...
    it("reconstructs namespace tuple from scope fields", () => {
      ...
      const table = adapter.toFrame([item]);
      const result = adapter.fromFrame(table);
      expect(at(result, 0).namespace).toEqual(["t", "ns", "p"]);
    });
    ...
    it("merges payload_json back into value", () => {
      ...
      const table = adapter.toFrame([item]);
      const result = adapter.fromFrame(table);
      expect(at(result, 0).value["text"]).toBe("test");
      expect(at(result, 0).value["extra"]).toBe("data");
      expect(at(result, 0).value["count"]).toBe(42);
    });
    ...
    it("sets createdAt and updatedAt from system_created_at", () => {
      ...
      const table = adapter.toFrame([item]);
      const result = adapter.fromFrame(table);
      expect(at(result, 0).createdAt.getTime()).toBe(date.getTime());
      expect(at(result, 0).updatedAt.getTime()).toBe(date.getTime());
    });
  ...
  describe("round-trip", () => {
    it("preserves data through toFrame -> fromFrame", () => {
      ...
      expect((r.value["customMeta"] as Record<string, unknown>)["source"]).toBe(
        "review"
      );
      expect(r.createdAt.getTime()).toBe(
    ...
    it("preserves multiple items", () => {
      ...
      for (let i = 0; i < 20; i++) {
        expect(at(restored, i).key).toBe(`item-${i}`);
        expect(at(restored, i).value["text"]).toBe(`Record number ${i}`);
        expect(at(restored, i).value["index"]).toBe(i);
      }
    });
  });
});
