...
function createMockStore(initialData?: {
  ...
  getKeyedMock: ReturnType<typeof vi.fn>;
  ...
  const getMock = vi
    ...
    .mockImplementation(
      (ns: string, _scope: Record<string, string>, key?: string) => {
        ...
        if (key)
          return Promise.resolve(records.filter((r) => r["key"] === key));
        return Promise.resolve(records);
      }
    );
  // `getKeyed` is required: a record's store key is not recoverable from its
  // value, so `SessionSearch` indexes through the keyed read only.
  const getKeyedMock = vi.fn().mockImplementation((ns: string) =>
    Promise.resolve(
      (data[ns] ?? []).map((value, i) => ({
  ...
  const store: SessionSearchStore = { get: getMock, getKeyed: getKeyedMock };
  return { store, getMock, getKeyedMock, data };
}
...
describe("SessionSearch", () => {
  ...
  describe("index()", () => {
    it("reads the store keyed, with namespace and scope", async () => {
      const { store, getMock, getKeyedMock } = createMockStore();
      const search = new SessionSearch(store);
      await search.index("decisions", SCOPE);
      expect(getKeyedMock).toHaveBeenCalledWith("decisions", SCOPE);
      // And not through the keyless read, which would discard record identity.
  ...
  describe("record key provenance", () => {
    /**
     * These replace three tests that characterized the old fallback —
     * `value['key']`, `''` when absent, `String(...)` when non-string. That
     * fallback was the defect: `get()` returns bare values and `put()` never
     * writes the key into the record, so against a real store the key field is
     * simply absent and every `SearchResult.key` came back empty. The tests
     * passed only because the double injected a `key` field into its values,
     * making the fabrication look like it worked.
     *
     * The contract now is that the key comes from the store's keyed read, so
     * what matters is that it is carried through faithfully and is independent
     * of the record's contents.
     */
    it("takes SearchResult.key from the store's keyed read", async () => {
      const { store } = createMockStore({
        ns1: [{ key: "my-special-key", text: "something" }],
      });
      const search = new SessionSearch(store);
      await search.index("ns1", SCOPE);
    ...
    it("reports the store key even when the value carries a conflicting one", async () => {
      // The store key is authoritative. A stale or unrelated `key`/`id` field
      // inside the value must not override the key the record is filed under —
      // deleting or replacing by the value's field is how the original defects
      // acted on the wrong record.
      const { store } = createMockStore();
      const search = new SessionSearch(store);
      store.getKeyed = vi.fn().mockResolvedValue([
        {
          key: "real-store-key",
          value: { key: "stale-inner-key", text: "hello" },
        },
      ]);
      await search.index("ns1", SCOPE);
      const results = await search.search({ text: "hello" });
      expect(results[0]!.key).toBe("real-store-key");
    });

    it("reports a key for records whose value has no key field at all", async () => {
      // The common real shape: `put(ns, scope, 'k1', { text: '...' })` stores a
      // value with no key field. This previously yielded `''`.
      const { store } = createMockStore();
      const search = new SessionSearch(store);
      store.getKeyed = vi
        .fn()
        .mockResolvedValue([{ key: "k1", value: { text: "no key here" } }]);
      await search.index("ns1", SCOPE);
  ...
  describe("invalidate()", () => {
    ...
    it("the store is read again on next index() after invalidate", async () => {
      const { store, getKeyedMock, data } = createMockStore();
      data.ns1 = [{ key: "a", text: "foo" }];
      const search = new SessionSearch(store);
      await search.index("ns1", SCOPE);
      const callsAfterFirstIndex = getKeyedMock.mock.calls.length;

      search.invalidate("ns1");
      await search.index("ns1", SCOPE);
      expect(getKeyedMock.mock.calls.length).toBe(callsAfterFirstIndex + 1);
    });
  });
  ...
  describe("multi-namespace integration", () => {
    ...
    it("search across multiple namespaces respects limit", async () => {
      const { store, data } = createMockStore();
      data.ns1 = Array.from({ length: 5 }, (_, i) => ({
        key: `a${i}`,
        text: "postgres",
      }));
      data.ns2 = Array.from({ length: 5 }, (_, i) => ({
        key: `b${i}`,
        text: "postgres",
      }));
      const search = new SessionSearch(store);
  ...
  describe("scope handling", () => {
    it("different scopes produce different indices", async () => {
      ...
      expect(getKeyedMock.mock.calls.length).toBe(2);
      expect(getKeyedMock.mock.calls[0]![1]).toEqual(scope1);
      expect(getKeyedMock.mock.calls[1]![1]).toEqual(scope2);
    });
