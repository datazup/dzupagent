...
export async function findExistingConvention(
  ...
  const records = await memoryService.get(
    namespace,
    { scope: CONVENTION_SCOPE_KEY },
    id
  );
...
export async function embedConventions(
  ...
  await semanticStore
    .upsert(
      ...
      conventions.map((c) => ({
        id: c.id,
        text: `${c.name}: ${c.description}. Category: ${c.category}. Pattern: ${
          c.pattern ?? ""
        }`,
        metadata: {
          category: c.category,
    ...
    .catch((err: unknown) => {
      // Non-fatal: vector indexing failures should not break convention
      // analysis. But a silently swallowed upsert leaves the convention
      // semantic index drifting out of sync with what callers believe was
      // embedded, so surface it via a structured log rather than dropping it.
      const message = err instanceof Error ? err.message : String(err);
      // eslint-disable-next-line no-console
      console.error("[memory] convention semantic index upsert failed", {
