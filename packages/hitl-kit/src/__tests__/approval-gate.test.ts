...
describe("InMemoryApprovalStateStore", () => {
  ...
  it("rejects poll() with ApprovalTimeoutError after timeoutMs", async () => {
    await store.createPending("run-1", "ap-1", null);
    await expect(store.poll("run-1", "ap-1", 20)).rejects.toBeInstanceOf(
      ApprovalTimeoutError,
    );
  });

  it("throws DuplicateApprovalError when createPending is called after a terminal outcome", async () => {
    await store.createPending("run-1", "ap-1", null);
    await store.grant("run-1", "ap-1");
    await expect(
      store.createPending("run-1", "ap-1", null),
    ).rejects.toBeInstanceOf(DuplicateApprovalError);
  });

  it("createPending is idempotent while pending: a duplicate does NOT throw (MPCO P6)", async () => {
    await store.createPending("run-1", "ap-1", { question: "first" });
    // Re-creating the SAME still-pending key must not throw (resume case).
    await expect(
      store.createPending("run-1", "ap-1", { question: "second" }),
    ).resolves.toBeUndefined();
    // The originally retained payload is preserved (not overwritten).
    expect(store.getPayload("run-1", "ap-1")).toEqual({ question: "first" });
  });

  it("createPending still throws DuplicateApprovalError once the entry is terminal (MPCO P6)", async () => {
    await store.createPending("run-1", "ap-2", null);
    await store.grant("run-1", "ap-2", { approvedBy: "alice" });
    // Re-creating a key that already reached a terminal outcome SHOULD throw (interface contract).
    await expect(
      store.createPending("run-1", "ap-2", null),
    ).rejects.toBeInstanceOf(DuplicateApprovalError);
  });

  it("a duplicate createPending preserves the existing waiters/outcome (MPCO P6)", async () => {
    await store.createPending("run-1", "ap-3", null);
    const pollPromise = store.poll("run-1", "ap-3", 5_000);
    await store.createPending("run-1", "ap-3", null); // idempotent — must not clobber the waiter
    await store.grant("run-1", "ap-3", 7);
  ...
  it("throws UnknownApprovalError when grant targets an unknown key", async () => {
    await expect(
      store.grant("run-missing", "ap-missing"),
...
describe("ApprovalGate", () => {
  ...
  it("uses defaultTimeoutMs when the per-call timeout is omitted", async () => {
    ...
    await expect(
      gate.waitForApproval("run-1", "ap-1", null),
...
describe("PostgresApprovalStateStore", () => {
  ...
  it("poll() times out with ApprovalTimeoutError", async () => {
    ...
    await store.createPending("run-1", "ap-1", null);
    await expect(store.poll("run-1", "ap-1", 40)).rejects.toBeInstanceOf(
      ApprovalTimeoutError,
    );
  });
  ...
  it("grant on an unknown key throws UnknownApprovalError", async () => {
    const client = new FakeSqlClient();
    const store = new PostgresApprovalStateStore(client);
    await expect(store.grant("no-such", "nope")).rejects.toBeInstanceOf(
      UnknownApprovalError,
    );
  });
});
