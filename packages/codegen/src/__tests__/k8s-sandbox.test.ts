...
let AgentSandboxReconciler:
  | (new (ctx?: Record<string, unknown>) => {
      ...
      ) => Promise<{ requeue: boolean; requeueAfterMs?: number }>;
    })
  | undefined;
...
describeOperator()("buildPodSpec", () => {
  it("maps default security level correctly", () => {
    ...
    const pod = buildPodSpec!(sandbox) as Record<
      string,
  ...
  it("maps strict security level with read-only root and seccomp", () => {
    ...
    const containerSecCtx = containers[0]!["securityContext"] as Record<
      string,
      unknown
    >;
    ...
    expect(containerSecCtx["seccompProfile"]).toEqual({
      type: "RuntimeDefault",
    });
  });
  ...
  it("sets owner reference for garbage collection", () => {
    ...
    const ownerRefs = metadata["ownerReferences"] as Array<
      Record<string, unknown>
    >;
...
describeOperator()("buildNetworkPolicy", () => {
  ...
  it("deny-all sets correct pod selector", () => {
    ...
    const spec = netpol["spec"] as Record<string, unknown>;
    const selector = spec["podSelector"] as Record<
      string,
      Record<string, string>
    >;
    expect(selector["matchLabels"]!["dzupagent.dev/sandbox"]).toBe(
      "test-sandbox"
    );
  });
  ...
  it("custom policy stores allowed hosts in annotation", () => {
    ...
    const metadata = netpol["metadata"] as Record<
      string,
      Record<string, string>
    >;
    expect(metadata["annotations"]!["dzupagent.dev/allowed-hosts"]).toBe(
      "example.com"
    );
  });
});
