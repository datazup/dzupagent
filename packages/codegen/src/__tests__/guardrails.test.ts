...
function makeStructure(packages?: Map<string, { name: string; dir: string; allowedDependencies: string[]; entryPoints: string[] }>): ProjectStructure {
  return {
    packages: packages ?? new Map([
      ['@forgeagent/core', { name: '@forgeagent/core', dir: 'packages/forgeagent-core/', allowedDependencies: [], entryPoints: ['index.ts'] }],
      ['@forgeagent/codegen', { name: '@forgeagent/codegen', dir: 'packages/forgeagent-codegen/', allowedDependencies: ['@forgeagent/core'], entryPoints: ['index.ts'] }],
      ['@forgeagent/agent', { name: '@forgeagent/agent', dir: 'packages/forgeagent-agent/', allowedDependencies: ['@forgeagent/core', '@forgeagent/codegen'], entryPoints: ['index.ts'] }],
      ['@forgeagent/server', { name: '@forgeagent/server', dir: 'packages/forgeagent-server/', allowedDependencies: ['@forgeagent/core', '@forgeagent/agent'], entryPoints: ['index.ts'] }],
    ]),
    rootDir: '.',
  }
}
...
describe('LayeringRule', () => {
  ...
  it('passes when imports follow correct direction', () => {
    const result = rule.check(makeContext([
      {
        path: 'packages/forgeagent-codegen/src/service.ts',
        content: "import { SomeType } from '@forgeagent/core'",
      },
    ]))
    expect(result.passed).toBe(true)
  })
  ...
  it('fails when lower-layer imports higher-layer', () => {
    const result = rule.check(makeContext([
      {
        path: 'packages/forgeagent-core/src/bad.ts',
        content: "import { Agent } from '@forgeagent/agent'",
      },
    ]))
  ...
  it('passes when importing from same layer', () => {
    const result = rule.check(makeContext([
      {
        path: 'packages/forgeagent-codegen/src/service.ts',
        content: "import { MemoryService } from '@forgeagent/memory'",
      },
    ]))
  ...
  it('ignores non-scoped imports', () => {
    const result = rule.check(makeContext([
      {
        path: 'packages/forgeagent-core/src/util.ts',
...
describe('ImportRestrictionRule', () => {
  ...
  it('passes for index-level imports', () => {
    const result = rule.check(makeContext([
      {
        path: 'src/service.ts',
        content: "import { Foo } from '@forgeagent/core'",
      },
    ]))
    expect(result.passed).toBe(true)
  })
  ...
  it('fails for deep imports into package internals', () => {
    const result = rule.check(makeContext([
      {
        path: 'src/service.ts',
        content: "import { secret } from '@forgeagent/core/src/internal/secret'",
      },
    ]))
    expect(result.passed).toBe(false)
  ...
  it('allows explicitly permitted subpaths', () => {
    ...
    const result = ruleWithAllowed.check(makeContext([
      {
        path: 'src/service.ts',
        content: "import { helper } from '@forgeagent/core/utils/helper'",
      },
    ]))
    expect(result.passed).toBe(true)
  })
...
describe('Full Guardrail Pipeline', () => {
    ...
    const files: GeneratedFile[] = [
      {
        ...
        content: [
          "import type { Foo } from '@forgeagent/core'",
          '',
  ...
  it('catches multiple violation types in one run', () => {
    ...
    const files: GeneratedFile[] = [
      {
        path: 'packages/forgeagent-core/src/BadFile.ts',
        content: [
          "import { Agent } from '@forgeagent/agent'",           // layering violation
          "import { internal } from '@forgeagent/core/src/int'", // deep import
