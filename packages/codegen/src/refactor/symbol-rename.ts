...
/**
 * Convenience wrapper that operates directly on a VirtualFS-like object
 * (anything with read/write/list methods).
 */
...
/**
 * Rename a symbol in-place on a VFSLike object.
 * Returns the rename report.
 */
export function renameSymbolInVFS(
  vfs: VFSLike,
  oldName: string,
  newName: string,
  options: RenameOptions = {}
  ...
  for (const change of result.changes) {
    const newContent = updatedFiles.get(change.path)!;
    vfs.write(change.path, newContent);
  }
