...
function mockRichAdapter(attemptBinding: ProviderSessionAttemptBinding): ProviderSessionAdapter {
  const session = {
  ...
  return {
    attemptBinding,
    async steer() { return { kind: 'steer', accepted: true } },
    async interruptTurn() { return { kind: 'interrupt-turn', accepted: true } },
    async forkSession() { return { kind: 'fork-session', session } },
    async startReview() {
      return {
        kind: 'start-review',
        review: {
...
describe('provider-session adapter companion contract', () => {
  it('keeps current SDK and CLI profiles source compatible without rich controls', () => {
    for (const id of ['sdk-default', 'cli-default']) {
      ...
      const adapter: ProviderSessionAdapter = { attemptBinding: binding(profile) }
      ...
      ).toContainEqual(expect.objectContaining({ code: 'CAPABILITY_REQUIRED_UNSUPPORTED' }))
    }
  })
  ...
  it('requires every method advertised by a native rich-control backend', () => {
    ...
    expect(validateProviderSessionAdapter(complete, profile.requiredCapabilities)).toEqual({
      valid: true,
      diagnostics: [],
    })

    const { forkSession: _omitted, ...incomplete } = complete
    expect(validateProviderSessionAdapter(incomplete).diagnostics).toContainEqual(
  ...
  it('does not promote legacy supportsFork into a provider-session capability', () => {
    ...
    expect(binding(sdk).descriptor.capabilities['fork-session'].status).toBe('unsupported')
  })

  it('publishes provider-session subpaths without provider SDK dependencies', () => {
    const adapterPackage = JSON.parse(readFileSync(join(process.cwd(), 'package.json'), 'utf8')) as {
      exports: Record<string, unknown>
      dependencies?: Record<string, string>
    }
    const runtimePackage = JSON.parse(
      readFileSync(join(process.cwd(), '..', 'runtime-contracts', 'package.json'), 'utf8'),
  ...
  it('loads the built runtime and adapter subpaths through their export maps', async () => {
    const runtime = await import('@dzupagent/runtime-contracts/provider-session')
