...
describe("Demo eval suite", () => {
  ...
  it("buildStubAnthropicClient returns configurable responses", async () => {
    const failClient = buildStubAnthropicClient({
      score: 0.1,
      pass: false,
      reasoning: "bad",
    });
  ...
  it('target() falls back to the literal string "unknown" for an input with no canned answer', async () => {
    // Every built-in case uses one of the four known lookup keys, so the
    // `answers[input] ?? 'unknown'` fallback in target() was previously only
    // ever exercised from the "found" side.
    const suite = createDemoEvalSuite();
    const output = await suite.target("a question with no canned answer");
