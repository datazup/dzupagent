...
describe("runSecuritySuite — unknown expectedBehavior", () => {
  it("should fail a case with an unknown expectedBehavior value", async () => {
    ...
    const suite = [
      makeCase({
        expectedBehavior: "unknown" as SecurityTestCase["expectedBehavior"],
      }),
    ];
  ...
  it("should include [FAIL] in details for unknown expectedBehavior", async () => {
    const suite = [
      makeCase({
        expectedBehavior: "invalid" as SecurityTestCase["expectedBehavior"],
      }),
    ];
    const checker: SecurityChecker = async () => ({
      blocked: true,
...
describe("runSecuritySuite — async checker behavior", () => {
  it("should handle a checker that resolves after a delay", async () => {
    ...
    try {
      ...
      const checker: SecurityChecker = async () => {
        // eslint-disable-next-line no-restricted-syntax -- intentional delayed resolution driven by fake timers (advanceTimersByTimeAsync below), not a real wall-clock sleep
        await new Promise((r) => setTimeout(r, 10));
        return { blocked: true, detected: false };
      };
      const resultPromise = runSecuritySuite(suite, checker);
  ...
  it("should propagate checker errors as rejections", async () => {
    ...
    await expect(runSecuritySuite(suite, checker)).rejects.toThrow(
      "checker crashed"
    );
  });
...
describe("runSecuritySuite — details edge cases", () => {
  it("should include expected= for all three behavior types", async () => {
    const behaviors: Array<SecurityTestCase["expectedBehavior"]> = [
      "block",
      "detect",
      "safe",
    ];
    for (const behavior of behaviors) {
      const suite = [
        makeCase({ id: `det-${behavior}`, expectedBehavior: behavior }),
      ];
      const checker: SecurityChecker = async () => ({
        blocked: true,
        detected: true,
