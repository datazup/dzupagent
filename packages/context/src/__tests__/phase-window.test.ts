...
import type { PhaseWindowConfig } from "../phase-window.js";
import {
  HumanMessage,
  AIMessage,
  SystemMessage,
...
function msgs(
  ...
  return contents.map((c) => {
    switch (c.role) {
      case "human":
        return new HumanMessage(c.text);
      case "ai":
        return new AIMessage(c.text);
      case "system":
        return new SystemMessage(c.text);
      case "tool":
        return new ToolMessage({
          content: c.text,
          tool_call_id: c.toolCallId ?? "tc-1",
        });
    }
  });
}
...
describe("PhaseAwareWindowManager", () => {
  ...
  describe("detectPhase()", () => {
    ...
    it("should only scan the last N messages (phaseDetectionWindow)", () => {
      ...
      const messages = msgs(
        {
          role: "human",
          text: "Let me plan the architecture and design the structure",
        },
  ...
  describe("scoreMessages()", () => {
    it("should give system messages the highest base score", () => {
      ...
      // `toolScore` was bound and never used; the base-score comparison this
      // test describes was never actually made.
      expect(systemScore.score).toBeGreaterThan(toolScore.score);
    ...
    it("should add +1 for file paths in content", () => {
      sut = new PhaseAwareWindowManager();
      const messages = [new AIMessage("Check the file at src/utils/helper.ts")];
    ...
    it("should add +2 for error indicators in content", () => {
      ...
      const messages = [
        new HumanMessage(
          "I got a TypeError: cannot read property of undefined"
        ),
      ];
    ...
    it("should subtract 2 for very short messages", () => {
      sut = new PhaseAwareWindowManager();
      const messages = [new HumanMessage("ok")];
  ...
  describe("findRetentionSplit()", () => {
    ...
    it("should never return a negative split index", () => {
      sut = new PhaseAwareWindowManager();
      const messages = msgs({ role: "human", text: "hello" });
  ...
  describe("custom PhaseWindowConfig", () => {
    it("should use custom phases when provided", () => {
      ...
      const messages = msgs({
        role: "human",
        text: "This has the custom-trigger word",
      });
    ...
    it("should fall back to defaults for fields not provided in config", () => {
      ...
      const messages = msgs({
        role: "human",
        text: "I found a bug that causes a crash",
      });
      const result = sut.detectPhase(messages);
      expect(result.phase).toBe("debugging");
    });
