...
describe("ContextTransferService — extended", () => {
  ...
  describe("context serialization", () => {
    it("produces a JSON-serializable IntentContext", () => {
      const msgs = makeMessages([
        "Build feature",
        "Decided to use React. Modified src/app/index.tsx.",
      ]);
    ...
    it("roundtrips through formatAsMessage and back to string", () => {
      const msgs = makeMessages([
        "Setup DB",
        "Decided to use Prisma for ORM. Created src/db/schema.ts.",
      ]);
  ...
  describe("selective transfer by scope", () => {
    it("summary-only scope excludes decisions and files", () => {
      const msgs = makeMessages([
        "Build auth",
        "Decided to use JWT. Created src/auth/jwt.ts.",
      ]);
    ...
    it("decisions-only scope includes decisions but not files", () => {
      const msgs = makeMessages([
        "Edit code",
        "Decided to refactor auth. Modified src/auth/index.ts.",
      ]);
    ...
    it("files-only scope includes files but not decisions", () => {
      const msgs = makeMessages([
        "Implement auth",
        "Decided to use JWT. Created src/auth/service.ts.",
      ]);
    ...
    it("all scope includes everything", () => {
      const msgs = makeMessages([
        "Plan DB",
        "Decided to use Postgres. Created src/db/schema.ts.",
      ]);
    ...
    it("all scope omits working state section when empty", () => {
      const msgs = makeMessages(["Plan DB", "Decided to use Postgres."]);
  ...
  describe("context merging — multiple injections", () => {
    it("can inject context from multiple sources sequentially", () => {
      ...
      const target = messagesWithSystem("You are a debugger", [
        "Fix bug",
        "Looking",
      ]);
  ...
  describe("transfer with tool messages", () => {
    ...
    it("filters out tool messages from summary (only human+ai)", () => {
      const msgs: BaseMessage[] = [
        ...
        new ToolMessage({
          content:
            "this is a very long tool result that should not appear in summary",
          tool_call_id: "tc-1",
        }),
  ...
  describe("token budget enforcement", () => {
    it("truncates context when exceeding maxTransferTokens", () => {
      ...
      const msgs = makeMessages([
        "Build a very large feature with many requirements",
        "Here is a comprehensive plan with extensive details about the implementation approach",
      ]);
    ...
    it("token estimate reflects actual formatted content size", () => {
      const msgs = makeMessages([
        "Build auth module",
        "Decided to use JWT for authentication. Created src/auth/jwt.ts.",
      ]);
      const ctx = service.extractContext(msgs, "implement");
  ...
  describe("custom relevance rules", () => {
    ...
    it("picks highest priority rule when multiple match", () => {
      ...
      expect(custom.getTransferScope("plan_auth", "review_auth")).toBe(
        "decisions-only"
      );
    });
  });
  ...
  describe("edge cases", () => {
    ...
    it("handles messages with Unicode content", () => {
      const msgs = makeMessages([
        ...
        "Decided to use Prisma. Die Datei ist src/db/schema.ts.",
      ]);
      const ctx = service.extractContext(msgs, "implement");
    ...
    it("extractContext caps recent messages to RECENT_MESSAGE_PAIRS (6)", () => {
      ...
      const summaryLines = ctx.summary
        .split("\n")
    ...
    it('formatAsMessage uses "all" scope when toIntent is empty', () => {
      const msgs = makeMessages([
        "Plan",
        "Decided to use Prisma. Created src/db/schema.ts.",
      ]);
    ...
    it("decision patterns match various phrasings", () => {
      ...
      for (const phrase of testCases) {
        ...
        const ctx = service.extractContext(msgs, "test");
        // `toBeGreaterThanOrEqual` takes no message argument — the second one
        // was silently dropped, so a failure named no phrase. `soft` carries a
        // label and reports every failing phrase instead of only the first.
        expect
          .soft(ctx.decisions.length, `decisions extracted for: "${phrase}"`)
    ...
    it("is idempotent when injecting the same context twice", () => {
      ...
      expect(twice.map((m) => m._getType())).toEqual(
        once.map((m) => m._getType())
      );
