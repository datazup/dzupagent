...
| `agent` native-stream model gates (enumerated run-path files + focused specs) | **CLAIMED 2026-08-17 23:28** | current Codex session | Owns only `src/agent/dzip-agent.ts`, `src/agent/dzip-agent-run-coordinator.ts`, `src/agent/model-invocation.ts`, `src/agent/provider-failover.ts`, `src/agent/streaming-run.ts`, `src/agent/streaming-run-types.ts`, `src/agent/streaming-run-iteration.ts`, `src/agent/streaming-run-provider.ts`, new `src/__tests__/native-stream-model-gates.test.ts`, and new server consumer spec `packages/server/src/__tests__/dzip-agent-run-executor-model-gates.test.ts`. Closes T1-1: native `.stream()` must use the shared rate gate once per real provider attempt, enforce model/run timeout plus caller abort across both open and iteration, record real distributed usage once, stop on confirmed cost breach without blaming/failing over providers, and preserve no-replay after partial output/tool results. Historical plugin/coordinator rows above are stale: their target implementations are committed at `3ef792ae` and `c6f8a9e9`; all enumerated paths were clean at claim. No other Agent, pipeline, server runtime, provider, push, deployment, publication, or production surface is claimed. |
