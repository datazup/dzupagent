...
| `agent` native-stream gate review fixes (5 enumerated paths only) | **CLAIMED 2026-08-18 00:05 local; re-scoped 00:09** | continuation Codex session | Owns only `packages/agent/src/agent/{provider-failover,streaming-run-provider,streaming-run-iteration,rate-limit-coordinator}.ts` and `packages/agent/src/__tests__/native-stream-model-gates.test.ts`. The generic failover helper needs one optional fault-classification seam because cancellation during multi-provider stream open is otherwise recorded before the stream-specific retry predicate runs. Regression scope: caller/run cancellation must not record provider failure or emit provider-failure/replay behavior, synchronous iterator cleanup throws must not mask the model timeout/cancellation error, and real streamed usage lacking provider model metadata must use the selected model for distributed-cost pricing. No other Agent/server path, push, provider, deployment, publication, or production surface is claimed. |
