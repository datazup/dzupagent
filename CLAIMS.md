...
| `agent` (tool-result security public-config seam) | **RELEASED 2026-08-17 23:12 local — LOCAL CANDIDATE** (`c6f8a9e9`, follow-up `7b485a54`) | current Codex session | Public `security.promptInjectionToolResults` / `piiToolResults` now project through one shared scanner policy into both generate/tool-loop and real native-stream paths even without `toolExecution`; configured blocks halt before another model turn, scanner exceptions fail closed, public events remain content-free, combined warn-plus-block attribution follows the blocking policy, and no-policy behavior remains compatible. Stable-head focused validation at `7b485a54`: 5 files / 104 tests, Agent source typecheck, test-typecheck, lint with 0 errors / 232 pre-existing warnings, and root scripts 265/265 all passed. After the path-disjoint stuck-cost commit, integrated stable-head validation at `701dfa41` passed source and test typecheck plus the full Agent suite: 368 files passed / 1 skipped, 7665 tests passed / 6 skipped. No push, remote CI, provider, deployment, or production qualification is claimed. |
...
| `agent` (stuck-run model-cost bound) | **RELEASED 2026-08-17 23:14 local — SHIPPED** (`701dfa41`) | current Codex session | Dynamic runtime-block provenance now advances generate through the existing stage-2/stage-3 terminal transition and stops native stream on the first retry after stage 1; static `blockedTools` remain ordinary denials. Economic proof: generate stops `stuck` after 4 model turns / 40 observed tokens, native stream after 3 / 30, versus configured `maxIterations: 100`; each executes the tool twice. Focused adjacent suites 117/117, source + test typecheck exit 0, lint 0 errors / 232 existing warnings, full Agent graph 21/21 tasks, custody-aware filtered typecheck 28/28 tasks. Two isolated mutants deleting each gate's escalation both failed with `iteration_limit`; temporary worktree removed. |
...
| build-custody clean-checkout follow-up | **RELEASED 2026-08-17 23:12 local — SHIPPED** (`a2358f6c`) | current Codex session | ENUMERATED FILES ONLY: moved `packages/memory-ipc/src/__tests__/import-frame-real-store.test.ts` to `packages/memory/src/__tests__/memory-ipc-import-frame-real-store.test.ts`. Exact-head CI for `497bdf38` proved the resource kill was removed but exposed this test's undeclared reverse import of `@dzupagent/memory`; the move keeps the existing forward dependency instead of creating a `memory-ipc -> memory -> memory-ipc` cycle. Clean isolated verification: eval graph 37/37 tasks, moved test 1/1, memory typecheck graph 13/13. Exact-head run `32068688266` passed the build-custody rehearsal live before the later `cf22b00d` push cancelled its long strict-verification remainder. |
