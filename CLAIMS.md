...
| `agent` tool-result focused-command acceptance (one test path only) | **RELEASED 2026-08-18 00:48 local — UNCOMMITTED LOCAL CANDIDATE** | root Codex continuation session | Owns only `packages/agent/src/__tests__/tool-result-prompt-injection.test.ts`. The exact pasted focused command now crosses the public `DzupAgentConfig.security` to native-stream tool-result scanner seam for block, combined warn/redact, and explicit off behavior. Stable `60ac544c` proof: focused file 8/8, Agent test typecheck, strict targeted lint, full Agent aggregate 370 files / 7,687 tests with 1 file / 6 integration tests skipped, and diff check all passed. The target file digest stayed stable throughout. No production source, other test, commit, push, provider, deployment, publication, or production surface changed or was qualified. |
