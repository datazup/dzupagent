...
| `agent` (native-stream model gates — **enumerated files only**) | **in_progress** | native-stream-gate session 2026-08-17 22:00 UTC | ENUMERATED FILES ONLY, `packages/agent` as a whole is NOT claimed: `src/agent/streaming-run.ts`, `streaming-run-{iteration,provider,types}.ts`, NEW `streaming-run-model-gate.ts`, NEW `src/__tests__/native-stream-model-gates.test.ts`, plus a **two-line forward** of the gate deps in `src/agent/dzip-agent-run-coordinator.ts` (`RunStreamDeps` field + its pass-through) and `src/agent/dzip-agent.ts` (the `stream()` deps literal). Closes the 09-11 handoff §5: the native `.stream()` path opens the provider stream directly while `generate()` goes through `model-invocation.ts` + `rate-limit-coordinator.ts`, so rate limit, run deadline/abort, and distributed cost are **not applied to any native stream**. ⚠️ Overlaps the 20:45 `dzip-agent-run-coordinator.ts` row above, whose stated work shipped in `3adcb7dbf`/`e4abaf39f`; treated as stale for those two lines only, and committed pathspec-form. Does NOT touch `streaming-run-{policy,tool-handler,fallback}.ts`. |
