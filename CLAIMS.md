...
| `core` (hooks/middleware contract) | **claimed** | run-lifecycle-hooks session 2026-08-17 19:35 | Lane: the hooks contract packet from `NEXT_SESSION_PROMPT_2026-09-01-post-gap2-contract-lane.md`. Files: `hooks/hook-runner.ts`, `hooks/hook-types.ts`, `middleware/types.ts`, `__tests__/hook-runner.test.ts`, `__tests__/lifecycle-hooks-*.test.ts`, `__tests__/plugin-*.test.ts`. **Disjoint from the 19:30 union sweep**, whose enumerated `core` sites are `plugin/plugin-types.ts`, `events/agent-bus.ts`, `tools/tool-governance.ts` — no file in common. Verified clean at `32c5c538`. |
| `agent` (`src/agent/` run path only) | **claimed** | run-lifecycle-hooks session 2026-08-17 19:35 | Same lane, agent half: dispatch `onRunStart`/`onRunComplete`/`onRunError` at the run boundary and repair `beforeAgent`. Files: `agent/dzip-agent-run-coordinator.ts`, `agent/middleware-runtime.ts`, `agent/run-engine/prepare-state.ts`, new `__tests__/`. **`src/pipeline/**` is another session's live for-each lane and is NOT entered** — the 11 dirty paths are all under `src/pipeline/` or `__tests__/pipeline-for-each-*`; every file above was clean at `32c5c538`. |
| `memory` + `agent-types` | **claimed** | run-lifecycle-hooks session 2026-08-17 19:35 | Taking the stale 17:10 row: its 4 structural errors closed at `ae4e3722` (17:11), 2h+ ago, and both trees are clean. Lane is the architectural remainder, not the error count: `knowledge/__tests__/filesystem-knowledge-store.test.ts:6` still deep-imports `agent-types/src/.../knowledge-store-contract.test.js`, which is why `tsconfig.flipcheck.json:23` still overrides `rootDir` to `..`. Needs a publishable, vitest-free contract entry point. |
