...
| `agent` native-stream gate review fixes (4 enumerated paths only) | **CLAIMED 2026-08-18 00:05 local** | continuation Codex session | Owns only `packages/agent/src/agent/streaming-run-provider.ts`, `packages/agent/src/agent/streaming-run-iteration.ts`, `packages/agent/src/agent/rate-limit-coordinator.ts`, and `packages/agent/src/__tests__/native-stream-model-gates.test.ts`. Regression scope: caller cancellation must not record provider failure, synchronous iterator cleanup throws must not mask the model timeout/cancellation error, and real streamed usage lacking provider model metadata must use the selected model for distributed-cost pricing. No other Agent/server path, push, provider, deployment, publication, or production surface is claimed. |
