...
| `create-dzupagent` (publish bin metadata regression) | **CLAIMED 2026-08-17 23:19 local** | current Codex session | EXACT FILE ONLY: `packages/create-dzupagent/package.json` (`bin` field only) plus this claim row. Restore the named object mapping required by the npm-11 publish-normalization guard after the pre-fix string snapshot reappeared. Validate `check:publish-metadata`, package typecheck/test, and diff check. No workflow, lockfile, publication, or provider path is included. |
