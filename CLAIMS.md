...
| build-custody clean-checkout follow-up | **CLAIMED 2026-08-17 20:55 UTC** | current Codex session | ENUMERATED FILES ONLY: `packages/memory-ipc/src/__tests__/import-frame-real-store.test.ts` (move source), new `packages/memory/src/__tests__/memory-ipc-import-frame-real-store.test.ts`, and this claims row. Exact-head CI for `497bdf38` proved the resource kill was removed but exposed this test's undeclared reverse import of `@dzupagent/memory`; move it to the already-forward-dependent `memory` package rather than creating a `memory-ipc -> memory -> memory-ipc` package cycle. Excludes the active memory knowledge-store paths and every dirty/claimed Agent path. |
