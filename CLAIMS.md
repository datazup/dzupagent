...
| `agent` native-stream model gates (enumerated run-path files + focused specs) | **CLAIMED 2026-08-17 23:28; re-scoped 23:35** | current Codex session | Owns only `src/agent/dzip-agent.ts`, `src/agent/dzip-agent-run-coordinator.ts`, `src/agent/model-invocation.ts`, `src/agent/provider-failover.ts`, `src/agent/streaming-run.ts`, `src/agent/streaming-run-types.ts`, `src/agent/streaming-run-iteration.ts`, `src/agent/streaming-run-provider.ts`, new `src/__tests__/native-stream-model-gates.test.ts`, plus `packages/server/src/runtime/dzip-agent-run-executor.ts` and new `packages/server/src/__tests__/dzip-agent-run-executor-model-gates.test.ts`. Closes T1-1: native `.stream()` must use the shared rate gate once per real provider attempt, enforce model/run timeout plus caller abort across both open and iteration, record real distributed usage once, stop on confirmed cost breach without blaming/failing over providers, preserve no-replay after partial output/tool results, and receive the server run's cancellation signal. Historical plugin/coordinator rows above are stale: their target implementations are committed at `3ef792ae` and `c6f8a9e9`; all enumerated paths were clean at claim. No other Agent, pipeline, server runtime, provider, push, deployment, publication, or production surface is claimed. |

| `agent` permission-tier metadata (enumerated registry/factory/audit paths only) | **CLAIMED 2026-08-17 23:52 UTC — adopting released dirty candidate** | continuation Codex session | Adopts the released, uncommitted candidate at stable source `291b7a68` and dirty-diff identity `fe57b7345ee5bf...c05636`; owns only `packages/agent/src/tools/{tool-tier-registry,human-contact-tool,agent-as-tool}.ts`, `packages/agent/src/mailbox/mail-tools.ts`, `packages/agent/src/agent/{agent-types-config,dzip-agent-resolvers,agent-construction}.ts`, `packages/agent/src/orchestration/supervisor-runner.ts`, Agent tier exports/tests, and the additive `unclassifiedTools` field in `packages/core/src/events/event-types-agent.ts`. Scope is review, commit, and stable-head provider-free qualification of explicit framework tiers, opt-in fail-closed handling for unclassified third-party tools, compatibility audit visibility, and supervisor no-escalation. Does not touch the active native-stream model-gates paths, pipeline, server runtime, provider, push, deployment, publication, or production surfaces. |
